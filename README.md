...
→ Sandbox Runner가 통과한 exact 계획만 실행하고 결과·PoC 반환
...
| 동적검증·Sandbox | 조근석 ([@Potatonion](https://github.com/Potatonion)) | Controller의 세부 안전 정책 검사, Runner의 승인된 exact `ReproductionPlan` 실행, log·동적 결과·redacted PoC 반환 |
...
동적 재현의 역할 연결은 `R6 Verification의 모드·계획 결정 → R4 Runtime Validator의 호출 전제 확인 → R7 Sandbox Controller의 세부 정책 검사 → R7 Sandbox Runner의 exact plan 실행·결과 반환 → R6의 최종 판정`입니다. R7은 실행 불가능·정책 차단·환경 실패를 기록할 수 있지만 모드를 바꾸거나 최종 verdict를 만들지 않습니다.
