"""Bounded live validation for the official Codex LOCAL_EVALUATION route.
...
class LocalCodexValidationError(RuntimeError):
    """A safe local-only reason why Codex cannot be enabled for this run."""
...
class LocalCodexBindingSource(Protocol):
    @property
    ...
    @property
    def binding(self) -> ApprovedCodexExecutionBinding: ...


@dataclass(frozen=True, slots=True)
class LocalValidatedCodexExecutionBinding:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise LocalCodexValidationError(
                "LOCAL_CODEX_SUPPORTED_REVISION_INVALID"
            ) from error
        if (
    ...
    @property
    def executable(self) -> ApprovedCodexExecutable:
        return self.experimental_binding.executable

    @property
    def codex_home(self) -> Path:
        return self.experimental_binding.codex_home
...
@dataclass(frozen=True, slots=True)
class LocalCodexValidationResult:
    """Locally supported revision and its exact secret-free probe receipt."""

    provider: ProviderProfile
    evidence_ref: StoredDataRef
    binding: LocalValidatedCodexExecutionBinding


class LocalEvaluationCodexProcessRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self._inner = inner or CodexCliProcessRunner(
        ...
        if not isinstance(supported_ref, StoredDataRef) or not isinstance(
            ...
            raise LocalCodexValidationError("LOCAL_CODEX_ROUTE_MISMATCH")
...
async def validate_local_codex_binding(
    *,
    records: LocalCodexBindingSource,
    ...
    if (
        ...
        raise LocalCodexValidationError("LOCAL_CODEX_VALIDATION_INPUT_INVALID")

    runner = live_runner or CodexCliProcessRunner(binding=records.binding)
    if unauthenticated_runner is None:
        with tempfile.TemporaryDirectory(prefix="sastsimi-codex-auth-probe-") as root:
            isolated_binding = ApprovedCodexExecutionBinding(
                provider_profile=records.provider,
                client_execution_profile=records.client,
                executable=records.binding.executable,
                codex_home=Path(root).resolve(),
                runtime_environment=records.binding.runtime_environment,
                provider_validation_evidence=None,
            )
            auth_runner = CodexCliProcessRunner(binding=isolated_binding)
    ...
    try:
        async with asyncio.timeout((probe_timeout_ms * 4) / 1_000):
            first, second = await asyncio.gather(
                live_runner.execute(
                    request("local-codex-new-session-a", probe_timeout_ms)
                ),
                live_runner.execute(
        ...
        raise LocalCodexValidationError("LOCAL_CODEX_PROBE_DEADLINE_EXCEEDED") from None
    except asyncio.CancelledError:
        raise
    except Exception as error:
        if isinstance(error, LocalCodexValidationError):
            raise
        raise LocalCodexValidationError("LOCAL_CODEX_PROBE_EXECUTION_FAILED") from None

    if not _valid_success(first) or not _valid_success(second):
        raise LocalCodexValidationError("LOCAL_CODEX_STRUCTURED_PROBE_FAILED")
    if first.provider_session_id == second.provider_session_id:
        raise LocalCodexValidationError("LOCAL_CODEX_NEW_SESSION_PROBE_FAILED")
    if not _failure_is(timeout, "TIMED_OUT"):
        raise LocalCodexValidationError("LOCAL_CODEX_TIMEOUT_PROBE_FAILED")
    if not _failure_is(authentication, "AUTH_REQUIRED"):
        raise LocalCodexValidationError("LOCAL_CODEX_AUTH_PROBE_FAILED")

    checked_at = clock.now()
    if checked_at.tzinfo is None or checked_at.utcoffset() is None:
        raise LocalCodexValidationError("LOCAL_CODEX_VALIDATION_CLOCK_INVALID")
            ...
            canonical_bytes(
                {
                    ...
                    "evidence_kind": "local_codex_live_probe",
