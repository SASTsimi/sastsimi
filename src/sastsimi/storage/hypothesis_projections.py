...
def initial_hypothesis_projection(
    works: WorkService,
    ...
    *,
    ...
    if not proposals or work.work_type != "HYPOTHESIS_PROPOSAL":
        return ()
    if len(proposals) != 1 or proposals[0].origin != "INITIAL":
        raise ValueError("INITIAL_PROPOSAL_REQUIRED")
    ...
    validate_static_current(
        ...
        normalized,
        commit,
        tuple(item for item in rules if isinstance(item, RuleExecutionRecord)),
        attempt=attempt,
    )
