...
class StaticBootstrapResult(ContractModel):
    ...
    workspace_path: Path
    # Recorded once for the whole analysis, not per hypothesis, so a stage
    # that needs it is handed this ref directly instead of reaching for it
    # through a hypothesis-scoped checkpoint chain that never carries it.
