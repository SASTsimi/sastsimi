"""Compose safe operator-only CodeQL provisioning and registry actions.

The CLI is a presentation adapter.  Concrete configuration, registry, source
preparation, and Docker implementations are selected here at the composition
boundary so presentation code never depends on executable adapters directly.
"""
...
def register_database(
    *,
    ...
    try:
        ...
        return _blocked("CONFIG_ERROR", "CODEQL_DATABASE_ALREADY_PUBLISHED")
    except ValueError as error:
        if str(error) == "CODEQL_DATABASE_PROVISION_CANCELLED":
            return _blocked("BLOCKED", "CODEQL_DATABASE_PROVISION_CANCELLED")
        return _blocked("INTEGRITY_ERROR", "CODEQL_DATABASE_REGISTRATION_FAILED")
    except OSError:
        return _blocked("INTEGRITY_ERROR", "CODEQL_DATABASE_REGISTRATION_FAILED")
...
def provision_database(
    *,
    ...
    if language != "python":
        return _blocked(
            "CAPABILITY_UNSUPPORTED", "CODEQL_PROVISION_LANGUAGE_UNSUPPORTED"
        )
    ...
    try:
        ...
        with TemporaryDirectory(
            ...
            with TemporaryDirectory(
                ...
                if (
                    ...
                    return _blocked(
                        "BLOCKED", outcome.reason or "CODEQL_PROVISION_FAILED"
                    )
                published = publish_codeql_database(
                    registry_root=config.database_registry_root,
                    database_root=database_root,
        ...
        return _blocked("INTEGRITY_ERROR", "CODEQL_PROVISION_FAILED")
...
def _blocked(code: OperatorCode, reason_code: str) -> OperatorCommandResult:
    return OperatorCommandResult(
        code,
