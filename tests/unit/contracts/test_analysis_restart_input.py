    ...
    with_defaults = AnalysisRunInput.model_validate_json(
        json.dumps(
            ...
            | {
                ...
                "production_authority_catalog_ref": None,
            }
        )
    )
...
def test_four_descriptor_input_retains_exact_pre_catalog_bytes_and_hash() -> None:
    payload = _legacy() | {
        "workspace_id": "workspace",
        "commit_id": "a" * 40,
        **{
            field: {
                "stored_data_id": digest * 64,
                "data_kind": "artifact",
            ...
            for field, digest in (
                ("production_profile_ref", "b"),
                ("production_onboarding_ref", "c"),
            )
        },
    }
    expected = payload | {
        "meta": meta(
            False,
            record_type="analysis_run_input",
            created_at="2026-09-07T00:00:00.000000Z",
        )
    }
    for extra in ({}, {"production_authority_catalog_ref": None}):
        value = AnalysisRunInput.model_validate_json(json.dumps(payload | extra))
        assert canonical_bytes(value) == canonical_bytes(expected)
        assert content_hash(value) == (
            "da6fce48d4daaafa13e501078e2e005deb42c4e2820a8306be36cdb272d20dad"
        )
