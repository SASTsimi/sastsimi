...
- R7은 요청 목적·가설을 바꾸지 않습니다. R7 sandbox policy owner는 `sandbox_profile_ref`의 외부 접근·격리와 CPU·RAM·disk·PID·요청 가능 최대 시간을 소유·확정합니다. `RUN_SANDBOX`는 current `RunPolicyState`와 `LOCAL_ONLY`를 고정하며 live asset·외부 계정·허용되지 않은 egress를 차단합니다. Sandbox 안에서 command·PoC·관찰·재시도를 자율적으로 선택해도 이 입장 경계를 우회할 수 없습니다.
