...
- [ ] R7 자율 retry에서 같은 Agent session의 조정은 현재 attempt에 기록하고, session 재시작이 필요한 work-level retry만 새 attempt를 사용합니다. 외부 설정·정책·승인·resource profile 변경을 기다릴 때만 `BLOCKED`이며 해소 뒤에는 `trigger=RESUME`인 새 attempt로 재개합니다.
