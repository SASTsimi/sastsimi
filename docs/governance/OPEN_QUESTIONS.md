...
1. 담당자 계정 확인, 대체 검토자와 GitHub 권한
   - 확정된 내용: 8개 역할을 맡을 사람은 정했습니다. 현재 #3에는 `@zv9uvr`, #6에는 `@kimhr8463`, #7에는 `@UltraPeachKeen`이 실제 담당자로 지정되어 있습니다.
   - 무엇을 정해야 하나: 위 실제 담당자가 팀이 알려 준 `@meow`, `@kimhr8465`, `@UltraPaechKeen`과 각각 같은 사람의 계정인지 확인해야 합니다. `@v1sion`을 #1·#4·#5의 공동 담당자로 지정할지도 확인해야 합니다. 이어서 역할별 대체 검토자, 중앙 문서 승인과 브랜치 병합 권한을 정합니다.
   - 정하지 않으면 생기는 문제: Issue가 잘못된 사람에게 배정될 수 있고, `CODEOWNERS`(파일별 자동 검토 요청 설정)를 안전하게 추가할 수 없습니다.
