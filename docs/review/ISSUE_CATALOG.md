...
| Technical ACCEPT만 받은 TRUE | run 초기화에서 준비된 정책 소비·Rule Scope의 금지 테스트 판정과 `PrimitiveAdmissionDecision`이 끝날 때까지 result Primitive·Chaining·Reporter 금지 (Rule Scope Gate는 새 policy collection/parsing을 실행하지 않음) |
