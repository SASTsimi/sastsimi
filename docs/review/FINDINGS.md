...
| ID | 상태 | 쉽게 말하면 | 정확한 문제 | 처리·완료 조건 | 담당 역할 | Issue |
|---|---|---|---|---|---|---|
| H-002 | IN_PROGRESS | 공통 계약을 작성한 뒤 R2 대행 검토와 R6·R7 리뷰에서 찾은 결함까지 수정했습니다. 이제 수정본 재확인과 R3 통합 검토가 남았습니다. | 정적 사실·위치·도구 범위, 질문별 반증 결과, 동적 실행 상태와 관측 의미, 정확한 Verification revision 참조, 안전한 오류 문구를 정본·전문·Wiki에 동기화함 | R2 대행 검토 반영을 최종 확인하고 R6·R7 재검토와 R3 생산·소비 검토 기록을 Issue #13과 PR에 남기면 `RESOLVED` 후보로 변경 | PM + 정적/동적/검증/통합 | [#13](https://github.com/SASTsimi/sastsimi/issues/13), [#3](https://github.com/SASTsimi/sastsimi/issues/3), [#7](https://github.com/SASTsimi/sastsimi/issues/7), [#8](https://github.com/SASTsimi/sastsimi/issues/8) |
