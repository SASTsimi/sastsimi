...
| ID | 상태 | 쉽게 말하면 | 정확한 문제 | 처리·완료 조건 | 담당 역할 | Issue |
|---|---|---|---|---|---|---|
| H-002 | IN_PROGRESS | 공통 ID·상태·오류·시간·수정 이력의 이름을 정본 계약에 통일했고, 담당 파트 확인만 남았습니다. | `logical_record_id`, 상태별 소유 field, `created_at`, `DataGap`, `AnalysisError`, `failure_reason`, `hypothesis_disproved`, `disproof_evidence_refs`의 구조와 자동 `FALSE` 금지 규칙 작성 완료 | R2·R6·R7·R3가 생산·소비 의미를 실제로 확인한 기록을 Issue #13과 PR에 남기면 `RESOLVED` 후보로 변경 | PM + 정적/동적/검증/통합 | [#13](https://github.com/SASTsimi/sastsimi/issues/13), [#3](https://github.com/SASTsimi/sastsimi/issues/3), [#7](https://github.com/SASTsimi/sastsimi/issues/7), [#8](https://github.com/SASTsimi/sastsimi/issues/8) |
