...
| H-005 | OPEN | Docker 재현과 공식 정책 수집의 threat model·ADR 및 안전 설계가 아직 완료되지 않았습니다. | R7-03 #22의 daemon 격리, image/build provenance·공급망, 외부 통신·network policy, 자원 회수와 공식 정책 수집 경계의 설계·구현·교차 검토 미완료 | R4는 exact reference와 nullable/cleanup 조합을 정의했고 R7-04는 Controller 판정·실제 환경·Runner log·PoC·동적 관측의 상세 payload, redaction, finalization barrier, 자원 ledger와 부정 시나리오를 설계함. H-005는 R7-03 #22의 threat model·ADR와 R3 구현 검증, R5 정책 출처 인증·최신성 교차 검토가 끝날 때까지 OPEN 유지 | 동적검증·Sandbox + Gate | [#6](https://github.com/SASTsimi/sastsimi/issues/6), [#8](https://github.com/SASTsimi/sastsimi/issues/8), [#21](https://github.com/SASTsimi/sastsimi/issues/21), [#22](https://github.com/SASTsimi/sastsimi/issues/22) |
