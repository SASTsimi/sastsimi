...
| H-005 | OPEN | Docker 재현과 공식 정책 수집의 안전 설계가 아직 구현·교차 검토 전입니다. | Docker reproduction과 policy capture의 구현·검증 미완료 | R4는 exact reference와 nullable/cleanup 조합을 정의했고 R7은 Controller 판정·실제 환경·Runner log·PoC·동적 관측의 상세 payload, redaction, 자원 ledger와 부정 시나리오를 설계함. daemon·image 공급망·외부 통신 구현 검증은 R7/R3, 정책 출처 인증·최신성은 R5 교차 검토가 필요 | 동적검증·Sandbox + Gate | [#6](https://github.com/SASTsimi/sastsimi/issues/6), [#8](https://github.com/SASTsimi/sastsimi/issues/8), [#21](https://github.com/SASTsimi/sastsimi/issues/21), [#22](https://github.com/SASTsimi/sastsimi/issues/22) |
