...
| B-007 | OPEN | 일부 핵심 결과를 누가 어떤 저장 요청으로 확정하는지 빠져 있습니다. | `CodeWorkspace`, `ToolRunResult`, schema-valid `HypothesisProposal`, `AnalysisRunResult`가 pipeline output에는 있지만 공통 `SAVE_RESULT` registry의 `result_kind → schema → 유일 producer` 연결이 없음 | 네 output 각각의 저장 entry point·유일 producer·current pointer·atomic commit 사용 여부를 공통 계약과 검증 시나리오에 확정 | R4 공통 계약 + R2·R3·R8 검토 | [#5](https://github.com/SASTsimi/sastsimi/issues/5), [#4](https://github.com/SASTsimi/sastsimi/issues/4) |
| B-008 | OPEN | 가설 생성 전 Docker 준비가 기존 동적 재현 권한을 우회하지 않게 계약을 정해야 합니다. | run-init Docker baseline 준비는 가설별 exact request와 attempt가 필요한 기존 `RUN_SANDBOX`를 그대로 사용할 수 없지만, 별도 action·입력·출력·상태·예산·권한 연결이 미확정 | 실제 Docker 상태를 바꾼다면 run-init 전용 action과 requester, exact input/output, retry·취소·보안 경계와 R7 상한·R8 예산을 확정한다. 확정 전에는 상태를 바꾸지 않는 readiness 확인만 허용 | R3·R4·R7·R8 | [#4](https://github.com/SASTsimi/sastsimi/issues/4), [#5](https://github.com/SASTsimi/sastsimi/issues/5), [#8](https://github.com/SASTsimi/sastsimi/issues/8), [#9](https://github.com/SASTsimi/sastsimi/issues/9) |
...
| H-009 | RESOLVED | 발동할 수 없는 체이닝 재료 회수 절차를 제거하고 admission을 등록 시점 1회 판정으로 확정했습니다. | current `PrimitiveIndexState`에서 Primitive를 빼는 트리거는 `PrimitiveAdmissionDecision`이 `DENY`로 바뀌는 것 하나뿐이다. `af6ffc0`이 정책을 run 안에서 고정한 뒤 남은 조건은 `VerificationResult`·`TechnicalEvidenceReview`·`RuleScopeImpactReview`의 검증 근거 revision 변경인데, 셋 다 새 Verification generation에서만 새 revision이 되고 그 generation은 Technical `REVISE`에서만 만들어져 그 시점에는 result Primitive가 아직 없다. `05`는 사람 주도 과정을 `AnalysisRunResult` 확정 뒤로 두고, 중단·재개는 저장된 Primitive를 취소하지 않는다. | ADR-014로 admission을 Primitive 등록 시점의 1회 판정으로 확정하고 회수 절차를 제거했다. 정책이나 판정이 달라졌으면 다음 run에서 새로 판정한다. run 도중 회수가 필요해지면 트리거·권한·전파 범위를 함께 설계한다. | PM + Gate·Finding·보고서 + 검증·반박·플레이북 | [#5](https://github.com/SASTsimi/sastsimi/issues/5) |
