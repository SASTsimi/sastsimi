...
|---|---|---|---|---|---|---|---|
| 전체 관리 | Architecture v5 검토 중 설계 초안 전체 검토·승인 | [#1](https://github.com/SASTsimi/sastsimi/issues/1) | 김태현 `@taehyeon-git`, 윤희섭 `@YHS-Sec` | — | `README.md`, `docs/architecture-v5/README.md`, 협업·검토 문서 | R1–R8, 최종 검토·승인 담당자 | OPEN |
...
| R3 | 통합 구현 가능성·계약 준수 테스트 | [#4](https://github.com/SASTsimi/sastsimi/issues/4) | 김태현 `@taehyeon-git`, 윤희섭 `@YHS-Sec` | `review/integration-feasibility` | `01`, `03`, `08`, `09`, `10`, `11`, `13` | R4, 변경 영향 역할, R8 | OPEN |
| R4 | PM·Control Plane·중앙 계약·워크플로 | [#5](https://github.com/SASTsimi/sastsimi/issues/5) | 김태현 `@taehyeon-git`, 윤희섭 `@YHS-Sec` | `review/control-plane` | root README, `01`, `03`, `08`, `09`, `10`, `11`, `13`, governance/review | 전체 역할 | OPEN — R4 설계 완료 기록 유지 |
...
| R6 | 찬반 근거·동적 모드·`EnvironmentRequirements`·`ReproductionPlan`·최종 판정 | [#7](https://github.com/SASTsimi/sastsimi/issues/7) | 임채민 `@UltraPeachKeen` | `review/verification` | `03`, `04`, `07`, `08`, `13` | R2, R7, R1, R4, R5, R8 | OPEN |
| R7 | Controller 정책 판정·실제 환경 비교·Runner exact 실행·log/PoC 상세 artifact와 동적 결과 조립 | [#8](https://github.com/SASTsimi/sastsimi/issues/8) | 조근석 `@Potatonion` | `review/dynamic-sandbox` | `04`, `07`, `08`, `10`, `13` | R6, R4, R8, R3, R5 | OPEN |
...
R6가 동적 재현 모드·환경 요구사항·계획을 결정하고, R4 trusted runtime이 requirements/plan schema·reference·호출 권한·상태·예산을 검사합니다. R7의 Sandbox Controller는 세부 실행 정책을 검사하고 Runner는 실제 환경을 비교해 필수 조건이 맞을 때만 승인된 exact 공격 단계를 실행합니다. R8이 만든 예산 profile도 R4 runtime을 통해 강제됩니다. 연결 역할은 상대 역할의 전문 결정을 대신하지 않고 생산자·소비자 계약을 교차 검토합니다.
...
역할에 사용하는 실제 GitHub 계정은 #1·#4·#5 `@YHS-Sec`, #3 `@zv9uvr`, #6 `@kimhr8463`, #7 `@UltraPeachKeen`으로 확정했습니다. #10은 김태현 `@taehyeon-git`이 최종 검토·승인 담당자로 관리합니다.
