...
- final TRUE: validated PoC와 exact Technical `ACCEPT` 뒤 제공 능력 하나마다 `result`가 있는 Primitive 하나 생성
...
### 3. Technical admission과 Rule Scope 분리

validated PoC가 있는 final TRUE가 exact Technical `ACCEPT`을 받으면 result Primitive와 Chaining 자격을 얻습니다. Rule Scope Impact Gate는 보고·제출 가능성만 판단합니다. `FAIL | UNCERTAIN | DENY`는 Reporter를 막지만 이미 admission된 Primitive나 Chaining을 취소하지 않습니다.

Technical Gate는 verdict·근거 연결 외에도 금지된 재현으로 근거가 오염되지 않았는지, 실제 코드 경로와 restrictions가 정확한지 검토합니다.
...
시간·비용·작업 수 제한은 R8의 전역 예산 정책과 Runtime Validator가 강제합니다. token은 사용량만 관측하며 계획값 초과·누락만으로 체이닝을 중단하지 않습니다. 중복 fingerprint, ancestor cycle 또는 전역 예산 중단은 취약점 `FALSE`가 아닙니다.
