...
- 대체 대상: [ADR-002](./ADR-002-sandbox-policy-enforcement.md), [ADR-004](./ADR-004-r6-request-r7-poc-production.md)의 과거 실행기 중심 세부 계약
...
1. `ReproductionPlan`에는 목적·가설·환경 요구사항·재현 목표·전략 요약과 선택적 `requested_evidence`만 둡니다. 재현 범위를 구분하는 enum과 exact step·command·payload·cleanup allowlist는 current 계약에서 고정하지 않습니다.
...
7. Runtime Validator는 current generation의 exact request·EnvironmentRequirements·sandbox profile과 same-work `ReproductionPlan` reference closure만 검사합니다. `ReproductionPlan` 내부 command·payload·order·cleanup 의미를 사전 승인하거나 Agent의 내부 command를 검사하지 않습니다.
...
- baseline image·recipe는 persistent baseline으로 보존하고, 가설별 container·network·volume·임시 build는 session ephemeral 자원으로 분리해 성공·실패·차단·취소 뒤 정리합니다.
...
- `agent_invoked`는 외부 경계 승인 뒤 Sandbox 안의 R7 Agent 실행 단계만 뜻합니다. 사전 requirements·plan 작성 호출과 구분하며, Agent 실행 전 정책 차단은 `agent_invoked=false`, `failure_category=POLICY`, `poc_ref=null`인 결과로 기록합니다.
- Session Manager는 Agent 호출·중단, command 허용, retry와 cleanup 전략을 결정하지 않습니다.

## retry와 실패

- 같은 session 안의 command·PoC·환경 조정은 한 attempt의 event입니다.
- 재시도 가능한 환경·실행·관찰 실패는 R8 한도 안에서 같은 work의 새 attempt로 retry하고, 이전 실패 event와 결과는 보존합니다.
- `BLOCKED`는 `failure_category=PLAN | ENVIRONMENT | EXECUTION | OBSERVATION | POLICY | TIMEOUT | OTHER`와 함께 현재 attempt를 진행할 수 없지만 같은 work에서 조건 수정·backoff·재구성·retry가 가능한 경우 사용합니다. 외부 설정·승인·resource profile 변경을 기다리는 경우도 같은 규칙을 적용합니다.
- 복구 불가능하거나 retry 한도를 소진하면 `FAILED + failure_category`와 `INCONCLUSIVE`로 끝냅니다.
- 실패 attempt 결과는 routing·감사용으로 기록할 수 있지만 R6 final verdict의 evidence로 소비하지 않습니다. 실패는 R6의 `FALSE | HOLD`로 자동 변환하지 않고 final VerificationResult와 Technical Gate를 만들지 않습니다.
...
관련 schema와 result-owner registry는 새 MAJOR로 배포합니다. 이전 실행기 중심 schema·field와 세부 실행 지시는 자동 변환하지 않습니다. 과거 기록은 history로만 보존하며 current `AgentLog`나 결과로 추정하지 않습니다.

## 검증 조건

- 정본, Wiki, Mermaid, governance와 validator가 같은 역할·필드·상태를 설명함
- 외부 경계와 Sandbox 내부 자율성의 충돌이 없음
- 재시도 가능한 실패의 `BLOCKED`와 최종 `FAILED`, 외부 대기 조건이 구분됨
