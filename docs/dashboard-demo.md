...
3. `정적분석 도구`와 `분석 파이프라인`에서 현재 단계를 설명합니다.
4. `Source → Sink·체이닝 시각화`에서 데이터 흐름과 부모 가설을 설명합니다.
5. PoC와 정적·동적 증거를 각각 열어 판정의 근거를 보여 줍니다.
6. Markdown 보고서를 렌더링 화면과 원문으로 전환합니다.
7. `Esc`로 발표 모드를 종료한 뒤 Agent 로그와 LLM 요청·응답의 안전한 사본을 보여 줍니다.
8. `전체 결과 ZIP 다운로드`로 로그, JSON 아티팩트, PoC·증거와 보고서를 한 번에 받습니다.
