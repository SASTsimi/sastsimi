...
## INPUT_SLOTS
- `finding`, `verification`, `technical`, `scope`, `cwe`, `pro`, `con`, `facts`, `contexts(OPTIONAL_MANY)`, `dynamic_request`, `run_policy_state`, `collection`, `policy`, `dynamic`, `poc`, `sandbox_policy`, `environment_recipe`, `environment`, `agent_log`, `chaining_results(OPTIONAL_MANY)`, `chaining_primitives(OPTIONAL_MANY)`

`FindingIndexState` is not a registered prompt input slot: the Runtime Validator checks its exact current pointer at authorization, provider invocation, and draft save. For a CHAINING-origin Finding, `chaining_results` contains only the exact source-match result and required lineage closure, and `chaining_primitives` is set-equal to their `input_primitive_refs`; for an INITIAL-origin Finding both are empty. `input_primitive_refs` remains `[StoredDataRef]` only. Read Primitive `source_verification_ref` and non-null `technical_review_ref` through `chaining_primitives`, then verify that their de-duplicated union equals the ChainingResult's `source_result_refs`. Use the Primitive refs, match candidates, chained proposals, exclusions, and no-match reasons to write report-template §9. Never add, infer, or represent CWE, Rule Scope, or Primitive admission refs as ChainingResult provenance.
...
## DECISION_CRITERIA
Use only exact refs; never latest lookup or cross-attempt artifacts. Every path:line must occur in exact Verification evidence locations. State only verified claims; preserve restrictions/limitations/unresolved conditions; use validated `poc_ref` only. For §9, describe Chaining matches and proposals as provenance or unvalidated follow-up unless their child has its own validated upstream chain; never turn a match into a confirmed vulnerability claim. Remove credential, token, cookie, private key, session secret, unnecessary PII, and private/raw reasoning before storage.
