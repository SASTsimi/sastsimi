...
- AST·OpenGrep·CodeQL이 같은 파일·줄을 지목한 결과의 교차 비교
...
- 저장된 Agent 이벤트를 cursor로 이어 받는 증분 로그와 과거 실행 재생
- 실패 코드에 따른 인증·Docker·정적분석 도구별 복구 안내
- 단계별 JSON·텍스트 아티팩트와 비밀값을 제거한 LLM 요청·응답
- 저장된 reference를 기반으로 한 아티팩트 입력→출력 관계 추적
- 검증된 PoC와 정적·동적 증거의 분리된 목록 및 개별 다운로드
- Finding별 가설→Source/Sink→PoC·증거→보고서 전체 추적과 브라우저 로컬 고정
- 가설의 Source → 취약점 → Sink 흐름과 Chaining 부모·자식 관계 시각화
- 한국어·준비된 영문 보고서의 렌더링/원문 전환과 개별 다운로드
- 다른 저장 분석과 상태·진행률·Finding·가설·LLM 사용량 비교
- 항목을 선택한 ZIP 또는 `전체 결과 ZIP 다운로드`를 통한 로그·아티팩트·보고서 일괄 저장
- 시연 순서와 요약이 포함된 `발표 패키지 ZIP 다운로드`
...
화면에서 상태·보기·다운로드를 제공하고 전체/발표 ZIP의 `reports/en/`에 함께 포함됩니다.
대시보드는 영문 내용을 새로 번역하거나 판정을 변경하지 않습니다.

증분 로그는 저장된 Agent 활동 이벤트를 마지막 event ID 이후부터 읽습니다. 과거 실행
재생 역시 저장 이벤트의 시간 순서를 화면에서 다시 보여 주는 기능이며 분석이나 Provider를
다시 실행하지 않습니다. Finding 고정 상태는 브라우저의 local storage에만 저장되고 분석
데이터를 수정하지 않습니다. 실행 비교도 두 저장 결과를 읽기만 합니다.
