...
### 2026-09-11 Context crash-recovery and reuse decision

- `ContextRetrievalService.recover_pending` is the explicit restart entry for
  the pre-claim and claimed/pre-dispatch phases. The caller resubmits the same
  non-persisted intent plus exact work, action, decision, reservation, plan,
  workspace, bundle, and identity inputs. Recovery exact-resolves and
  recomputes that closure before it claims or dispatches anything; it never
  creates a second action, reservation, or request.
- T08 does not reuse a COMMITTED `CodeContextResponse` across attempts. The
  response is attempt-owned, and no approved contract safely republishes an
  old-attempt response as a new-attempt output. The normalized fingerprint is
  retained for durable request counting, duplicate detection, and audit. A new
  claimed request consumes a slot; only the same pending attempt resumes
  without another slot through `recover_pending`.
- The Context integration test is named
  `tests/integration/static_analysis/test_context_retrieval_flow.py` because
  pytest's default import mode cannot safely collect a unit and integration
  module that share the basename `test_context_retrieval.py`.
- These decisions add no schema, table, mutable cache, or new authority. The
  Wave 2C file allowlist otherwise remains unchanged.
...
- CodeQL starts only when a trusted `StaticOutputQuotaPort` proves an OS/container write-denying root bound to the exact action, attempt, profile revision, and `max_attempt_output_bytes`. The same port exposes a sticky `limit_breached` status and trusted `breach_evidence` after every process so a denied cap+1 write cannot be mistaken for a valid result. The original database and query pack remain immutable; their verified working copies, common cache, temporary files, logs, and SARIF all stay under that root. Polling and size measurement are defense in depth, never the hard-cap authority. Oversized or non-regular SARIF is never partially parsed.
...
- Create `tests/integration/static_analysis/test_context_retrieval_flow.py`.
...
- Produces: the sole new domain record `StaticToolProfile`; `StaticToolRequest.tool_profile_ref`; its generated schema and typed registry/resolver; non-persisted `CanonicalRepositorySource`, `MonotonicActionDeadline`, `ProcessSpec`, `ProcessReceipt`, `StaticActionReceipt`, `ProcessResult`, `StaticRuleMapping`, `StaticOutputQuotaBinding`, concrete transport `ToolCapabilityResult`, `StaticCapabilityObservation`, `StaticToolObservation`, `PublishedWorkspaceMaterial`, `PublishedStaticToolMaterial`, `PrebuiltCodeQLDatabase`, workspace storage lease/policy projections, `StaticProcessAdapter`, `StaticOutputQuotaPort`, `StaticExternalExecutionPort`, `StaticToolProfileResolverPort`, `StaticAttemptPublisherPort`, `WorkspaceStoragePort`, `WorkspacePreparationPublisherPort`, and `WorkspaceLocatorPort`.
...
**Interfaces:**
- Consumes: trusted absolute CodeQL executable, exact expected CLI version, `PrebuiltCodeQLDatabase` bound to the current workspace/commit, exact query-pack path/digest, complete rule catalog, selected rule IDs/packs, a trusted write-denying `StaticOutputQuotaPort` binding for the guarded attempt root, `StaticToolProfile.max_attempt_output_bytes + max_output_file_bytes + max_artifact_read_bytes`, the action's shared deadline, safe process runner, and current request.
...
Before any CodeQL process starts, exact-resolve a trusted `StaticOutputQuotaBinding` and require the same lease/root/action/attempt/profile/cap and enforcement evidence before copying inputs, after the version probe, and after analysis. Capability probing uses its own non-overlapping `probe_root` and quota lease; it never rebinds or consumes the analysis attempt's root or lease. Each probe creates one identity-tracked child directory and removes only that exact owned directory on runner construction failure, duplicate rejection, command-validation failure, process failure, cancellation, or success. The binding's sticky `limit_breached` must remain false; a true value is valid only with trusted `breach_evidence` and always discards the result as `STATIC_OUTPUT_LIMIT`. Missing, contradictory, plain-directory, polling-only, stale, substituted, or weakened bindings fail closed with zero analysis calls. The requested root must be absolute and have a symlink/junction/reparse-free parent chain before backend lookup; the returned binding root must pass the same checks before any write. Create one exclusive execution tree under the analysis root, reject every descendant symlink/junction/reparse point and every regular file whose link count is not exactly one, copy the verified database and query pack there, and re-hash both copies before use. Analyze only the database copy because CodeQL query execution writes results/cache into its database; never write into the immutable source database. Put `--common-caches`, temporary paths, working directory, logs, and exactly one SARIF path inside the same quota root, and add CodeQL's own disk-cache limit only as defense in depth. A concurrent watcher may request faster cancellation, but it is not the write-denying authority. After process exit, recheck the exact quota binding, original database/query-pack digests, and resolved-root/type/count/size closure. The SARIF path must be one regular file inside the owned output directory, not a hard link to an existing external file, and no larger than `max_output_file_bytes`; the whole execution tree must remain within `max_attempt_output_bytes`. Read at most `max_artifact_read_bytes + 1` bytes and require the SARIF to fit both the output-file and read caps. Compare pre-open/post-open identity and size, and parse only if the complete file fits. Never truncate JSON and attempt to parse it. Oversize, quota violation, replacement, or short/changed read yields `ToolRunResult.status=FAILED` with `DataGap(stage=STATIC_ANALYSIS, code=STATIC_OUTPUT_LIMIT, reason=TRUNCATED)` and `AnalysisError(stage=STATIC_ANALYSIS, code=STATIC_OUTPUT_LIMIT)` because no CodeQL finding is usable; `PARTIAL` is permitted only if another independently complete, bounded adapter output has already been validated, which this single-SARIF CodeQL path does not produce. The oversized SARIF is not stored as a raw artifact.
...
- [ ] **Step 1: Write RED capability, command-policy, and output-boundary tests.** Assert exact version match, missing executable, wrong version, database/query-pack digest mismatch, and every prohibited build/autobuild command. Use a fake executable; do not require host CodeQL. Cover attempt-directory escape, pre-existing output, every descendant symlink/junction/reparse point, every regular descendant with `st_nlink != 1`, hard-link replacement where supported, directory quota overflow during execution, a trusted backend breach latch after a denied cap+1 write, SARIF at exactly and one byte above `max_output_file_bytes`, exactly and one byte above `max_artifact_read_bytes`, size/identity change during read, and a second unexpected file that pushes total size over `max_attempt_output_bytes`. Assert every cap is independent, contradictory or breached quota status fails closed, over-limit content is never passed to the JSON parser or artifact store, and the exact gap/error/status above is produced.
...
`max_requests_per_hypothesis` uses the derived durable view above to count distinct claimed `code_request_id` values across generations for that hypothesis. A repeated normalized fingerprint does not authorize cross-attempt response reuse in T08 and a new claimed request consumes a slot. Same-attempt crash recovery exact-resolves the original pending request instead of creating another ID. The service never silently widens limits or relies on process memory.
...
- CodeQL's working DB, working query pack, common cache, temporary files, logs, and SARIF are confined to one exact action/attempt/profile-bound OS/container write-denying root; polling is only a cancellation aid. Separate total-directory, output-file, and read caps remain enforced, and cap+1, replaced, partial, or truncated content is never parsed or stored as usable raw evidence.
...
- [ ] Confirm CodeQL refuses to start without an exact hard-quota binding; rechecks sticky backend breach evidence after each process; analyzes only verified attempt-owned DB/query-pack copies; confines common cache, temporary files, logs, and SARIF to that root; preserves the original inputs; and rejects binding substitution, cap+1 burst, descendant reparse/hard-link escape, race/replacement, or bounded-read overflow without truncated parsing.
