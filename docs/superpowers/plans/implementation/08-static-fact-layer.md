...
Recovery uses the existing `ExternalDispatch`, reservation/ledger, and transition journals plus the attempt-owned `ProcessReceipt` set and its single aggregate `StaticActionReceipt`; it adds no database table. A prepared but undispatched claim may resume only while the same action, attempt, reservation, profiles, and inputs remain valid. For `DISPATCHED` without `returned_at`, an exact complete action receipt with matching action/attempt/input fingerprint, ordered process-receipt hashes, guarded relative observation path, bounded observation size/hash, and a successfully decoded closed typed observation lets recovery mark returned and continue accounting/publication without running the operation again. This works for one-process CodeQL/AST, multi-batch OpenGrep, and multi-command Git. Missing, temporary, torn, stale, oversized, symlinked, path-escaping, or mismatched receipt/payload is an ambiguous external outcome, not caller cancellation: call the existing uncertain-dispatch guard and transition the work `RUNNING -> BLOCKED` with `waiting_for=(DEPENDENCY,)` and `stop_reason=EXTERNAL_OUTCOME_UNKNOWN`; the current attempt ends `CANCELLED` only as required bookkeeping for that nonterminal work transition. No `ToolRunResult` or rule record is published while the actual external outcome is unknowable, and neither operation factory nor tool is invoked again. The work remains BLOCKED and is excluded from normalizer fan-in until trusted explicit reconciliation proves one exact outcome. Reconciliation must either validate the prior receipt and finish its return/account/result closure exactly once, or prove no usable execution and use the existing recovery/retry path; the work cannot reach a terminal status without the resulting canonical `ToolRunResult`. Recovery never guesses, treats ambiguity as ordinary caller cancellation, or automatically retries. For RETURNED plus RESERVED, commit measured action-receipt usage once; for COMMITTED reservation without output, rebuild the typed observation from the exact receipt/files and publish once; for PREPARED/COMMITTED transition, use existing transition replay. A stale/denied/unclaimed decision, mismatched reservation, or ambiguous dispatch always produces zero new adapter/process invocations.
...
The publisher checks that every ordinary terminal `STATIC_TOOL` work closure—including every caller-cancellation closure—has exactly one `ToolRunResult` and that each result/attempt/work combination matches exactly one publishing row above. The solely nonterminal exception is `EXTERNAL_OUTCOME_UNKNOWN`: work is BLOCKED, the current attempt is bookkeeping-CANCELLED, and no result is permitted until explicit reconciliation. The publisher rejects a nonexistent `WorkStatus.SKIPPED`, `SUCCEEDED` with gaps/errors/incomplete rule telemetry, `PARTIAL` without usable committed evidence and gaps, `FAILED` without errors, ordinary caller-cancelled work with anything other than the declared SKIPPED-without-usable-output or PARTIAL-with-usable-output result, ambiguity mislabeled as caller cancellation, timeout represented as cancellation, and any empty-output ordinary cancellation. It validates catalog equality, exact profile/configuration refs, tool/version/kind, attempt identity, bounded raw reference, timestamps, and output closure before the atomic publication.
...
- Every ordinary terminal `STATIC_TOOL` work closure has exactly one canonical `ToolRunResult`, including caller cancellation before trusted execution and cancellation-won races. Cancellation without usable output publishes SKIPPED plus a required gap; cancellation after complete usable output publishes PARTIAL plus its exact raw ref and gap, while attempt/work remain CANCELLED. Every ordinary RULE_BASED caller cancellation has the exact same-attempt `RuleExecutionRecord`, with selected unexecuted rules marked `NOT_EXECUTED/CANCELLED`; result/rule/attempt/work/output refs commit atomically. `EXTERNAL_OUTCOME_UNKNOWN` is distinct: work stays nonterminal BLOCKED, its current attempt is bookkeeping-CANCELLED with no result or fan-in eligibility, and explicit trusted reconciliation is required before any terminal result closure.
...
- [ ] Confirm every ordinary terminal STATIC_TOOL work closure—including all caller-cancellation races—atomically publishes exactly one ToolRunResult; RULE_BASED caller cancellation always publishes its exact RuleExecutionRecord, and usable partial cancellation retains only complete raw evidence. Confirm `EXTERNAL_OUTCOME_UNKNOWN` instead remains BLOCKED with a bookkeeping-CANCELLED attempt, no result/fan-in, and no retry until explicit reconciliation.
