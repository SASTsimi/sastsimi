...
- The trusted ceiling is canonical JSON stored as an immutable content-addressed configuration artifact. Its exact artifact identity is `StoredDataRef(data_kind="artifact", stored_data_id=<sha256>, content_hash=<same sha256>, workspace_id, commit_id, record_id=null)`. That ref is fixed once in the `CONTEXT_RETRIEVAL` work, the plan artifact, the `READ_CODE` action inputs, and `ActionDecision.checked_config_refs`; replay never substitutes any different profile ref, even one with higher limits.
...
OpenGrep batching           Coordinator + fan-in       Context Retrieval
              |                       |                       |
...
class StaticActionReceipt:
    action_id: str
    attempt_id: str
    operation_kind: Literal["REPOSITORY_PREPARE", "STATIC_TOOL"]
...
**Interfaces:**
- Consumes: non-persisted `ContextRetrievalIntent`, current `CONTEXT_RETRIEVAL` work/attempt with exact proposal/bundle/workspace/ceiling-profile inputs, READY `CodeWorkspace`, exact current `StaticFactBundle`, `WorkspaceLocatorPort`, `ContextLineageReaderPort`, `ContextLimitPolicyPort`, existing content-addressed `ArtifactStore`, clock/ID ports, and a trusted request-count/fingerprint ledger.
- Produces: canonical pre-read `ContextReadPlan` and exact plan artifact ref, an exactly authorized/claimed `READ_CODE` action, exact `CodeContextRequest`, verified fragment artifacts, bounded `CodeContextResponse`, gaps/errors, and a COMMITTED context work output.
...
- [ ] **Step 3: Write RED two-phase authorization and complete-path tests.** Assert the call order `intent -> pure expand -> canonical plan/hash -> artifact -> exact READ_CODE action -> ALLOW/claim -> persisted request -> recompute -> mark_dispatched -> first open`. Cover a location-only request, entity-only request, lineage-only start, caller expansion, data-flow expansion, auth fact/callee, and route binding. Omit each expanded path or required ref from the action, inject one unrelated extra path, alter a relation/entity/lineage input or plan byte between authorization and execution, and substitute another plan artifact with a different hash; assert `CONTEXT_PATH_MISMATCH` or `CONTEXT_PLAN_CHANGED` before any open. Also reject another analysis/hypothesis/attempt, unclaimed/unused decision, and non-current bundle.
- [ ] **Step 4: Write RED immutable trusted-ceiling tests.** Use lower/equal limits as valid. Raise each of `max_depth`, `max_fragments`, `max_bytes`, `max_requests_per_hypothesis`, and `timeout_ms` above the exact profile one at a time; assert pre-dispatch rejection. Also assert WorkBudget timeout is the tighter ceiling, repeated/new request accounting cannot reset on retry/generation, and caller limits never replace the profile. Reject missing/duplicate profile refs in work/action, changed profile bytes, ref/hash mismatch, wrong JSON kind/version/field, a different profile ref with higher limits, profile absent from `ActionDecision.checked_config_refs`, and replay with a substitute profile; all cause zero reads.
...
- [ ] **Step 7: Implement intent planning, plan/profile artifact binding, authorization, recomputation, lineage validation, and filesystem reading separately.** The pure planner receives contracts/resolved lineage and returns canonical `ContextReadPlan`; the storage adapter only exact-resolves records/artifacts; the application handler stores the plan, builds/authorizes/claims the exact action, then asks `ContextBindingService` to create the request. The reader receives only the recomputed authorized plan plus guarded workspace. Extend `ContextBindingService`, READ_CODE validator checks, and response policy to require entity/explicit/expanded paths, exact plan/profile artifacts, checked config ref, and the same authorization closure. No file API is reachable from the intent/planning phase.
...
- Context follows intent -> canonical graph/lineage plan artifact -> exact READ_CODE authorization -> recomputation -> read. The exact immutable ceiling-profile artifact ref/hash is fixed in work, plan, action, and decision and cannot be replaced on replay.
