...
- Clone and checkout storage is allocated only through an attempt-owned `WorkspaceStoragePort` lease backed by enforceable byte/file-count quota controls. The exact immutable quota-policy artifact ref/hash is fixed in the `WORKSPACE_PREP` work and Git `RUN_TOOL` action inputs. If the backend cannot enforce every configured limit or reserve requirement, fail closed before starting Git; there is no unbounded directory fallback.
...
Before `begin` or any Git dispatch, exact-resolve and hash-verify one immutable canonical JSON workspace storage policy artifact with precisely `kind="workspace_storage_policy"`, `schema_version="1.0"`, `max_git_bytes`, `max_checkout_bytes`, `max_file_count`, and `min_free_bytes`, all positive bounded integers. Its identity is `StoredDataRef(data_kind="artifact", stored_data_id=<sha256>, content_hash=<same sha256>, workspace_id, commit_id, record_id=null)`. The trusted run builder—not the repository caller, Agent, repository, or adapter—fixes that exact ref once in the `WORKSPACE_PREP` work `input_refs`; the Git `RUN_TOOL` action repeats it in `input_refs`, and the dispatch/receipt input fingerprint covers it. Because it is artifact-backed with `record_id=null`, it is deliberately excluded from record-only `ActionDecision.checked_config_refs`; validation occurs against exact work/action inputs and canonical artifact bytes. Recovery rejects a missing, duplicate, substituted, stale-scope, wrong-hash, or caller-increased policy before allocation or Git.
...
- [ ] **Step 3: Write RED hard-quota allocation and cleanup tests.** In `test_workspace_storage.py`, use an enforceable fixture backend and assert exact attempt ownership, overflow-safe reserve arithmetic, private empty roots, separate `.git`/checkout/file-count counters, and write denial at each limit+1. In repository integration, test a giant pack/object set above `max_git_bytes`, one giant checked-out blob/tree above `max_checkout_bytes`, and a tree above `max_file_count`; each must terminate the process tree, prevent later Git commands, publish no READY/manifest/static work, and either delete the exact lease root or leave it sealed/quarantined and non-reusable. Also test insufficient `min_free_bytes`, an OS/backend reporting any limit unenforceable, caller-supplied destinations, and plain-directory fallback: all fail before clone with zero Git calls. Crash/replay tests substitute a larger policy or another lease and must fail before resumption; exact same ref/hash/lease may recover idempotently.
...
The publisher checks that every terminal `STATIC_TOOL` attempt has exactly one `ToolRunResult` and that each result/attempt/work combination matches exactly one publishing row above. It rejects a nonexistent `WorkStatus.SKIPPED`, `SUCCEEDED` with gaps/errors/incomplete rule telemetry, `PARTIAL` without usable committed evidence and gaps, `FAILED` without errors, cancelled work with anything other than the declared SKIPPED-without-usable-output or PARTIAL-with-usable-output result, timeout represented as cancellation, and any empty-output cancellation. It validates catalog equality, exact profile/configuration refs, tool/version/kind, attempt identity, bounded raw reference, timestamps, and output closure before the atomic publication.
...
- [ ] **Step 3: Write RED closed status-mapping tests.** Parameterize every mapping row above, including exit 0 complete, exit 0 with gaps, nonzero with/without complete earlier batch, timeout with/without earlier batch, malformed/oversized output, preflight not applicable, cancellation before claim, after claim before spawn, during the first process before usable output, after one complete OpenGrep batch, after a complete observation before publication, and the cancellation/publication race in both atomic orders. Every terminal row asserts exactly one `ToolRunResult` plus exact presence/absence of `StaticActionReceipt` and `RuleExecutionRecord`, exact `WorkAttempt`, `WorkExecutionState`, stop reason, gap/error, raw-ref, and output-ref values. For each RULE_BASED cancellation, assert one exact same-attempt rule record: pre-execution selected rules are all `NOT_EXECUTED/CANCELLED`; partial cancellation preserves executed hit counts and marks only unfinished selected rules cancelled. Assert SKIPPED cancellation has no raw ref, PARTIAL cancellation has a complete bounded raw ref, result/rule/work/attempt transition is atomic, no code references nonexistent `WorkStatus.SKIPPED`, timeout never maps to CANCELLED, and cancellation never maps to FALSE, HOLD, failure, or zero hits. Crash at each staging/commit boundary and concurrent cancel-vs-publish retries must converge to one result and one terminal transition with no empty-output terminal state.
...
- The clone root comes only from an exact attempt-owned `WorkspaceStoragePort` lease whose immutable policy ref/hash fixes maximum Git bytes, checkout bytes, checkout file count, and minimum free reserve. An unenforceable backend, insufficient reserve, giant pack/tree/blob, or file-count overflow fails closed before or during Git, terminates later work, and cleans or seals/quarantines only the exact lease; it can never publish READY.
...
- Every terminal `STATIC_TOOL` attempt has exactly one canonical `ToolRunResult`, including caller cancellation before trusted execution and cancellation-won races. Cancellation without usable output publishes SKIPPED plus a required gap; cancellation after complete usable output publishes PARTIAL plus its exact raw ref and gap, while attempt/work remain CANCELLED. Every RULE_BASED cancellation has the exact same-attempt `RuleExecutionRecord`, with selected unexecuted rules marked `NOT_EXECUTED/CANCELLED`; result/rule/attempt/work/output refs commit atomically.
...
- [ ] Confirm workspace allocation has no plain-directory fallback, all four quota/reserve limits are hard-enforced before clone, giant pack/tree/file-count cases stop and clean/quarantine safely, and recovery cannot substitute a higher policy or another lease.
...
- [ ] Confirm every terminal STATIC_TOOL attempt—including all caller-cancellation races—atomically publishes exactly one ToolRunResult; RULE_BASED cancellation always publishes its exact RuleExecutionRecord, and usable partial cancellation retains only complete raw evidence.
