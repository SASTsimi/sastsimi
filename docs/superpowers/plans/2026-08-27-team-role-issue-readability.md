...
- `6~7` 같은 bare pipeline 범위 대신 단계 이름과 `6–7단계` 표기를 사용한다.
...
- [ ] `git diff --check` 후 커밋하고 `docs/role-review-governance`에 push한다.
- [ ] Draft PR #11이 두 팀 매핑 문서와 Issue #1~#10을 계속 연결하는지 확인한다.
