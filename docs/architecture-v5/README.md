...
- 공식 정책은 가설마다 다시 수집하지 않는다. 비-LLM Collector가 원문과 출처를 고정하고 LLM Parser가 구조화하며, 같은 실행의 current `RunPolicyState`를 Sandbox와 Rule Scope가 exact reference로 공유한다. 정책은 코드 사실이나 Hypothesis 사전 scope 필터가 아니다.
- Sandbox는 `LOCAL_ONLY`와 외부 격리 경계를 강제하고 current 정책 reference를 기록하지만 정책 의미·scope·보고 가능성을 대신 판정하지 않는다. 만료 시각은 R8 기준으로 정하고 Sandbox·Rule Scope·Reporter action 직전에 다시 검사한다.
