...
Architecture v5는 정적 분석 결과를 최종 판정으로 취급하지 않고, LLM Agent가 검증 가능한 취약점 가설을 만들고 검토하는 데 사용하는 사실 계층으로 취급한다. 이 문서 묶음은 이 저장소에 가져온 **candidate baseline**이며 아직 승인된 정본, 구현 완료 또는 성능 개선을 주장하지 않는다.

번호 문서 `01`~`13`이 검토 시 의미의 기준이다. Wiki는 탐색을 위한 파생 설명 계층이며 새로운 계약이나 결정을 만들 수 없다. 검토 결정은 이 저장소의 Issue, ADR와 PR에서 먼저 확정하고, 승인된 snapshot만 별도 동기화 PR로 구현 저장소에 반영한다.
