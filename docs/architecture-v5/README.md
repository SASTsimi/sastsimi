...
17. Technical `ACCEPT`인 exact TRUE는 실행 초기에 준비한 current `RunPolicyState`를 재사용해 Rule Scope 검토를 진행한다. Rule Scope는 금지 테스트 위반 여부를 `testing_restriction_compliance`로 다른 판단과 분리하고, 비-LLM Primitive Admission Runtime은 이를 `PrimitiveAdmissionDecision=ALLOW | DENY`로 기계적으로 변환한다. `ALLOW`일 때만 제공 능력을 result로 가진 Primitive를 저장한다.
...
- 공식 정책은 가설마다 다시 수집하지 않는다. 비-LLM Collector가 원문과 출처를 고정하고 LLM Parser가 구조화한다. Rule Scope는 같은 실행의 current `RunPolicyState`를 사용하고 Sandbox는 실행 요청 당시 관측한 state를 exact 감사 reference로 남긴다. 정책은 코드 사실이나 Hypothesis 사전 scope 필터가 아니다.
- Sandbox는 `LOCAL_ONLY`와 외부 격리 경계를 강제하고 요청 당시 정책 reference를 감사용으로 기록하지만 정책 의미·scope·보고 가능성을 대신 판정하지 않는다. 정책이 `PREPARING`이어도 검증 가능한 clone·same-attempt mock/fixture와 격리 network만 쓰는 로컬 재현은 가능하다. 만료 시각은 R8 기준으로 정하고 Rule Scope·Reporter action 승인과 호출 직전에 다시 검사한다.
