...
12. Verification이 `NOT_REQUIRED | LIMITED_REPRO | FULL_REPRO`를 고르고 exact `ReproductionPlan`을 만든다. trusted runtime 허가 뒤 R7 Sandbox가 계획을 실행해 동적 결과와 PoC를 반환한다.
...
- Verification은 가설 내부 Context·Pro/Con, 동적 재현 필요성·모드·`ReproductionPlan`, 판정·Technical `REVISE`·Gate 제출과 Chaining handoff를 소유한다. 실행 허가는 Runtime Validator가 강제하고 R7 Sandbox는 승인된 계획만 실행해 결과를 생산한다.
