...
- `result=null`인 HOLD Primitive, result가 있는 Technical-accepted·admission-allowed TRUE Primitive, `PrimitiveAdmissionDecision`, match가 직접·부모 체인에서 사용한 `source_admission_refs`, upstream result→downstream input match와 재검증 여부
