...
- Technical 및 Rule Scope Impact Gate, current `RunPolicyState`, `FOUND | ABSENT_CONFIRMED | COLLECTION_FAILED` 정책 수집 결과, parser 결과, 공식 `ProgramPolicyRecord`과 freshness·재사용 관측, 두 Gate·보고서가 사용한 서로 일치하는 Verification·current CWELabel revision reference
