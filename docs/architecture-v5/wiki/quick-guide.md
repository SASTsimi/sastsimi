...
- HOLD의 필요 조건은 `inputs`, 결과는 `null`인 Primitive로 즉시 저장한다. TRUE는 validated PoC, Technical `ACCEPT`, 같은 exact chain의 Rule Scope `testing_restriction=PASS`가 모두 있어야 `result` Primitive가 된다. 한 Primitive의 result가 다른 Primitive의 특정 input을 충족할 때만 연결한다.
...
→ Technical Gate → Rule Scope Impact Gate → testing_restriction PASS → result Primitive → Chaining → new hypothesis loop
→ all Rule Scope Impact report conditions → Reporter eligibility
