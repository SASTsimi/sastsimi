...
- 검증(`Verification`)은 한 가설의 Context·찬반, 동적 재현 목적 요청·결과 소비·판정·Gate 보완·연계 handoff를 관리한다. R7 Reproduction Agent는 요청에 연결된 환경 요구사항·재현 계획·PoC candidate를 만들고 clean Sandbox에서 자율 실행한다. 모든 final TRUE에는 재현 성공과 validated PoC가 필요하며, 생성·환경·실행 실패는 `failure_category`와 함께 verdict 없이 `BLOCKED | FAILED`로 라우팅한다.
...
→ R7 requirements plan and PoC candidate → runtime request check → external boundary check → autonomous execution
