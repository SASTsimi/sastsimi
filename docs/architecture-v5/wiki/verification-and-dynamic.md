...
Sandbox profile 외부 격리 경계 차단·환경 구성·Agent·PoC 생성·실행 실패·timeout·취소는 verdict가 아닙니다. `DynamicReproductionResult.status=BLOCKED | FAILED | CANCELLED`와 `hypothesis_outcome=INCONCLUSIVE`를 기록하고 final `VerificationResult`와 Gate 요청을 만들지 않습니다. `BLOCKED`는 current work의 불변 입력을 바꾸지 않는 외부 조건을 기다리는 비종료 상태이고 `FAILED | CANCELLED`는 종료 상태입니다. 프로그램 정책 준비·freshness·testing restriction은 `LOCAL_ONLY` 실행의 `POLICY_BLOCKED` 사유가 아닙니다.
