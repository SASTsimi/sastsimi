...
최종 Verification은 `hypothesis_ref`로 검토한 정확한 가설 revision을 남깁니다. decision/result/status/PoC 조합의 정본은 [경량 데이터 계약의 compatibility matrix](../08-lightweight-data-contracts.md#dynamic-decisionresultpoc-compatibility)입니다. `NOT_REQUIRED`이면 동적 결과 ref는 없어야 하지만 PoC ref 허용 여부는 미결정입니다. 종료된 LIMITED/FULL 실행은 실패·부분·차단·취소도 결과 ref를 저장하지만 성공이나 반증으로 과장하지 않습니다. PoC의 모드별 허용·필수성과 부분/실패 PoC 보존은 아직 R7·공통 계약 결정 사항입니다.
