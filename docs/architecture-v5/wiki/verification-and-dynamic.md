...
R7은 설계·구현 담당 역할이며 실제 환경은 Sandbox Environment Builder와 Runner가 준비합니다. Verification Agent는 계정·권한·데이터·서비스·Mock·허용 fallback·Health Check를 immutable `EnvironmentRequirements`에 고정하고 `ReproductionPlan.environment_requirements_ref`로 current exact revision을 연결합니다. Builder는 같은 workspace·commit에서 그 조건을 바꾸지 않고 최소 build 설정과 image를 만듭니다. 기존 Dockerfile·Compose도 비신뢰 입력으로 읽기 전용 분석과 Sandbox Controller 정책 검사를 거칩니다. 없거나 안전하게 실행할 수 없으면 원본 밖의 Sandbox 작업영역에 일회성 실행 설정을 만들고 원본·생성 설정 hash와 차이를 남깁니다.

환경 설정과 image를 먼저 build해 immutable build artifact와 실제 image digest를 확정합니다. Verification은 이 artifact가 원 요구사항 또는 미리 허용한 fallback 안인지 확인하고, exact plan·requirements·build artifact·digest를 포함한 실제 재현용 `RUN_SANDBOX`를 요청합니다. Runner가 만든 actual `SandboxEnvironment`는 공통 `environment_ref`가 가리키며 requirement별 `MATCH | MISMATCH | NOT_CHECKED | ERROR`를 기록합니다. required 항목이 모두 `MATCH`일 때만 공격 단계를 실행합니다. 따라서 image를 만들기 전에 digest를 가정해 실행을 승인하지 않고, Verification의 확인도 Controller의 Sandbox 정책 검사를 우회하지 않습니다.

SQLite·PostgreSQL·MySQL은 Django settings와 dependency에서 engine을 탐지해 격리된 DB·migration·test record를 준비합니다. Builder는 계정·fixture·Mock 차이를 객관적으로 기록할 뿐 수락하지 않습니다. 허용 범위 밖 차이면 Verification이 실행을 중단하고 새 requirements와 plan을 만들지 판단합니다. runtime version은 lock/version file, Docker 설정, CI, project metadata, framework 설정, 승인된 기본 version 순으로 선택하고 근거와 fallback을 기록하며 미재현을 `FALSE`로 바꾸지 않습니다.

Health Check에는 command 또는 URL, 예상 status·응답, 필수 service·DB 조건, 최대 대기·주기·확인 횟수, 실패 log와 성공 판정식을 `EnvironmentRequirements`에 미리 고정합니다. 프로세스가 살아 있다는 사실만으로 성공하지 않으며 Health Check는 취약점 성공 신호가 아닙니다. 환경 준비 실패는 `FAILED + ENVIRONMENT_SETUP`, 정책 차단은 `BLOCKED + POLICY_BLOCKED`, time limit은 `FAILED + TIMEOUT`으로 구분합니다. exact requirements·profile/policy·image·command·mount/network·resource·cleanup과 plan closure가 같은 일시 실패만 기존 plan의 새 attempt에서 재시도합니다. 환경 전제조건이 바뀌면 새 requirements와 plan, 실행 단계나 정책 closure가 바뀌면 새 plan이 필요하며 기존 실패·차단 결과는 보존합니다.

종료 뒤 container·image·volume·network·fixture·credential을 정리합니다. cleanup 실패는 별도 상태로 남기고 환경을 격리해 재사용하지 않으며 재현 관측과 자동으로 합치지 않습니다. Actual `SandboxEnvironment`와 동적 artifact 상세는 #21, Sandbox enforcement는 #22, 숫자 예산은 R8, build action 공통 계약은 R4·R3와 확정합니다.
