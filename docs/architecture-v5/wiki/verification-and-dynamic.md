...
Verification Agent가 세 모드 중 하나를 결정합니다. 동적 재현이 필요하면 Verification이 필요한 환경을 exact `EnvironmentRequirements`로 기록하고 이를 가리키는 `ReproductionPlan`을 만듭니다. trusted runtime은 두 record의 schema·reference·권한·예산을 검사해 확정합니다. R7의 Sandbox Controller·Runner·비-LLM Result Assembler는 요구사항·허용 대체 버전·모드·계획을 다시 선택하거나 수정하지 않습니다.
...
실행 전에 Verification이 `EnvironmentRequirements`에 애플리케이션 역할·인증 방식·데이터·DB/service·fixture/mock·필수/대체 버전·Health Check와 근거를 고정합니다. `ReproductionPlan`에는 그 exact reference와 LIMITED/FULL mode, 가설, 순서가 있는 단계, 각 단계의 명령·공격 입력, cleanup 정책을 고정합니다. Runtime Validator의 `RUN_SANDBOX` 허가는 요청자·상태·예산과 current 계획·requirements reference를 확인한 `ALLOW + UNUSED`입니다. Controller 호출 직전 같은 조건인지 다시 확인해 달라졌으면 `EXPIRED`로 거절하고, 그대로이면 `USED`로 한 번만 claim합니다. 이 `action_decision_ref`는 Controller 호출 허가일 뿐 Sandbox 정책 통과 기록이 아닙니다.

USED decision과 exact plan·requirements reference만 Controller에 전달합니다. Controller는 image·명령·파일·네트워크·자원·cleanup 정책을 검사하고, plan hash·profile/policy revision·항목별 결과·안전한 reason code·Runner handoff를 담은 exact `sandbox_policy_decision`을 저장합니다. 그 `policy_decision_ref`가 `ALLOW`인 계획만 Runner가 사용합니다. Runner는 실제 환경·Health Check를 모든 requirement와 비교하고 필수 항목이 모두 `MATCH`일 때만 공격 단계를 실행하며 실제 단계·명령·공격 입력을 `SandboxStepLog`에 남깁니다. 비-LLM Result Assembler는 같은 분석·가설의 exact R6 plan·requirements closure와 같은 R7 실행 attempt의 정책·환경 비교·log·PoC·정리 reference를 동적 결과로 묶고 저장 때 조합을 다시 대조합니다. Verification은 `COMMITTED`된 결과만 읽어 최종 판정에 사용합니다.

Controller가 `DENY`하면 `policy_decision_ref`가 필수이고 Runner를 호출하지 않습니다. 결과는 `BLOCKED + POLICY_BLOCKED`, `runner_invoked=false`, `steps_ref=null`, 빈 공격 입력·관측, `hypothesis_outcome=INCONCLUSIVE`이며 반증으로 사용하지 않습니다. 실제 환경이 없으면 `environment_created=false + environment_ref=null`이고, 만들었다면 plan과 같은 requirements revision을 가리키는 exact 환경 reference가 필요합니다. 정리 대상이 없을 때만 `cleanup_required=false + cleanup_status=NOT_REQUIRED`이며, 자원이 있으면 cleanup 후 `SUCCEEDED | FAILED`를 기록합니다. 정책 검사 전에 만든 `poc_ref`가 있어도 실행 성공을 뜻하지 않습니다.

기존 plan을 재사용할 수 있는 경우는 `EnvironmentRequirements` revision, sandbox profile·policy revision, image digest, command/tool, file/mount/network, resource/time/process, cleanup policy와 나머지 plan closure의 exact reference가 모두 같은 일시적 실행 오류뿐입니다. 이때도 기존 결과를 보존하고 새 attempt·새 `RUN_SANDBOX` decision·새 Controller 정책 판정을 사용합니다. R6이 환경 조건이나 허용 대체값을 바꾸면 새 `EnvironmentRequirements`와 이를 가리키는 새 `ReproductionPlan`을 함께 만들고, requirements는 그대로지만 다른 plan closure가 바뀌면 새 plan을 만듭니다. 두 경우 모두 새 work generation·attempt·action·decision을 만들고 Runtime Validator와 Controller 정책 검사를 다시 거치며, R6의 수용은 Sandbox 정책을 우회하지 않습니다. 취소·supersede·revision 불일치로 늦게 도착한 결과는 stale로 거절합니다.

동적 결과는 정확한 PoC·Controller 정책 판정·실제 생성 환경·Runner 단계 로그를 reference로 전달합니다. 요구사항 reference는 결과에 중복 저장하지 않고 `reproduction_plan_ref → environment_requirements_ref`와 `environment_ref → requirements_ref`가 같은지 확인합니다. Runner가 호출되지 않았으면 단계 로그는 비어 있고, 호출됐다면 환경 차이로 첫 공격 단계 전에 멈춰도 로그가 필요합니다. 실제 환경이 없으면 환경 reference도 비어 있습니다. 정리할 자원이 전혀 없을 때만 `cleanup_status=NOT_REQUIRED`를 사용합니다. PoC reference가 있어도 정책에 막혀 실행되지 않았을 수 있으므로 상태와 로그를 함께 확인합니다. R4는 공통 필드·null·상태·reference 조합을, R6는 필요한 조건과 허용 차이를, R7은 실제 비교와 정책 판정·환경·PoC·실행 log·cleanup artifact의 상세 내용을 작성합니다.
