...
| Sandbox 외부 안전 경계 정책 | Sandbox Controller | Sandbox 생성·Agent 호출·내부 명령별 사전 허가·환경 요구사항·최종 판정 변경 |
| 내부 환경 구성·PoC·실행·관찰·retry | Reproduction Agent | host·Docker daemon·secret·허용되지 않은 egress 접근 또는 final verdict 판단 |
| 동적 실행 기록·결과 문서화 | Reproduction Session Manager | Agent 호출·실행·retry·cleanup 개입, 다른 attempt 혼합 또는 의미 재판단 |
...
동적 재현 전에는 R6 Verification이 `EnvironmentRequirements`와 이를 가리키는 목표 중심의 최소 `ReproductionPlan`을 고정합니다. exact step·command·payload·cleanup policy는 R6가 지정하지 않습니다. Runtime Validator는 요청자·상태·예산과 current plan·requirements·profile reference를 확인해 R7 호출만 허가합니다. Sandbox Controller는 host·Docker daemon·secret·허용되지 않은 egress·다른 workspace·R8 자원·lifecycle의 Sandbox 외부 경계 정책을 결정·강제하고 판정을 저장합니다. Controller는 Sandbox를 만들거나 Agent를 호출하지 않습니다. R7 Sandbox Setup Automation이 승인된 정책으로 clean Sandbox를 만들고, 그 안에서 `REPRODUCTION_AGENT`가 versioned `EnvironmentRecipe`를 사용해 package·계정·fixture/mock·PoC·명령·관찰·retry를 자율적으로 수행합니다. 기존 runtime/tool 계층이 실제 action event를 방출하면 Reproduction Session Manager가 Agent를 제어하지 않고 `AgentLog`로 수동 기록하며, 실행을 시작한 최종 `PoCBundle`과 같은 attempt의 사실을 최종 결과 문서로 확정합니다. R7은 동적 증거를 `SUPPORTED | DISPROVED | INCONCLUSIVE`로 전달하고, R6가 다른 근거와 합쳐 final `TRUE | FALSE | HOLD`를 정합니다.
