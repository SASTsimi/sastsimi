...
12. Verification이 LIMITED/FULL과 `ReproductionPlan` 결정 → Runtime Validator가 호출 전제 확인 → R7 Sandbox Controller가 세부 정책 판정 저장 → Runner가 exact plan 실행 → Result Assembler가 exact reference를 결과로 묶어 반환
