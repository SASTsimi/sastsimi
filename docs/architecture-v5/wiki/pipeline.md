...
정책은 가설마다 다시 가져오지 않습니다. workspace가 준비되면 정적 도구와 별도로 한 번 수집·파싱해 `RunPolicyState`에 고정합니다. Rule Scope는 호출 당시 current exact revision을 사용하고 Sandbox는 실행 요청 당시 관측한 state를 감사 reference로 남깁니다. 정책은 `StaticFactBundle`에 들어가지 않으며 Hypothesis Agent가 scope를 이유로 기술 가설을 사전 삭제하는 데 쓰지 않습니다. Sandbox는 `LOCAL_ONLY`와 외부 격리만 강제하고 정책 의미는 Rule Scope Gate가 판단합니다.
