...
AST와 SAST가 코드 사실을 모으면 LLM이 취약점 가능성을 제안합니다. Orchestration은 가설을 등록해 Verification에 배정하고, Verification은 코드·찬성·반대·Docker 재현·Gate 보완을 관리해 `TRUE / FALSE / HOLD`를 판정합니다. HOLD는 `result=null`인 Primitive로 즉시 저장하고, TRUE는 Technical `ACCEPT` 뒤 `result`가 있는 Primitive로 Chaining에 사용합니다. Rule Scope는 보고 가능성만 판단합니다. 새 공격 주장은 새 가설로 다시 검증하며 Reporter의 `ReportDraft`와 결과 저장 뒤 Agent 자동화가 끝납니다.
