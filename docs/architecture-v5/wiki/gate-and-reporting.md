...
정책 준비는 hypothesis별이 아니라 **program별 작업**입니다. repository와 버그바운티 program이 확정되면 run 초기화에서 정적 근거 준비·공통 환경 준비와 병렬로 `Policy Collector`(비-LLM, 공식 원문 수집)와 `Policy Parser`(LLM, 구조화)가 current `ProgramPolicyRecord`를 준비합니다. 같은 run·program에서 한 번만 수행하고, 다른 run에서도 current·fresh하고 parser version이 일치하면 재사용합니다. **Gate evaluation order는 바뀌지 않고**, Rule Scope Gate가 Technical `ACCEPT` 이후 이 준비된 결과를 소비합니다.

정책 경로는 `run init: Policy collection/parser -> PolicyParserResult -> PolicyCollectionResult 확인 -> ProgramPolicyRecord`로 준비하고, 이후 `Technical ACCEPT -> Rule Scope Impact Gate (준비된 current ProgramPolicyRecord 소비) -> 신뢰 runtime의 current Finding 정규화 -> Reporter`로 소비합니다. Parser와 Collector가 공통 artifact를 만들고 R5는 Rule·Scope·Impact 의미만 판단하며, Parser output 자체는 authoritative policy evidence가 아닙니다. Finding 정규화는 아래 "Finding 정규화" 섹션을 따릅니다.

Technical `ACCEPT`인 `TRUE`만 준비된 정책 수집 결과와 함께 검토합니다. 정책 수집은 `FOUND`, `ABSENT_CONFIRMED`, `COLLECTION_FAILED`를 구분합니다. `FOUND`이면 exact `ProgramPolicyRecord`도 함께 읽고, `ABSENT_CONFIRMED`(공식 source 확인 결과 해당 정책/항목 부재)이면 정책을 추정하지 않고 `UNCERTAIN + DENY`로 검토할 수 있습니다. `COLLECTION_FAILED`는 Rule Scope review를 만들지 않습니다. `COLLECTION_FAILED`는 fetch 실패(collection failure)와 원문 수집 후 parser 실패(parser failure)를 원인으로 구분하되 정책 부재를 의미하지 않습니다. 어느 준비 실패도 `VerificationResult`의 verdict를 바꾸지 않고 policy-dependent action만 fail-closed됩니다.
