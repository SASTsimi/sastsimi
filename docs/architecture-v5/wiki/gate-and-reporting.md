...
ReportDraft가 마지막 Agent 산출물이다. 이후 trusted runtime이 `AnalysisRunResult + AnalysisRunState`를 원자적으로 확정하면 Agent 자동화가 종료된다. Human Review·초안 수정·외부 제출/공개는 사람 주도 과정이며 active architecture는 이를 위한 schema, state, decision enum이나 자동 action을 정의하지 않는다.
