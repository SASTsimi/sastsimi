...
| `primitives` | result 없는 HOLD 조건, Technical-accepted + testing-restriction-PASS TRUE 능력과 exact Verification·Gate provenance |
...
- ACTIVE VerificationAssignment, result 없는 HOLD Primitive, result 있는 Technical-accepted + testing-restriction-PASS TRUE Primitive와 upstream result→downstream input match 수
