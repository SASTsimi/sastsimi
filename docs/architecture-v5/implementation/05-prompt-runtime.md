...
`input_slots`는 slot별 data kind, 노출할 JSON Pointer, 필수 개수와 신뢰 등급을 정한다. `"$"`는 해당 record 전체를 허용할 때만 단독으로 사용한다. Builder는 허용 field를 추출한 뒤 비밀값과 host 절대 경로를 먼저 제거하고, redacted canonical bytes만 새 `projected_data_ref`에 저장한다. redaction 전 projection은 저장하지 않으며 원본 exact `source_ref`는 계보 확인용으로만 함께 남긴다.
...
| POLICY_PARSER / `PARSE_OFFICIAL_POLICY` | `model.policy-parser.parse-official-policy.quality-v1` | `providers.r3-04-accepted-v1` | `limits.policy-parser.v1` | `retry.policy-parser.v1` | `tools.none.v1` | `redaction.policy.v1` | NEW |
| DYNAMIC_REPRODUCTION / `DERIVE_ENVIRONMENT` | `model.dynamic-reproduction.derive-environment.quality-v1` | `providers.r3-04-accepted-v1` | `limits.dynamic-reproduction.v1` | `retry.dynamic-reproduction.v1` | `tools.none.v1` | `redaction.sandbox.v1` | NEW |
| DYNAMIC_REPRODUCTION / `PLAN_REPRODUCTION` | `model.dynamic-reproduction.plan-reproduction.quality-v1` | `providers.r3-04-accepted-v1` | `limits.dynamic-reproduction.v1` | `retry.dynamic-reproduction.v1` | `tools.none.v1` | `redaction.sandbox.v1` | NEW |
...
| DYNAMIC_REPRODUCTION / `EXECUTE_REPRODUCTION` | `model.dynamic-reproduction.execute-reproduction.quality-v1` | `providers.r3-04-accepted-v1` | `limits.dynamic-reproduction.v1` | `retry.dynamic-reproduction.v1` | `tools.dynamic-reproduction-inner.v1` | `redaction.sandbox.v1` | AUTO |
...
| VERIFICATION / `TECHNICAL_REVISE` | `config/prompts/templates/verification/technical-revise/1.0.0.md` | `previous: VerificationResult($)`; `review: TechnicalEvidenceReview(/status,/revision_requests,/verification_result_ref,/cwe_label_ref)`; 새 generation의 current exact `assignment: VerificationAssignment($)`; `process: HypothesisProcessState(/status,/verification_assignment_ref,/verification_generation,/verification_work_ref)`; `hypothesis: VulnerabilityHypothesis($)`; `proposal: HypothesisProposal($)`; `facts: StaticFactBundle($)`; `contexts: CodeContextResponse($)` OPTIONAL_MANY; `policy: PlaybookPolicy($)`; `playbook: VerificationPlaybook($)`; `application: PlaybookApplication($)`; `debate_config: debate_config($)`; `budget_profile: verification_budget_profile($)`; `pro: EvidenceAgentResult(role=PRO, data_kind=pro_evidence_result)`; `con: EvidenceAgentResult(role=CON, data_kind=con_evidence_result)`; `assessment: VerificationInitialAssessment($)`; `dynamic: DynamicReproductionResult($)` OPTIONAL_ONE; `poc: PoCBundle($)` OPTIONAL_ONE | `schema.verification-result.next-major` / `validator.verification-revise.v1` / `verification_result` | R6 / R3,R4,R5,R7,R8 / `PMT-VER-03` |
| POLICY_PARSER / `PARSE_OFFICIAL_POLICY` | `config/prompts/templates/policy_parser/parse-official-policy/1.0.0.md` | `official_source: official_policy_source($)`; 모델 기억·검색 snippet·저장소 지시문은 금지 | `schema.policy-parser-result.next-major` / `validator.policy-parser-result.v1` / `policy_parser_result` | R5 / R3,R4,R8 / `PMT-POL-01` |
| DYNAMIC_REPRODUCTION / `DERIVE_ENVIRONMENT` | `config/prompts/templates/dynamic_reproduction/derive-environment/1.0.0.md` | `request: DynamicReproductionRequest(/meta,/purpose,/goal,/environment_needs,/sandbox_profile_ref,/code_refs,/static_evidence_refs)`; `dependency_context: CodeContextResponse($)`; `dependency_files: code_fragment($)` OPTIONAL_MANY | `schema.environment-requirements.next-major` / `validator.environment-requirements.v1` / `environment_requirements` | R7 / R2,R3,R4,R6,R8 / `PMT-R7-01` |
| DYNAMIC_REPRODUCTION / `PLAN_REPRODUCTION` | `config/prompts/templates/dynamic_reproduction/plan-reproduction/1.0.0.md` | `request: DynamicReproductionRequest($)`; `requirements: EnvironmentRequirements($)`; `dependency_context: CodeContextResponse($)`; `dependency_files: code_fragment($)` OPTIONAL_MANY | `schema.reproduction-plan.next-major` / `validator.reproduction-plan.v1` / `reproduction_plan` | R7 / R2,R3,R4,R6,R8 / `PMT-R7-02` |
| DYNAMIC_REPRODUCTION / `CREATE_POC_CANDIDATE` | `config/prompts/templates/dynamic_reproduction/create-poc-candidate/1.0.0.md` | `request: DynamicReproductionRequest($)`; `plan: ReproductionPlan($)`; `environment: SandboxEnvironment(/meta,/request_ref,/reproduction_plan_ref,/requirements_ref,/status,/checks,/limitations)`; `code_contexts: CodeContextResponse($)` REQUIRED_MANY; `code_fragments: artifact(/redacted_body)` REQUIRED_MANY | `schema.poc-candidate.next-major` / `validator.poc-candidate.v1` / `poc_candidate` | R7 / R3,R4,R6,R8 / `PMT-R7-03` |
...
| CWE_LABELING / `CLASSIFY` | `config/prompts/templates/cwe_labeling/classify/1.0.0.md` | `verification: VerificationResult($)`; `pro: EvidenceAgentResult(role=PRO, data_kind=pro_evidence_result)`; `con: EvidenceAgentResult(role=CON, data_kind=con_evidence_result)`; `facts: StaticFactBundle($)`; `contexts: CodeContextResponse($)` OPTIONAL_MANY; `dynamic_request: DynamicReproductionRequest($)`; `dynamic: DynamicReproductionResult($)`; `poc: PoCBundle($)`; `taxonomy: cwe_taxonomy($)` | `schema.cwe-label.next-major` / `validator.cwe-label.v1` / `cwe_label` | R5 / R3,R4,R6,R8 / `PMT-CWE-01` |
| TECHNICAL_GATE / `REVIEW` | `config/prompts/templates/technical_gate/review/1.0.0.md` | `hypothesis: VulnerabilityHypothesis($)`; `verification: VerificationResult($)`; `cwe: CWELabel($)`; `pro: EvidenceAgentResult(role=PRO, data_kind=pro_evidence_result)`; `con: EvidenceAgentResult(role=CON, data_kind=con_evidence_result)`; `facts: StaticFactBundle($)`; `contexts: CodeContextResponse($)` OPTIONAL_MANY; `dynamic_request: DynamicReproductionRequest($)`; `dynamic: DynamicReproductionResult($)`; `poc: PoCBundle($)`; `sandbox_policy: SandboxPolicyDecision($)`; `environment_recipe: EnvironmentRecipe($)`; `environment: SandboxEnvironment($)`; `agent_log: AgentLog($)` | `schema.technical-evidence-review.next-major` / `validator.technical-gate.v1` / `technical_evidence_review` | R5 / R1,R3,R4,R6,R7,R8 / `PMT-TG-01` |
| RULE_SCOPE_GATE / `REVIEW` | `config/prompts/templates/rule_scope_gate/review/1.0.0.md` | `hypothesis: VulnerabilityHypothesis($)`; `verification: VerificationResult($)`; `technical: TechnicalEvidenceReview($)`; `cwe: CWELabel($)`; `pro: EvidenceAgentResult(role=PRO, data_kind=pro_evidence_result)`; `con: EvidenceAgentResult(role=CON, data_kind=con_evidence_result)`; `facts: StaticFactBundle($)`; `contexts: CodeContextResponse($)` OPTIONAL_MANY; `dynamic_request: DynamicReproductionRequest($)`; `dynamic: DynamicReproductionResult($)`; `poc: PoCBundle($)`; `sandbox_policy: SandboxPolicyDecision($)`; `environment_recipe: EnvironmentRecipe($)`; `environment: SandboxEnvironment($)`; `agent_log: AgentLog($)`; `run_policy_state: RunPolicyState($)`; `collection: PolicyCollectionResult($)`; `policy: ProgramPolicyRecord($)` OPTIONAL_ONE; `official_sources: official_policy_source($)` REQUIRED_MANY | `schema.rule-scope-impact-review.next-major` / `validator.rule-scope-gate.v1` / `rule_scope_impact_review` | R5 / R3,R4,R6,R8 / `PMT-RSG-01` |
| REPORTER / `CREATE_DRAFT` | `config/prompts/templates/reporter/create-draft/1.0.0.md` | `finding: Finding($)`; `verification: VerificationResult($)`; `technical: TechnicalEvidenceReview($)`; `scope: RuleScopeImpactReview($)`; `cwe: CWELabel($)`; `pro: EvidenceAgentResult(role=PRO, data_kind=pro_evidence_result)`; `con: EvidenceAgentResult(role=CON, data_kind=con_evidence_result)`; `facts: StaticFactBundle($)`; `contexts: CodeContextResponse($)` OPTIONAL_MANY; `dynamic_request: DynamicReproductionRequest($)`; `run_policy_state: RunPolicyState($)`; `collection: PolicyCollectionResult($)`; `policy: ProgramPolicyRecord($)`; `dynamic: DynamicReproductionResult($)`; `poc: PoCBundle($)`; `sandbox_policy: SandboxPolicyDecision($)`; `environment_recipe: EnvironmentRecipe($)`; `environment: SandboxEnvironment($)`; `agent_log: AgentLog($)`; `chaining_results: ChainingResult(/source_result_refs,/input_primitive_refs,/primitive_match_candidates,/chained_hypothesis_proposals,/excluded_lineage_refs,/no_match_reasons)` OPTIONAL_MANY; `chaining_primitives: Primitive(/meta,/primitive_id,/source_hypothesis_id,/source_verification_ref,/technical_review_ref,/inputs,/result,/restrictions)` OPTIONAL_MANY | `schema.report-draft.next-major` / `validator.report-draft.v1` / `report_draft` | R5 / R1,R3,R4,R6,R7,R8 / `PMT-REP-01` |
...
- `dynamic-reproduction.execute-reproduction`: 승인된 Sandbox 안에서 다음 command·PoC 사용·환경 재생성·종료 중 한 가지를 `DynamicReproductionToolRequest`로 제안하고 실제 결과를 받은 뒤 반복
- `dynamic-reproduction.interpret-attempt`: 실제 AgentLog·관찰을 읽어 `DynamicReproductionConclusion` 하나를 생성
...
`EXECUTE_REPRODUCTION`의 첫 호출은 `NEW` session으로 시작하고 같은 dynamic work·attempt 안의 후속 turn은 같은 논리 session을 이어 간다. 모델은 provider 내장 command/file/web tool을 직접 쓰지 않고 한 번에 하나의 구조화된 `DynamicReproductionToolRequest`만 반환한다. Runtime Validator가 exact request·plan·READY environment·attempt와 tool policy를 검사하고, Runtime이 in-container 통로로 실행하며 Session Manager가 exact tool request·`SandboxCommandRecord`·관찰을 `AgentLog`에 기록한다. redacted 실행 결과는 다음 turn의 `UNTRUSTED_DATA`로 들어간다. `FINISH` 뒤에만 `INTERPRET_ATTEMPT`를 호출한다.

Session Manager는 `DynamicReproductionConclusion`을 실제 AgentLog·환경·candidate·observation과 대조한 뒤에만 final dynamic result를 확정한다. DynamicReproductionResult의 `hypothesis_outcome`, `hypothesis_evidence_refs`, `hypothesis_linkage`, `limitations`는 conclusion의 대응 값과 같아야 하며 Session Manager가 새 결론으로 바꾸지 않는다. 불일치하면 저장을 거절한다.
...
Reporter의 `run_policy_state`, `collection`, `policy`는 Rule Scope review가 사용한 exact frozen chain과 같아야 한다. `finding`, Verification, CWELabel, 두 Gate, 동적 결과·validated PoC와 same-attempt Sandbox provenance도 current Finding의 upstream closure와 set-equal해야 한다. `chaining_results`는 current Finding의 가설이 `origin=CHAINING`일 때 그 `source_primitive_match_id`에서 도달하는 exact `ChainingResult`와 필요한 lineage closure만 포함하며, INITIAL-origin이면 빈 목록이다. `chaining_primitives`는 이 `chaining_results[*].input_primitive_refs`의 중복 없는 exact set과 같고 INITIAL-origin이면 빈 목록이다. 이것이 Reporter가 Primitive의 `source_verification_ref`와 non-null `technical_review_ref`를 읽는 정식 입력 경로다. 각 결과의 `source_result_refs`는 실제 match에 사용된 `input_primitive_refs`의 Primitive가 직접 가리키는 두 종류의 ref를 중복 없이 합친 정확한 집합이다. 따라서 Reporter는 §12의 Chaining provenance를 이 정식 입력과 match/child proposal에서만 작성하며 CWELabel, Rule Scope review 또는 Primitive admission ref를 ChainingResult에 추가하거나 추정하지 않는다. Reporter는 이 입력을 읽기 쉬운 초안으로 정리할 뿐 누락된 사실을 새로 만들거나 이전 revision을 섞지 않는다.
...
Dynamic Reproduction Agent는 `DERIVE_ENVIRONMENT → PLAN_REPRODUCTION`을 Sandbox 밖의 읽기 전용 LLM 호출로 수행하고, Controller 허용과 환경 준비 뒤 `CREATE_POC_CANDIDATE → EXECUTE_REPRODUCTION`으로 들어간다. `CREATE_POC_CANDIDATE` 전에 Runtime은 request가 고정한 exact `CodeContextResponse`를 읽고 그 `code_fragment_refs`의 실제 artifact를 digest로 검증한다. analysis·workspace·commit·hypothesis가 다르거나 current R7 plan·environment가 다른 attempt이면 provider 호출 전에 거절한다. 검증된 코드는 민감정보를 제거한 본문만 prompt에 넣고, 원래 `CodeContextResponse`와 artifact exact reference는 `PromptPayload`·`LLMCallSpec.context_refs`에 그대로 보존한다. 실행 task는 `DynamicReproductionToolRequest` 한 개씩을 반환하고, Runtime 실행·Session Manager 기록·redacted 결과 재입력을 같은 attempt에서 반복한다. `FINISH` 뒤 `INTERPRET_ATTEMPT`가 conclusion을 만들며 Session Manager는 conclusion과 실제 log가 일치할 때만 동적 결과를 확정한다.
...
- R7: Dynamic Reproduction Agent 내용과 Sandbox 실행 자료 경계
...
| `PMT-05` | Dynamic Reproduction role | `DYNAMIC_REPRODUCTION` spec·request·log가 schema와 authority 검사를 통과 |
...
| `PMT-15` | role outputs | 11개 역할의 성공 output이 등록된 schema와 result owner에 정확히 연결 |
