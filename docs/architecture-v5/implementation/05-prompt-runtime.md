...
`restrictions[].fact_refs`에서 Agent는 `fact_id`만 반환하고 trusted 출력 검증 runtime이 `CodeFactRef.bundle_ref`를 채운다. 08번이 INITIAL proposal의 각 restriction에 final `StaticFactBundle`을 가리키는 `fact_refs`를 요구하지만, `bundle_ref`는 `StoredDataRef`이고 그 `stored_data_id`·`data_kind`·`content_hash`는 `facts` slot이 노출하는 어떤 field에도 없다. `RecordMeta`에도 그 세 값이 없으므로 `/meta`를 추가해도 해결되지 않는다. Agent가 만들면 저장 식별자 발명 금지에 걸리므로, Builder가 `PromptContextBinding.source_ref`로 이미 고정해 둔 exact bundle reference를 출력 검증 단계에서 붙인다. `observed_facts`의 `CodeFact.producer.raw_result_ref`는 입력 안에 완성된 `StoredDataRef`로 들어 있으므로 Agent가 그대로 옮긴다.
...
`VerificationResult.restrictions`의 새 제한도 같다. Agent는 `fact_refs`에 `fact_id`만 반환하고 trusted 출력 검증 runtime이 `bundle_ref`를 채운다. 4.1의 이유가 그대로 적용된다.
