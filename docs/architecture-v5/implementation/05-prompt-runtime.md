...
Reporter의 `run_policy_state`, `collection`, `policy`는 Rule Scope review가 사용한 exact frozen chain과 같아야 한다. `finding`, Verification, CWELabel, 두 Gate, 동적 결과·validated PoC와 same-attempt Sandbox provenance도 current Finding의 upstream closure와 set-equal해야 한다. Reporter는 이 입력을 읽기 쉬운 초안으로 정리할 뿐 누락된 사실을 새로 만들거나 이전 revision을 섞지 않는다.
