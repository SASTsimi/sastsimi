...
| 3. AST·SAST와 실행 단위 정책 준비(정적 분석과 병렬) | 외부 도구 / R2 Static Tool Coordinator, R5-02 Policy Parser, 비-LLM Policy Collector | `StaticToolCoordinator.run_all`의 도구별 `STATIC_TOOL`; `PolicyPreparationService.prepare`의 분석 단위 `POLICY_FETCH` | READY `CodeWorkspace`; static 설정·rule catalog·tool profile; program ID·공식 source 설정·Parser call spec·R8 freshness 기준 | static: raw artifact, `ToolRunResult`, `RuleExecutionRecord`; policy: `RunPolicyState`, `PolicyParserResult`, `PolicyCollectionResult`, FOUND이면 `ProgramPolicyRecord` | static은 기존 `RUN_TOOL`; policy는 `FETCH_POLICY → CALL_LLM(requested_by=POLICY_PARSER) → SAVE_RESULT`; 역할·official source·exact ref·dedupe·freshness 검사 | static은 `facts`, 정책은 `policies`, 상태·오류는 `runs`; 둘은 별도 COMMITTED transition과 current pointer 사용 | AST·CodeQL·OpenGrep과 policy preparation을 독립 fan-out. static만 4에서 join하고 policy는 Sandbox에 요청 당시 감사 ref, 17에 호출 당시 current state로 join | static 실패와 policy 실패는 서로의 결과를 바꾸지 않음. policy는 `(analysis_id, program_id, generation)` active work 하나, 가설별 재수집 금지; 만료 시 새 generation | `tests/integration/static_analysis/`, `tests/integration/test_run_policy_preparation.py`, `tests/security_negative/test_policy_static_fact_separation.py`; 01·02·05·08·10·ADR-006/013 / R2·R3·R4·R5·R8 |
...
| 17. current 정책 재사용·Rule Scope·Primitive admission | LLM Gate + 비-LLM 결정 / R5·R4 / Rule Scope Agent·Admission Runtime | `PolicyAndScopeWorkflow.evaluate` / `RULE_SCOPE_GATE`, `PRIMITIVE_UPDATE`; 정책 준비는 Step 3의 `POLICY_FETCH` | same TRUE/CWE/Technical ACCEPT chain; current `RunPolicyState`; exact `PolicyCollectionResult`, FOUND이면 `ProgramPolicyRecord` | Gate: `RuleScopeImpactReview`. R4 runtime: `PrimitiveAdmissionDecision`; ALLOW이면 result Primitive와 index | Verification owner의 `CALL_RULE_SCOPE_GATE`; admission runtime의 `REGISTER_WORK/SAVE_RESULT`; freshness·exact chain·testing restriction decision table 검사 | `policies/gates/primitives/invocations/runs`; current policy·Rule Scope·admission/index refs. `COLLECTION_FAILED`에는 Gate review 없음 | 15 ACCEPT → current policy와 17에서 join; admission ALLOW → 18, DENY → 19/22 | stale면 새 policy generation 준비 전 Gate 금지; fetch/parser 오류는 `COLLECTION_FAILED`, 공식 부재는 `ABSENT_CONFIRMED → UNCERTAIN+DENY`; testing restriction FAIL만 Primitive admission DENY | `tests/integration/test_policy_scope_admission.py`, `tests/security_negative/test_testing_restriction.py`; 05·06·07·08·10·ADR-011/013 / R5·R4·R1·R8 |
...
| 21. ReportDraft | LLM Agent / R5 / Reporter Agent Runtime | `ReportService.create_draft` / `REPORT_DRAFT` | Step 19에서 검사한 current Finding, same Verification/CWE/Technical/Rule Scope, exact current `RunPolicyState`/policy chain, dynamic/PoC, restrictions·limitations·unresolved conditions | internal `ReportDraft`; REPORTER만 생산, 공개·제출 권한 없음 | Verification owner의 `CREATE_REPORT_DRAFT`; Runtime Validator가 Gate order/readiness/revision, Rule Scope와 같은 current policy state·freshness, redaction을 action 승인·호출·저장 시 검사 | `reports/invocations/runs`; draft·report work SUCCEEDED·`ReportProcessState(DRAFTED)`를 atomic commit. raw secret/hidden reasoning 미저장 | 19 pass → 21 → 22; 가설별 독립 초안 가능 | missing Finding, stale refs, policy state 변경·만료, redaction 실패는 `REPORT_NOT_READY/REPORT_ERROR`; upstream revision 변경 시 과거 draft는 history만 유지 | `tests/contract/test_report_draft.py`, `tests/security_negative/test_report_redaction.py`; 05·07·08·10·12 / R5·R4·R6·R7 |
...
| `policies` | RunPolicyState, PolicyParserResult, PolicyCollectionResult, ProgramPolicyRecord | POLICY_PARSER는 parser 결과, POLICY_COLLECTOR는 state·collection·policy record | 실행 단위 current pointer, 수집 status와 exact official source/parser/freshness provenance |
