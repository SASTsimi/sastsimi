...
| 19. 보고 가능 조건 확인 | 비-LLM readiness 검사 / R4·R5 / Reporting Condition Runtime | `ReportingPolicy.evaluate` / 별도 work 없음, 통과 시 `REPORT_DRAFT` 준비 | current Finding, final TRUE, current dynamic+PoC, Technical ACCEPT, Rule Scope exact review와 policy/CWE | report-ready 또는 구체적인 차단 사유; 새 vulnerability/policy verdict를 생산하지 않음 | `CREATE_REPORT_DRAFT` action의 `REVISION/REPORT_READY/REDACTION` preflight; `review_status=PASS`, `rule_compliance=PASS`, `scope_compliance=PASS`, `testing_restriction_compliance=PASS`, `security_impact=SUFFICIENT`, `report_permission=ALLOW`를 각각 exact 검사 | `actions/runs`; 허용/차단 decision과 `REPORT_NOT_READY`, current Finding/report pointer는 변경하지 않음 | 17의 Rule Scope 결과 → pass면 21, fail/uncertain/deny면 22; admission/Chaining과 독립 축 | 정책 stale·Finding 없음·revision 불일치는 Reporter 차단만 하고 기술 TRUE·Primitive 자격을 바꾸지 않음 | `tests/contract/test_report_readiness.py`, `tests/security_negative/test_reporter_preconditions.py`; 05·07·08·10 / R4·R5·R6·R8 |
| 20. material child 전역 등록 | 비-LLM registry + 필요 시 LLM duplicate review / R1·R3 / Proposal Validator·Hypothesis Registry·Assignment Runtime | `HypothesisRegistry.register_child` / `HYPOTHESIS_PROPOSAL`, 이후 새 `VERIFICATION` | COMMITTED Verification-origin 또는 Chaining-origin proposal, exact parent IDs. Chaining-origin은 `source_primitive_match_id`와 admissible lineage | 새 `VulnerabilityHypothesis`, process state와 ACTIVE `VerificationAssignment`; 기존 부모 결과와 독립 | Step 7과 같은 validation·duplicate `CALL_LLM/SAVE_RESULT`. Chaining-origin 등록 전에는 Proposal Validator·Hypothesis Registry·Assignment Runtime이 exact `PrimitiveMatchCandidate`, upstream `result.entity_refs`, upstream의 모든 `inputs[].entity_refs`, 매칭된 downstream input의 `entity_refs`, downstream의 나머지 `inputs[].entity_refs`, `proposal.meta.workspace_id`·`proposal.meta.commit_id`와 유효한 entity 또는 location 최소 1개를 검사한다. R6 Verification Agent는 이 등록 전 검사를 수행하지 않음 | `hypotheses/runs/invocations`; parent-child refs와 새 current process pointer | 13/18 → 20 → 등록 성공한 child만 8–21 전체 반복; 가설 간 병렬 가능 | 계보 단절, workspace·commit 불일치, 매칭된 downstream 위치 누락, upstream 전제조건 누락 또는 유효한 시작점 부재 시 새 가설을 등록하지 않고 Verification도 배정하지 않는다. duplicate/invalid도 새 child 없음. 부모 verdict 불변 | `tests/integration/test_child_hypothesis_registration.py`, `tests/integration/test_chained_child_context.py`, `tests/e2e/test_chaining_child_full_revalidation.py`; 03·04·06·07·08·10 / R1·R2·R3·R4·R6 |
...
| `reports` | ReportDraft | REPORTER | current Finding과 같은 Verification/CWE/two-Gate/policy chain |
...
### B2. current Finding 생산 단계·저장 권한 부재 — 구현 시작 Blocker

`CREATE_REPORT_DRAFT`와 `ReportDraft.finding_ref`는 current Finding을 필수로 요구하지만 정본 22단계와 `WorkExecutionState.work_type`, result-owner registry에는 Finding을 누가 언제 어떤 action으로 확정하는지 없다. R3-01은 Step 19·21의 필수 입력으로만 표시했으며 값을 추정하지 않는다.

- 담당: R5가 Finding 의미·생성 조건을 확정하고 R4가 최소 schema·result owner·state/action 연결을 반영
- 완료 기준: 같은 Verification/CWE/두 Gate exact chain에 묶인 Finding 생산 단계와 Reporter 전제조건을 구현자가 하나로 해석할 수 있음
