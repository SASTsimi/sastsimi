...
6. 한 Verification generation에는 `DYNAMIC_REPRO` work가 하나뿐이다. 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정은 현재 attempt를 유지한다. session 재시작은 같은 `work_id`의 새 `attempt_id`·`trigger=RETRY`, 외부 조건 해소 뒤 재개는 새 `attempt_id`·`trigger=RESUME`를 사용한다.
...
9. TRUE 경로는 CWE_LABELING → Technical Evidence Gate → run에 고정한 `RunPolicyState` 사용 → Rule Scope Impact Gate 순서를 지킨다. Gate 직전에 정책을 다시 수집하지 않는다.
...
15. workspace가 READY가 되면 R3 runtime은 정적 분석과 정책 준비를 독립 branch로 등록한다. 가설 전 Docker baseline pull·build·cache warm·container 준비 branch는 만들지 않는다. Docker capability는 사용자가 `doctor`를 실행할 때 또는 실제 `DYNAMIC_REPRO` 진입 시 확인한다.
...
| 3. run-init 두 갈래 병렬 준비 | 비-LLM runtime·외부 도구 / R3 등록, R2 정적 실행, Policy Collector·Policy Parser 정책 준비 | R3 `RunInitializationService.start_branches`; `StaticToolCoordinator.run_all`의 도구별 `STATIC_TOOL`; `PolicyPreparationService.prepare`의 분석 단위 `POLICY_FETCH` | READY `CodeWorkspace`; 시작 상태의 단일 `program_id`; static 설정·rule catalog·tool profile; 공식 source 설정·Parser call spec·R8 freshness 기준 | static: raw artifact, `ToolRunResult`, `RuleExecutionRecord`; policy: run마다 새 `RunPolicyState`, `PolicyCollectionResult`, FOUND이면 `ProgramPolicyRecord`; 새 수집이면 `PolicyParserResult`, 성공 준비면 `PolicyCacheRecord` | R3 runtime은 두 branch의 등록과 상태 관측만 담당한다. static은 `RUN_TOOL`; policy는 run 시작 시 exact cache 검사, hit이면 run-local record materialize, miss이면 `FETCH_POLICY → CALL_LLM(requested_by=POLICY_PARSER) → SAVE_RESULT` | static은 `facts`, 정책·run-neutral cache는 `policies`, 상태·오류는 `runs`; 각 branch는 별도 COMMITTED transition과 current pointer 사용 | 정적 분석과 정책 준비를 서로 기다리지 않고 시작한다. static은 4에서 join하고 정책은 17에서 준비 완료 때 고정한 exact state로 join한다. run-init에는 Docker branch가 없다. Docker capability·image·container는 실제 동적 재현이 필요한 가설의 Step 12에서 current request·generation·attempt에 맞춰 확인·생성한다. | 두 branch 실패는 서로의 결과를 바꾸지 않는다. policy는 `(analysis_id, program_id, work_type=POLICY_FETCH)` active work 하나, retry는 같은 work의 attempt이며 가설별·run 중 재수집하지 않는다. Docker 미설치·실행 불가는 Step 12의 실행 상태·오류일 뿐 policy나 verdict `FALSE | HOLD`로 바꾸지 않는다. | `tests/integration/static_analysis/`, `tests/integration/test_run_policy_preparation.py`, `tests/integration/test_run_init_fanout.py`, `tests/security_negative/test_policy_static_fact_separation.py`, `tests/security_negative/test_no_run_init_docker_mutation.py`; 01·02·03·04·05·08·10·ADR-006/013 / R2·R3·R4·R5·R7·R8 |
...
| 9. 위치 기반 Context 조회 | 비-LLM 코드 조회 / R2·R6 / Context Retrieval Service | `ContextService.retrieve` / 요청마다 `CONTEXT_RETRIEVAL` | ACTIVE assignment, current hypothesis와 exact proposal, `CodeContextRequest`, exact workspace/commit, allowed relation과 limits. `origin=CHAINING`이고 직접 entity·location이 비어 있으면 등록 단계에서 고정한 exact `source_primitive_match_id` 계보 reference | `CodeContextResponse`, code fragment artifact refs, `DataGap/AnalysisError`; Context Retrieval Service만 고정된 계보 reference 확인과 조회 결과 생산 | `READ_CODE`와 `SAVE_RESULT`; 일반 요청은 file path·workspace·commit·depth/byte/request budget·symlink boundary 검사. 체이닝 자식은 코드 조회 전에 exact `PrimitiveMatchCandidate`, upstream `result.entity_refs`, upstream의 모든 `inputs[].entity_refs`, 매칭된 downstream input의 `entity_refs`, downstream의 나머지 `inputs[].entity_refs`, 고정한 계보 reference의 존재·`content_hash`와 `proposal.meta.workspace_id`·`proposal.meta.commit_id` 일치를 검사 | `contexts`와 code fragment artifact; request/response exact pair와 returned location 기록 | 8 → 9 → 10/11; Verification은 `CodeContextRequest`만 만들고 Context Retrieval Service가 고정된 계보 reference를 확인하고 Context를 반환함 | 일부 조회 실패는 retry·대체 조회 가능. 고정한 계보 reference를 찾을 수 없거나 `content_hash`가 다르거나, workspace·commit이 다르거나, 매칭된 downstream 위치·upstream 전제조건·유효한 시작점이 누락되면 Context와 Verification work를 final verdict 없이 중단한다. 조회 오류를 HOLD/FALSE로 바꾸지 않음 | `tests/integration/test_context_retrieval.py`, `tests/integration/test_chained_child_context.py`, `tests/security_negative/test_code_path_escape.py`; 02·04·06·07·08·10 / R1·R2·R3·R6·R4 |
...
| 12. 동적 재현·PoC | LLM + 비-LLM Sandbox / R6 요청·R7 실행 / Dynamic Reproduction Agent·Reproduction Setup Automation·Sandbox Controller·Reproduction Session Manager | `DynamicReproductionService.execute` / `DYNAMIC_REPRO` | R6의 exact `DynamicReproductionRequest`, parent Verification/generation, R7이 만든 current `EnvironmentRequirements`·current exact `ReproductionPlan`, 요청 당시 `RunPolicyState` 감사 ref, `sandbox_profile_ref`, exact `DynamicReproductionLifecycleProfile` | Dynamic Reproduction Agent: requirements/plan/PoC candidate·해석. Reproduction Setup Automation: recipe/environment/cleanup. Sandbox Controller: policy decision. Reproduction Session Manager: append-only `AgentLog`, validated `PoCBundle`, `DynamicReproductionResult` | R6 `REQUEST_DYNAMIC_REPRO` → Dynamic Reproduction Agent의 경계 전 `CALL_LLM(requested_by=DYNAMIC_REPRODUCTION)`과 requirements/plan `SAVE_RESULT` → Reproduction Setup Automation의 `RUN_SANDBOX` 요청 → Runtime Validator가 exact request·requirements·plan·두 profile의 revision과 잔여 시간·attempt 한도를 검사하고 Sandbox Controller가 current CodeWorkspace clone·same-attempt mock/fixture·격리 network 기반 `LOCAL_ONLY`, live asset·외부 계정·허용되지 않은 egress와 외부 격리 경계를 강제 → 승인 뒤 Reproduction Setup Automation이 recipe·image·container·READY environment를 준비 → Dynamic Reproduction Agent 시작(`agent_invoked=true`)과 candidate·command·관찰·retry event → 역할별 `SAVE_RESULT` → Reproduction Session Manager가 append-only log와 final dynamic result를 확정. 요청 당시 policy state는 exact 감사 provenance로 기록하되 freshness로 local-only 권한을 바꾸지 않으며, policy state 변경만으로 기존 action을 만료시키지 않는다. Controller는 프로그램 정책 의미를 해석하지 않고 R7 profile의 외부 접근·격리와 CPU·RAM·disk·PID·요청 가능 최대 시간을 강제 | `dynamic`, artifact store, append-only AgentLog와 `SandboxCommandRecord`, `actions/runs`; `DynamicReproductionState`, work output, COMMITTED transition이 같은 final result ref를 가리킴 | 11 → 12 → 13; generation당 work 하나, Sandbox 내부 event는 직렬/반복 가능 | 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정은 같은 attempt의 event다. session 재시작이 필요한 일시 오류만 한도 안에서 같은 work의 새 attempt로 자동 retry한다. 현재 work의 `input_refs/input_hash`를 바꾸지 않는 재인증·승인·외부 환경 정비·resource 확보 대기만 `BLOCKED` 후 `trigger=RESUME`인 새 attempt를 만든다. exact request를 교체하거나 승인된 새 profile reference를 적용해야 하면 새 Verification generation에서 Pro·Con·초기 판단부터 다시 수행하고, 여전히 필요할 때만 새 request와 동적 work를 만든다. 불가·한도 소진은 `FAILED+INCONCLUSIVE`; Sandbox 경계 차단·실패는 final verdict 없음. validated PoC는 `SUCCEEDED+SUPPORTED`, `agent_invoked=true`, same-attempt AgentLog의 exact candidate revision·`content_digest` 실행 증명이 모두 있을 때만 생성하며 그 밖에는 `poc_ref=null` | `tests/integration/sandbox/`, `tests/integration/sandbox/test_agent_boundary.py`, `tests/integration/sandbox/test_attempt_lifecycle.py`, `tests/integration/sandbox/test_container_lifecycle.py`, `tests/contract/test_dynamic_provenance.py`, `tests/security_negative/test_sandbox_boundary.py`; 04·07·08·10·ADR-002/003/004/007/013 / R6·R7·R4·R8 |
...
| 15. Technical Evidence Gate | LLM 검토 / R5 / Technical Evidence Gate Agent | `GateService.review_technical` / `TECHNICAL_GATE` | exact final TRUE, current same-Verification `CWELabel`, current generation dynamic request/result/validated PoC, code·Pro/Con·restriction refs | `TechnicalEvidenceReview(ACCEPT/REVISE/REJECT)`; TECHNICAL_GATE만 생산 | Verification owner의 `CALL_TECHNICAL_GATE`; Runtime Validator가 exact pair·COMMITTED·provider/session·Gate order 검사, Gate는 근거 의미 검토 | `gates/invocations/runs`; Gate work output은 exact review 하나, current pointer는 입력 hash에 종속 | 14 TRUE → 15; ACCEPT → 17, REVISE → 16, REJECT → 22 내부 종결 | provider/invalid output은 같은 domain input의 제한 retry. `REVISE`는 retry가 아님. validated PoC 없는 TRUE는 호출 전 차단 | `tests/integration/test_technical_gate.py`, `tests/security_negative/test_gate_order.py`; 03·04·05·07·08·09·10 / R5·R6·R4·R7 |
| 16. Technical REVISE loop | hypothesis-local 제어 + LLM / R6·R5 / same Verification owner·CWE Labeling·Technical Gate | `RevisionWorkflow.start_new_generation` / 새 `VERIFICATION`, `PRO_EVIDENCE`, `CON_EVIDENCE`, 이후 새 `CWE_LABEL`, `TECHNICAL_GATE` | COMMITTED `TechnicalEvidenceReview(REVISE)`, 직전 final Verification/CWE, ACTIVE assignment; runtime이 current policy·playbook으로 새 `PlaybookApplication`과 전역 question ID를 생성 | 증가한 `verification_generation`, 새 고정 입력 묶음과 `debate_input_hash`, 새 Pro·Con child work와 각 `EvidenceAgentResult`, 두 결과를 합성한 새 VerificationResult; final TRUE이면 새 dynamic work/validated PoC, 새 CWELabel revision, 새 Technical review | `REGISTER_WORK`와 state CAS; 필요 시 `READ_CODE`, Pro·Con 각각 필수 `CALL_LLM`, 조건부 `REQUEST_DYNAMIC_REPRO`, final 합성과 이후 Gate action. old work/action/decision·application·질문·Pro/Con ref 재사용 차단 | 모든 과거 revision은 history; 새 generation의 application·Pro·Con·Verification·CWE·Gate pointer만 새 exact chain으로 원자 전환 | 15 REVISE → 같은 owner. 9 Context는 필요 범위로 보완, 10 Pro·Con은 항상 새 child work로 병렬 재실행, 12 dynamic은 final TRUE 후보 또는 판정용 실행 근거가 필요할 때 실행, 이후 13/14/15 수행; Orchestration Runtime은 목적지를 선택하지 않음 | 새 Pro·Con은 같은 새 `debate_input_hash`를 사용하고 둘 다 COMMITTED·SUCCEEDED여야 join 가능. 한쪽이라도 미완료면 새 final 결과 저장 금지. 첫 attempt는 `INITIAL`; 입력 그대로인 provider 오류만 same-work `RETRY`. 보완 한도 소진은 Verification/Gate 실패이며 이전 generation 결과 자동 승격 금지 | `tests/e2e/test_technical_revise_generation.py`, `tests/security_negative/test_revise_old_debate_reuse.py`, `tests/security_negative/test_revise_debate_join.py`; 03·04·05·07·08·10 / R6·R5·R4·R7 |
| 17. 고정 정책 사용·Rule Scope·Primitive admission | LLM Gate + 비-LLM 결정 / R5·R4 / Rule Scope Impact Gate Agent·Primitive Admission Runtime | `PolicyAndScopeWorkflow.evaluate` / `RULE_SCOPE_GATE`, `PRIMITIVE_UPDATE`; 정책 준비는 Step 3의 `POLICY_FETCH`, 이 단계에서 새 collection/parsing 없음 | same TRUE/CWE/Technical ACCEPT chain; run 준비 완료 때 고정한 `RunPolicyState`; exact `PolicyCollectionResult`, FOUND이면 `ProgramPolicyRecord`와 공식 정책 원문(`source_ref + source_locator`) | Gate: `RuleScopeImpactReview`. R4 runtime: `PrimitiveAdmissionDecision`; ALLOW이면 result Primitive와 index | Verification owner의 `CALL_RULE_SCOPE_GATE`; admission runtime의 `REGISTER_WORK/SAVE_RESULT`; frozen reference·exact chain·source 원문 대조·testing restriction decision table 검사 | `policies/gates/primitives/invocations/runs`; frozen policy·Rule Scope·admission/index refs. `PolicyCollectionResult.status=COLLECTION_FAILED`에는 Gate review 없음 | 15 ACCEPT → frozen policy와 17에서 join; admission ALLOW → 18, DENY → 19/22 | `RunPolicyState.status=UNVERIFIED`는 `UNCERTAIN+DENY`; fetch/parser 오류는 state `BLOCKED|FAILED`와 collection `COLLECTION_FAILED`, 공식 부재는 `ABSENT_CONFIRMED → UNCERTAIN+DENY`; 원문 확인 불가·Parser 정규화 모순 시 fail-closed; testing restriction FAIL만 Primitive admission DENY | `tests/integration/test_policy_scope_admission.py`, `tests/security_negative/test_testing_restriction.py`; 05·06·07·08·10·ADR-011/013/014 / R5·R4·R1·R8 |
| 18. Primitive Chaining | LLM match + trusted registry / R1 / Chaining Runtime·Chaining Agent | `ChainingService.match` / `CHAINING` | work 시작 시 고정한 current `PrimitiveIndexState`, 전체 후보인 `considered_primitive_refs`, 그 index에서 읽은 후보 Primitive | `ChainingResult`, `PrimitiveMatchCandidate`, `LineageExclusion`, `HypothesisProposal(origin=CHAINING)`; CHAINING만 결과·proposal을 생산 | `REGISTER_WORK`, `CALL_LLM(requested_by=CHAINING)`, `SAVE_RESULT`; 시작 시 고정한 전체 후보 입력을 검사하고, 저장 시 고정하지 않은 reference가 섞이지 않았는지와 result→input·lineage·match 조합 중복·restriction·source refs를 검사 | `chaining/hypotheses/runs/invocations`; DB를 삭제하지 않고 exact result/history 저장, child proposal은 아직 미등록 | HOLD 또는 allowed TRUE Primitive → 18; candidate별 fan-out 가능, 결과 child는 20; no-match는 22 | work가 고정하지 않은 Primitive·index reference가 결과에 섞이면 거절하고 부모 verdict는 유지한다. index revision이 올라간 것만으로는 결과를 거절하지 않는다. budget/LLM 실패는 `CHAINING_ERROR/BUDGET_EXCEEDED`; 미확인 match를 저장하지 않음 | `tests/contract/test_chaining_result.py`, `tests/integration/test_true_hold_true_true.py`; 03·06·07·08·10·ADR-001/005/011 / R1·R4·R5·R6·R8 |
| 19. current Finding 정규화와 보고 가능 조건 확인 | 비-LLM 신뢰 runtime / R4 storage·R5 semantic / Finding Normalization + Reporting Condition Runtime | `FindingNormalization.assemble` + `ReportingPolicy.evaluate` / `FINDING_NORMALIZE` 전용 비-LLM work(B2), 통과 시 `REPORT_DRAFT` 준비 | 같은 exact chain의 final TRUE, current dynamic+PoC, current CWELabel, Technical ACCEPT, current Rule Scope review(값 무관), restriction/limitation/unresolved condition | current `Finding`(정규화 record, 새 verdict 아님) 또는 `COLLECTION_FAILED`이면 Finding 없음; report-ready 또는 구체적 차단 사유 | Finding 정규화는 `RULE_SCOPE_GATE` `SUCCEEDED` 이후 신뢰 runtime이 exact chain·revision 일치 검사 후 atomic commit; `CREATE_REPORT_DRAFT` action의 `REVISION/REPORT_READY/REDACTION` preflight; `review_status=PASS`, `rule_compliance=PASS`, `scope_compliance=PASS`, `testing_restriction_compliance=PASS`, `security_impact=SUFFICIENT`, `report_permission=ALLOW`와 current non-stale Finding 존재를 각각 exact 검사 | `reports/actions/runs`; current Finding pointer(가설별 하나)와 허용/차단 decision. `report_permission=DENY`여도 Finding은 보존 | 17의 Rule Scope 결과 → 정규화 → pass면 21, fail/uncertain/deny면 22; admission/Chaining과 독립 축 | 미검증 policy state·Finding 없음·stale Finding·revision 불일치는 Reporter 차단만 하고 기술 TRUE·Primitive 자격을 바꾸지 않음 | `tests/contract/test_finding_normalization.py`, `tests/contract/test_report_readiness.py`, `tests/security_negative/test_reporter_preconditions.py`; 05·07·08·10 / R4·R5·R6·R8 |
| 20. material child 전역 등록 | 비-LLM registry + 필요 시 LLM duplicate review / R1·R3 / Proposal Validator·Hypothesis Registry·Assignment Runtime | `HypothesisRegistry.register_child` / `HYPOTHESIS_PROPOSAL`, 이후 새 `VERIFICATION` | COMMITTED Verification-origin 또는 Chaining-origin proposal, exact parent IDs; Chaining은 `source_primitive_match_id`와 그 계보 reference·workspace·commit·entity/location 유효성 | 새 `VulnerabilityHypothesis`, process state와 ACTIVE VerificationAssignment; 기존 부모 결과와 독립 | Step 7과 같은 validation/duplicate `CALL_LLM/SAVE_RESULT`, 이후 `REGISTER_WORK(VERIFICATION)`; Chaining-origin 등록 전에는 Proposal Validator·Hypothesis Registry·Assignment Runtime이 exact `source_primitive_match_id` 계보, upstream Primitive의 `result.entity_refs`와 모든 `inputs[].entity_refs`, `draft_id == matched_input_id`인 downstream input의 `entity_refs`, downstream의 나머지 `inputs[].entity_refs`, `proposal.meta.workspace_id`·`proposal.meta.commit_id` 일치와 유효한 entity 또는 location 최소 하나 이상 존재 여부를 검사한다. 하나라도 충족하지 못하면 가설을 등록하지 않고 Verification도 배정하지 않는다. | `hypotheses/runs/invocations`; parent-child refs와 새 current process pointer | 13/18 → 20 → 새 child는 8–21 전체 반복; 가설 간 병렬 가능 | duplicate/invalid는 새 child 없음. 계보 reference를 찾을 수 없거나 workspace·commit·entity/location이 맞지 않으면 등록·후속 사용 차단, 부모 verdict 불변 | `tests/integration/test_child_hypothesis_registration.py`, `tests/e2e/test_chaining_child_full_revalidation.py`; 03·04·06·07·08·10 / R1·R3·R4·R6 |
...
### run-init 두 branch의 합류 경계

- 정적 branch는 Step 4의 `StaticFactBundle`에서만 합류하며 Hypothesis 시작에 필요한 branch다.
- 정책 branch는 같은 run의 새 `RunPolicyState`를 만들고 Step 17에서 exact reference로 합류한다. 정책 준비가 끝날 때까지 정적 branch를 기다리게 하지 않는다.
- run-init에는 Docker image pull·build·cache warm·container 생성·PoC 실행 branch가 없다. `EnvironmentRequirements`, `ReproductionPlan`, `EnvironmentRecipe`, `SandboxEnvironment`, `AgentLog`와 PoC는 Step 12의 current 가설·generation·attempt에서만 만든다.
- 사용자는 분석 전 `sastsimi doctor`로 Docker 설치·daemon 접근 가능성을 확인할 수 있다. 이 확인은 분석 work나 취약점 근거가 아니며 Docker 상태를 바꾸지 않는다.
- 실제 동적 재현이 필요하면 Reproduction Setup Automation이 Step 12에서 승인된 current request와 profile을 사용해 clean 환경을 준비한다. Docker를 사용할 수 없으면 실제 원인의 `BLOCKED | FAILED` 실행 결과를 남기고 `FALSE | HOLD`로 자동 변환하지 않는다.
- 가설 간 writable container를 공유하지 않는다. read-only content-addressed layer를 재사용하더라도 exact digest와 사용 사실은 Step 12의 attempt-local recipe·환경 record에 고정한다.
...
1. 경계 전 Dynamic Reproduction Agent의 requirements·plan 작성 호출과 경계 승인 뒤 Sandbox 안의 Dynamic Reproduction Agent 시작을 구분한다. 전자는 `agent_invoked`를 `true`로 만들지 않으며, Sandbox profile 외부 격리 경계 차단이면 `agent_invoked=false`와 exact `SandboxPolicyDecision`·AgentLog를 남긴다.
2. 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정은 같은 attempt의 event로 남긴다. session 재시작이 필요한 일시 오류만 새 retry attempt를 만들고, 현재 work 입력을 바꾸지 않는 외부 조건을 기다리는 경우에만 `BLOCKED` 뒤 `trigger=RESUME`인 새 attempt를 만든다. R6에 반환하는 current 결과에는 이전 attempt의 중간 실패 결과나 늦게 도착한 event를 섞지 않는다.
...
4. validated `poc_ref`는 `SUCCEEDED + SUPPORTED + agent_invoked=true`이고 같은 attempt AgentLog가 exact `poc_candidate_ref` revision과 `content_digest`의 실제 실행을 입증할 때만 허용한다. candidate 생성·실행 실패, Sandbox profile 외부 격리 경계·환경 실패, timeout, `DISPROVED | INCONCLUSIVE`에서는 `poc_ref=null`이며 candidate reference만으로 성공을 추론하지 않는다.
5. build·container·network·volume·임시 파일 중 하나라도 생성되면 성공·실패·Sandbox profile 외부 격리 경계 차단과 관계없이 cleanup 결과와 exact reference를 기록한다. `cleanup_status=NOT_REQUIRED`는 정리할 자원이 하나도 생성되지 않았을 때만 허용한다.
...
| Sandbox Controller | R7 `sandbox_profile_ref` 외부 접근·격리와 CPU·RAM·disk·PID·요청 가능 최대 시간 강제 | 내부 command allowlist, R7 profile 값, R8 잔여 예산·새 attempt, 최종 verdict 결정 | 12 |
...
| `dynamic` | request, requirements, plan, `DynamicReproductionLifecycleProfile`, recipe, environment, policy decision, `SandboxCommandRecord`, AgentLog, PoC, cleanup, dynamic result | R6/R7의 등록된 producer | same work·attempt provenance와 current final result pointer |
...
[08. 경량 데이터 계약](../08-lightweight-data-contracts.md)의 `LLMCallSpec.agent_role`과 `LLMInvocationRequest.agent_role`, `CALL_LLM` 허용 역할에 `DYNAMIC_REPRODUCTION`이 함께 존재한다. Step 12는 이 exact identity를 사용한다.

- 반영 위치: R4 공통 enum·authority 검사, R3-05 #91 Prompt Registry와 call 구조
- 유지 조건: call spec, invocation request/log, Prompt Registry key와 action role 검사가 같은 `DYNAMIC_REPRODUCTION`을 사용함
...
### B3. 일부 core output의 `SAVE_RESULT` registry 연결 누락 — RESOLVED

[08. 경량 데이터 계약](../08-lightweight-data-contracts.md)의 result-owner registry와 output binding에 다음을 확정했다.

- `code_workspace -> CodeWorkspace -> REPOSITORY_LOADER`: `WORKSPACE_PREP` 결과와 `AnalysisRunState.workspace_ref`를 같은 transition에서 확정한다.
- `tool_run_result -> ToolRunResult -> STATIC_ANALYSIS`: 도구별 `STATIC_TOOL` attempt와 exact result를 묶는다.
- `hypothesis_proposal -> HypothesisProposal -> ORCHESTRATION`: trusted proposal 출력 검증 runtime이 source candidate에 전역 ID를 한 번 부여한다. INITIAL·VERIFICATION·CHAINING source output이 COMMITTED된 뒤 비-LLM 전역 등록 runtime은 그 ID와 내용을 바꾸지 않고 별도 proposal record를 저장한다.
- `analysis_run_result -> AnalysisRunResult -> ORCHESTRATION`: 신뢰 run finalization runtime이 종료 `AnalysisRunState.analysis_result_ref`와 같은 transition에서 확정한다.

네 record 모두 append-only revision, exact source/work/attempt reference와 `TransitionCommit`을 사용한다. `CodeWorkspace`는 `AnalysisRunState.workspace_ref`, proposal은 `ProposalProcessState.proposal_ref`, `ToolRunResult`는 COMMITTED `STATIC_TOOL.output_refs`, 최종 분석 결과는 `AnalysisRunState.analysis_result_ref`가 current 선택점이다. 별도 의미 없는 current pointer를 추가하지 않는다.

### B4. Orchestration Runtime 비-LLM 경계 — RESOLVED
...
가설 전 Docker 상태를 바꾸는 준비 branch를 첫 구현에서 제거했다. run-init은 정적 분석과 정책 준비만 병렬 등록하며 image pull·build·cache warm·container 생성은 하지 않는다.

- `sastsimi doctor`의 Docker 확인은 설치·daemon 접근 가능성을 읽는 사용자 진단이며 분석 work·domain output·성공 근거가 아니다.
- 실제 Docker capability 확인, recipe·image·container·AgentLog·PoC 생산은 exact `DynamicReproductionRequest`가 있는 Step 12의 가설별 `DYNAMIC_REPRO`에서만 수행한다.
- run-init에서 `RUN_SANDBOX`, `EnvironmentRequirements`, `ReproductionPlan`, `EnvironmentRecipe`, `SandboxEnvironment`와 `DynamicReproductionLifecycleProfile`을 사용하지 않는다.
- 따라서 run-init 전용 action·result·예산 profile을 새로 만들지 않는다. Docker 미설치나 실패는 실제 동적 재현이 요청된 work의 실행 상태·오류로 남고 취약점 `FALSE | HOLD`로 바꾸지 않는다.

B1~B5의 구현 시작 Blocker는 모두 해결됐다. 이 결정은 구현 가능한 계약을 확정한 것이며 실제 production pipeline 구현 완료를 뜻하지 않는다.
...
| R2 정적분석·Context `@zv9uvr` | 2–4, 9 | run-init static branch가 정책 branch와 독립인지, tool별 output, 규칙 실행 0건/미실행 구분, StaticFactBundle, 안전한 code retrieval |
| R3 통합 `@YHS-Sec`, `@taehyeon-git` | 1–22 | entry point, work 등록, 병렬/join, 모듈 의존 방향과 최종 종료 |
| R4 공통 계약 `@taehyeon-git` | 전 단계 | ID·revision·state·action·저장·authority와 B3·B5 결정, run-init Docker 상태 변경이 차단되는지 |
...
| R7 동적 재현 `@Potatonion` | 3, 12–13, 15, 21 | run-init에 Docker 변경이 없고 실제 준비는 가설별 clean Sandbox에서만 이뤄지는지, R6/R7 경계, Sandbox 외부 경계, AgentLog·same-attempt PoC provenance |
| R8 평가·예산 `@gitterable` | Step 3을 포함한 전 단계의 budget/metrics, 22 | static·policy branch와 실제 DYNAMIC_REPRO의 시간·비용·work·retry·disk·network 사용량을 서로 구분해 관측하는지. Docker 강제 상한은 R7 소유 |
...
## Chaining 자식 가설 복구 테스트

### `tests/integration/test_child_hypothesis_registration.py`

등록 전 검사에서 다음 시나리오를 확인한다.

- `source_primitive_match_id` 계보가 끊기면 자식 가설을 등록하지 않는다.
- 계보의 proposal·match candidate·부모 Primitive·entity 또는 location 중 하나라도 `proposal.meta.workspace_id`·`proposal.meta.commit_id`와 다르면 계보 전체를 거절한다.
- `draft_id == matched_input_id`인 downstream input 또는 해당 `entity_refs`가 없고 다른 유효한 시작점도 없으면 등록하지 않는다.
- upstream Primitive의 `inputs[].entity_refs`가 복구 대상에서 누락되면 등록하지 않는다.
- 모든 계보와 양쪽 부모의 시작점이 유효할 때만 자식 가설을 등록하고 Verification을 배정한다.

### `tests/integration/test_chained_child_context.py`

등록 후 Context 조회 전 확인에서 다음 시나리오를 확인한다.

- 등록 당시 고정한 match candidate 또는 부모 Primitive reference를 찾을 수 없거나 `content_hash`가 다르면 Context 조회를 중단한다.
- 등록 이후 어떤 `PrimitiveIndexState` revision이 생겨도 기존 자식 가설과 Verification을 무효화하지 않는다. 부모 admission과 최신 index 소속은 다시 검사하지 않는다.
- 등록 후 계보의 workspace·commit이 달라지면 일부 reference만 사용하지 않고 전체 요청을 거절한다.
- 매칭된 downstream input의 `entity_refs`가 사라지거나 무효가 되면 Context 조회를 중단한다.
- upstream의 전제조건 또는 downstream의 나머지 전제조건이 누락되면 Context 조회를 중단한다.
- 실패한 Context work로 final `VerificationResult` 또는 `TRUE | FALSE | HOLD`를 만들지 않는다.
- 정상 계보에서는 `CodeContextResponse`를 반환하고 R6가 부모 verdict를 상속하지 않은 채 자식 가설을 다시 검증한다.
...
- [x] run-init은 static·policy 두 branch만 사용하고, Docker 준비는 가설별 동적 재현에서만 수행하도록 표시했다.
...
- [x] 체이닝 자식의 직접 코드 시작점이 비어 있으면 등록 전에 Proposal Validator·Hypothesis Registry·Assignment Runtime이 exact `source_primitive_match_id` 계보에서 upstream result, 매칭된 downstream input, upstream의 모든 inputs와 downstream의 나머지 inputs에 연결된 entity·location을 복구·검사하고, 등록 후에는 Context Retrieval Service가 코드 조회 전에 고정한 계보 reference의 존재·`content_hash`와 workspace·commit을 검사하도록 표시했다.
- [x] R7의 경계 전 LLM 계획, 외부 경계 검사, 경계 뒤 Dynamic Reproduction Agent 실행, same-session retry·새 attempt·`BLOCKED/RESUME`, container와 cleanup, validated PoC 조건을 구분했다.
- [x] Chaining work 시작 시 current index에서 전체 후보를 고정하고, 저장 시에는 고정한 reference와 결과가 맞는지만 검사하도록 구분했다.
