...
| `reports` | Finding, ReportDraft | 신뢰 runtime(Finding 정규화, storage owner는 R4 B2), REPORTER(ReportDraft) | Finding: `RULE_SCOPE_GATE` 종료 뒤 exact chain을 조립한 가설별 current revision. ReportDraft: current non-stale Finding과 같은 Verification/CWE/two-Gate/policy chain |
...
### B2. current Finding 생산 단계·저장 권한 부재 — R5 semantic 확정, R4 storage binding 대기
...
- **R4 대기(storage binding)**: 전용 `work_type` 사용 여부 또는 `RULE_SCOPE_GATE` 종료 transaction / Step 19 readiness runtime의 추가 output으로 묶을지, `Finding`/`FindingCandidate` schema 본문, result-owner identity, current pointer·revision·CAS·stale enforcement, `AnalysisRunResult.finding_refs` 연결. 새 autonomous Agent나 새 `action_type`은 추가하지 않는다.
- 완료 기준: R4가 위 storage binding을 확정하면 같은 Verification/CWE/두 Gate exact chain에 묶인 Finding 생산 단계와 Reporter 전제조건을 구현자가 하나로 해석할 수 있음
...
- [x] B2의 Finding 의미·생성 closure·claim 제한·stale 조건을 R5가 확정했다([R5-03 후속이슈], `05`/`08`). storage `work_type`·result owner·current pointer·CAS binding은 R4 결정 대기.
