...
| 17. 고정 정책 사용·Rule Scope·Primitive admission | LLM Gate + 비-LLM 결정 / R5·R4 / Rule Scope Agent·Admission Runtime | `PolicyAndScopeWorkflow.evaluate` / `RULE_SCOPE_GATE`, `PRIMITIVE_UPDATE`; 정책 준비는 Step 3의 `POLICY_FETCH` | same TRUE/CWE/Technical ACCEPT chain; run 준비 완료 때 고정한 `RunPolicyState`; exact `PolicyCollectionResult`, FOUND이면 `ProgramPolicyRecord` | Gate: `RuleScopeImpactReview`. R4 runtime: `PrimitiveAdmissionDecision`; ALLOW이면 result Primitive와 index | Verification owner의 `CALL_RULE_SCOPE_GATE`; admission runtime의 `REGISTER_WORK/SAVE_RESULT`; frozen reference·exact chain·testing restriction decision table 검사 | `policies/gates/primitives/invocations/runs`; frozen policy·Rule Scope·admission/index refs. `PolicyCollectionResult.status=COLLECTION_FAILED`에는 Gate review 없음 | 15 ACCEPT → frozen policy와 17에서 join; admission ALLOW → 18, DENY → 19/22 | `RunPolicyState.status=UNVERIFIED`는 `UNCERTAIN+DENY`; fetch/parser 오류는 state `BLOCKED|FAILED`와 collection `COLLECTION_FAILED`, 공식 부재는 `ABSENT_CONFIRMED → UNCERTAIN+DENY`; testing restriction FAIL만 Primitive admission DENY | `tests/integration/test_policy_scope_admission.py`, `tests/security_negative/test_testing_restriction.py`; 05·06·07·08·10·ADR-011/013 / R5·R4·R1·R8 |
