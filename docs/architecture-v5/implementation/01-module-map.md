...
9. TRUE 경로는 `CWE_LABELING → Technical Evidence Gate → Rule Scope Impact Gate` 순서를 지킨다. policy collection/parsing은 이 순서 밖에서 run 초기화에 program별로 수행하고, Rule Scope Gate는 그 준비된 current `ProgramPolicyRecord`를 소비한다.
...
| 3. AST·SAST 병렬 실행 + run-init 병렬 준비 | 외부 도구 + Policy 준비 / R2·R5·R4·R7 / Static Tool Coordinator·Policy Collector·Policy Parser·환경 준비 | `StaticToolCoordinator.run_all` / 도구마다 `STATIC_TOOL`. repository·버그바운티 program 확정 시 병렬로 program별 `POLICY_FETCH`(Policy Collector → Policy Parser)와 공통 Docker/environment 준비 | READY `CodeWorkspace`, exact 분석 설정·rule catalog·tool profile; program identifier와 승인된 official source config | raw AST/SAST artifact, `ToolRunResult`; 규칙 기반 도구는 exact `RuleExecutionRecord`; STATIC_ANALYSIS만 생산. `POLICY_FETCH`는 program별 `PolicyParserResult`(Parser)·`PolicyCollectionResult`와 `FOUND`이면 current `ProgramPolicyRecord`(Collector/trusted runtime) 생산 | tool마다 `REGISTER_WORK → START_ATTEMPT → RUN_TOOL → SAVE_RESULT`; tool/path/budget와 attempt·catalog·규칙 set 검사. `POLICY_FETCH`는 `FETCH_POLICY(requested_by=POLICY_COLLECTOR)`와 `SAVE_RESULT`, source authenticity/freshness 검사 | raw 결과는 content-addressed artifact, 정규 record는 `facts`, 상태·오류는 `runs`; current tool output은 COMMITTED transition으로만 노출. 정책 record는 `policies`, program별 current pointer, cross-run current·fresh 결과 재사용 | AST·CodeQL·OpenGrep 등을 fan-out 병렬 실행 → 4에서 join. `POLICY_FETCH`·환경 준비는 정적 도구와 서로 차단하지 않고 독립 완료하며, 준비된 정책은 17단계 Rule Scope Gate가 정책 의미 판정에 소비 | 도구별 `SUCCEEDED/PARTIAL/FAILED/SKIPPED`; retry는 같은 work의 새 attempt. `EXECUTED+0`, 미실행, 확인 불가를 구분하고 일부 실패를 안전함으로 해석하지 않음. 정책 fetch/parser 실패는 `COLLECTION_FAILED`(collection/parser failure 구분)이며 `VerificationResult` verdict로 변환하지 않고 program-policy semantic 작업(Rule Scope Gate, Reporter, disclosure downstream)만 fail-closed. Sandbox isolation/safety enforcement는 정책 준비 상태에 종속되지 않음 | `tests/integration/static_analysis/`, `tests/contract/test_rule_execution_record.py`, `tests/integration/test_policy_scope_admission.py`; 01·02·05·07·08·10·ADR-006 / R2·R5·R4·R7·R8 |
...
| 17. 준비된 정책 소비·Rule Scope·Primitive admission | LLM Gate + 비-LLM 결정 / R5·R4 / Rule Scope Agent·Admission Runtime | `PolicyAndScopeWorkflow.evaluate` / `RULE_SCOPE_GATE`, `PRIMITIVE_UPDATE` (이 단계에서 `POLICY_FETCH`를 새로 실행하지 않음) | same TRUE/CWE/Technical ACCEPT chain; run 초기화(3단계)에서 준비된 current `PolicyCollectionResult`, `FOUND`이면 current `ProgramPolicyRecord`와 공식 정책 원문(`source_ref + source_locator`) | Gate: `RuleScopeImpactReview`(hypothesis별). R4 runtime: `PrimitiveAdmissionDecision`; ALLOW이면 result Primitive와 index | Verification owner의 `CALL_RULE_SCOPE_GATE`; admission runtime의 `REGISTER_WORK/SAVE_RESULT`; run-init에서 고정된 정책 revision 참조, exact chain, source-원문 대조, testing restriction decision table 검사 | `gates/primitives/invocations/runs`; current Rule Scope·admission/index refs. `COLLECTION_FAILED`에는 Gate review 없음. 정책 record는 3단계에서 저장된 current program-level artifact를 참조만 함 | 15 ACCEPT → 17; admission ALLOW → 18, DENY → 19/22; 모든 Rule Scope 결과는 19 보고 조건에도 사용 | 정책은 3단계에서 run 종료까지 쓸 exact revision으로 준비됨: `COLLECTION_FAILED`이면 Rule Scope 호출 금지, 공식 부재는 `ABSENT_CONFIRMED → UNCERTAIN+DENY`, `STALE | UNVERIFIED`이면 기존 `PASS | ALLOW` 재사용 금지. run 도중 freshness 재계산·재수집·정책 교체는 하지 않고, 공식 정책 변경이 확인되면 policy-dependent downstream만 fail-closed하며 갱신 정책은 새 analysis run에서 준비. 원문 확인 불가·Parser 모순 시 fail-closed. testing restriction FAIL만 Primitive admission DENY | `tests/integration/test_policy_scope_admission.py`, `tests/security_negative/test_testing_restriction.py`; 05·06·07·08·10·ADR-011 / R5·R4·R1·R8 |
...
| `policies` | PolicyParserResult, PolicyCollectionResult, ProgramPolicyRecord | POLICY_COLLECTOR (run 초기화, program별 1회, cross-run current·fresh 재사용) | 수집 status와 exact official source/freshness/`parser_version` provenance |
