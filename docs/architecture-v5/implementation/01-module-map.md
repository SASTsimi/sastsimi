...
| 9. 위치 기반 Context 조회 | 비-LLM 코드 조회 / R2·R6 / Context Retrieval Service | `ContextService.retrieve` / 요청마다 `CONTEXT_RETRIEVAL` | ACTIVE assignment, current hypothesis와 exact proposal, `CodeContextRequest`, exact workspace/commit, allowed relation과 limits. `origin=CHAINING`이고 직접 entity·location이 비어 있으면 등록 단계에서 검증된 exact `source_primitive_match_id` 계보 | `CodeContextResponse`, code fragment artifact refs, `DataGap/AnalysisError`; Context Retrieval Service만 계보 재검사와 조회 결과 생산 | `READ_CODE`와 `SAVE_RESULT`; 일반 요청은 file path·workspace·commit·depth/byte/request budget·symlink boundary 검사. 체이닝 자식은 코드 조회 전에 exact `PrimitiveMatchCandidate`, upstream `result.entity_refs`, upstream의 모든 `inputs[].entity_refs`, 매칭된 downstream input의 `entity_refs`, downstream의 나머지 `inputs[].entity_refs`, current 계보와 `proposal.meta.workspace_id`·`proposal.meta.commit_id` 일치를 다시 검사 | `contexts`와 code fragment artifact; request/response exact pair와 returned location 기록 | 8 → 9 → 10/11; Verification은 `CodeContextRequest`만 만들고 Context Retrieval Service가 계보를 재검사하고 Context를 반환함 | 일부 조회 실패는 retry·대체 조회 가능. 계보가 무효·stale이거나, workspace·commit이 다르거나, 매칭된 downstream 위치·upstream 전제조건·유효한 시작점이 누락되면 Context와 Verification work를 final verdict 없이 중단한다. 조회 오류를 HOLD/FALSE로 바꾸지 않음 | `tests/integration/test_context_retrieval.py`, `tests/integration/test_chained_child_context.py`, `tests/security_negative/test_code_path_escape.py`; 02·04·06·07·08·10 / R1·R2·R3·R6·R4 |
...
등록 후 Context 조회 전 재검사에서 다음 시나리오를 확인한다.

- 등록 당시 유효했던 match candidate 또는 부모 Primitive가 stale 상태가 되면 Context 조회를 중단한다.
