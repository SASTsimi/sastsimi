...
1. 경계 전 Dynamic Reproduction Agent의 requirements·plan 작성 호출과 경계 승인 뒤 Dynamic Reproduction Agent 시작을 구분한다. 전자는 `agent_invoked`를 `true`로 만들지 않으며, 정책 차단이면 `agent_invoked=false`와 exact 정책 결정·AgentLog를 남긴다.
2. 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정은 같은 attempt의 event로 남긴다. session 재시작이 필요한 일시 오류만 새 retry attempt를 만들고, 외부 조건을 기다리는 경우에만 `BLOCKED` 뒤 `trigger=RESUME`인 새 attempt를 만든다. R6에 반환하는 current 결과에는 이전 attempt의 중간 실패 결과나 늦게 도착한 event를 섞지 않는다.
