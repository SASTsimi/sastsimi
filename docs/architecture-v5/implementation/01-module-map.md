...
10. Technical `REVISE`는 기존 work를 되살리지 않고 같은 ACTIVE Verification owner에게 새 generation으로 돌아간다.
...
| 9. 위치 기반 Context 조회 | 비-LLM 코드 조회 / R2·R6 / Context Retrieval Service | `ContextService.retrieve` / 요청마다 `CONTEXT_RETRIEVAL` | ACTIVE assignment, current hypothesis, `CodeContextRequest`, exact workspace/commit, allowed relation과 limits | `CodeContextResponse`, code fragment artifact refs, `DataGap/AnalysisError`; Context service만 조회 결과 생산 | `READ_CODE`와 `SAVE_RESULT`; file path·workspace·commit·depth/byte/request budget·symlink boundary 검사 | `contexts`와 code fragment artifact; request/response exact pair와 returned location 기록 | 8 → 9 → 10/11; Verification·Pro·Con이 필요한 만큼 반복 요청 가능 | 일부 실패는 retry/대체 조회 가능. 필수 검증 미완료면 parent Verification `BLOCKED` 또는 최종 `FAILED`; 조회 오류만으로 HOLD/FALSE 금지 | `tests/integration/test_context_retrieval.py`, `tests/security_negative/test_code_path_escape.py`; 02·04·07·08·10 / R2·R6·R4 |
...
| 12. 동적 재현·PoC | LLM + 비-LLM Sandbox / R6 요청·R7 실행 / R7 Agent·Setup Automation·Sandbox Controller·Session Manager | `DynamicReproductionService.execute` / `DYNAMIC_REPRO` | R6의 exact `DynamicReproductionRequest`, parent Verification/generation, R8 limits; R7이 만든 current `EnvironmentRequirements`·`ReproductionPlan`; Sandbox profile | R7 Agent: requirements/plan/PoC candidate·해석. Setup: recipe/environment/cleanup. Controller: policy decision. Session Manager: append-only `AgentLog`, validated `PoCBundle`, `DynamicReproductionResult` | R6 `REQUEST_DYNAMIC_REPRO`; R7 Setup `RUN_SANDBOX`; 각 producer `SAVE_RESULT`; Runtime Validator는 identity/revision/state/budget/same-attempt, Controller는 host·Docker·mount/namespace·secret·egress·workspace·resource 경계 검사 | `dynamic`, artifact store, append-only AgentLog, `actions/runs`; `DynamicReproductionState`, work output, COMMITTED transition이 같은 final result ref를 가리킴 | 11 → 12 → 13; generation당 work 하나, Sandbox 내부 event는 직렬/반복 가능 | 자체 해결 오류는 same work에서 즉시 새 attempt, 외부 조건만 `BLOCKED`; 불가·한도 소진 `FAILED+INCONCLUSIVE`; 정책 차단·실패는 final verdict 없음. validated PoC는 `SUCCEEDED+SUPPORTED`와 same-attempt 실행 log일 때만 생성 | `tests/integration/sandbox/`, `tests/contract/test_dynamic_provenance.py`, `tests/security_negative/test_sandbox_boundary.py`; 04·07·08·10·ADR-002/003/004/007 / R6·R7·R4·R8 |
...
| 16. Technical REVISE loop | hypothesis-local 제어 + LLM / R6·R5 / same Verification owner·CWE Labeling·Technical Gate | `RevisionWorkflow.start_new_generation` / 새 `VERIFICATION`, 새 `CWE_LABEL`, 새 `TECHNICAL_GATE` | COMMITTED `TechnicalEvidenceReview(REVISE)`, 직전 final Verification/CWE, ACTIVE assignment | 증가한 `verification_generation`, 새 VerificationResult; final TRUE이면 새 dynamic work/validated PoC, 새 CWELabel revision, 새 Technical review | `REGISTER_WORK`와 state CAS; 필요한 `READ_CODE/CALL_LLM/REQUEST_DYNAMIC_REPRO`, 이후 Gate action. old work/action/decision 재사용 차단 | 모든 과거 revision은 history; current hypothesis result·CWE·Gate pointer만 새 exact chain으로 원자 전환 | 15 REVISE → 같은 owner 9/10/12/13/14/15를 필요한 범위로 재수행; Orchestration이 목적지를 선택하지 않음 | 첫 attempt는 `INITIAL`; 입력 그대로인 provider 오류만 same-work `RETRY`. 보완 한도 소진은 Gate/Verification 실패, 이전 결과 자동 승격 금지 | `tests/e2e/test_technical_revise_generation.py`, `tests/security_negative/test_stale_revision_reuse.py`; 03·04·05·07·08·10 / R6·R5·R4·R7 |
...
| 18. Primitive Chaining | LLM match + trusted registry / R1 / Chaining Runtime·Chaining Agent | `ChainingService.match` / `CHAINING` | work 시작 시 고정한 current PrimitiveIndexState·considered Primitive exact refs; 실제 match의 direct/ancestor current ALLOW admission refs | `ChainingResult`, `PrimitiveMatchCandidate`, `LineageExclusion`, `HypothesisProposal(origin=CHAINING)`; CHAINING만 결과·proposal 생산 | `REGISTER_WORK`, `CALL_LLM(requested_by=CHAINING)`, `SAVE_RESULT`; result→input·lineage·fingerprint·restriction·source refs·current admission 검사 | `chaining/hypotheses/runs/invocations`; DB를 삭제하지 않고 exact result/history 저장, child proposal은 아직 미등록 | HOLD 또는 allowed TRUE Primitive → 18; candidate별 fan-out 가능, 결과 child는 20; no-match는 22 | stale/DENY admission은 결과 거절, 부모 verdict 유지. budget/LLM 실패는 `CHAINING_ERROR/BUDGET_EXCEEDED`; 미확인 match를 저장하지 않음 | `tests/contract/test_chaining_result.py`, `tests/integration/test_true_hold_true_true.py`; 03·06·07·08·10·ADR-001/005/011 / R1·R4·R5·R6·R8 |
| 19. 보고 가능 조건 확인 | 비-LLM readiness 검사 / R4·R5 / Reporting Condition Runtime | `ReportingPolicy.evaluate` / 별도 work 없음, 통과 시 `REPORT_DRAFT` 준비 | current Finding, final TRUE, current dynamic+PoC, Technical ACCEPT, Rule Scope exact review와 policy/CWE | report-ready 또는 구체적인 차단 사유; 새 vulnerability/policy verdict를 생산하지 않음 | `CREATE_REPORT_DRAFT` action의 `REVISION/REPORT_READY/REDACTION` preflight; 조건 PASS/PASS/PASS/SUFFICIENT/ALLOW exact 검사 | `actions/runs`; 허용/차단 decision과 `REPORT_NOT_READY`, current Finding/report pointer는 변경하지 않음 | 17의 Rule Scope 결과 → pass면 21, fail/uncertain/deny면 22; admission/Chaining과 독립 축 | 정책 stale·Finding 없음·revision 불일치는 Reporter 차단만 하고 기술 TRUE·Primitive 자격을 바꾸지 않음 | `tests/contract/test_report_readiness.py`, `tests/security_negative/test_reporter_preconditions.py`; 05·07·08·10 / R4·R5·R6·R8 |
...
[08. 경량 데이터 계약](../08-lightweight-data-contracts.md)은 `CALL_LLM`의 `requested_by=R7_AGENT`를 허용하지만 `LLMCallSpec.agent_role`과 `LLMInvocationRequest.agent_role` enum에는 `R7_AGENT`가 없다. 이대로는 R7 Agent 호출이 schema를 통과할 수 없다.
