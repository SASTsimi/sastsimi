...
6. 한 Verification generation에는 `DYNAMIC_REPRO` work가 하나뿐이다. 같은 R7 Agent session의 command·PoC·환경 조정은 현재 attempt를 유지한다. session 재시작은 같은 `work_id`의 새 `attempt_id`·`trigger=RETRY`, 외부 조건 해소 뒤 재개는 새 `attempt_id`·`trigger=RESUME`를 사용한다.
...
1. 경계 전 R7 Agent의 requirements·plan 작성 호출과 경계 승인 뒤 Sandbox 실행 Agent 시작을 구분한다. 전자는 `agent_invoked`를 `true`로 만들지 않으며, Sandbox profile 외부 격리 경계 차단이면 `agent_invoked=false`와 exact `SandboxPolicyDecision`·AgentLog를 남긴다.
...
| Reproduction Session Manager | 실제 event와 exact `SandboxCommandRecord`를 append-only AgentLog로 기록하고 동적 결과·validated PoC 확정 | Agent 실행 전략·retry·cleanup 판단 | 12 |
