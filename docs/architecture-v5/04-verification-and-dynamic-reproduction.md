...
R6는 공식 프로그램 정책을 직접 수집하거나 해석하지 않으며 `DynamicReproductionRequest`에도 정책 reference를 복사하지 않는다. 정책 준비와 동적 재현 요청은 서로 독립적으로 진행할 수 있다. 정책 준비가 끝나지 않았더라도 R6의 정적·Pro·Con 검증은 계속 진행한다.

R6가 동적 재현을 요청하면 trusted runtime이 해당 `DYNAMIC_REPRO` work의 `WorkExecutionState.input_refs`에 같은 프로그램의 exact `PolicyCollectionResult`를 고정한다. `PolicyCollectionResult.status=FOUND`이면 그 결과가 가리키는 current exact `ProgramPolicyRecord`와 해당 record의 `parser_result_refs`도 함께 고정한다. Sandbox에서 적용할 금지 테스트 항목은 `ProgramPolicyRecord.testing_restrictions[].policy_item_id`로 식별한다.

정책 수집·파싱·최신성 확인이 진행 중이면 동적 work만 `BLOCKED`, `waiting_for=[DEPENDENCY]`로 대기한다. 이 대기는 정적·Pro·Con 검증을 중단시키지 않는다. 정책 준비 실패·정책 부재·정책 불확실성·정책 만료는 가설의 반증 근거가 아니므로 `FALSE | HOLD`로 변환하지 않는다.
...
- **Sandbox Controller**: R7 `sandbox_profile_ref`의 host·Docker daemon/socket·mount/namespace·secret·egress·다른 workspace 격리와 CPU·RAM·disk·PID·요청 가능 최대 시간을 강제한다. 또한 trusted runtime이 `DYNAMIC_REPRO` work에 고정한 exact `PolicyCollectionResult`, current `ProgramPolicyRecord`, parser 결과와 `testing_restrictions[].policy_item_id`를 확인해 실행하려는 테스트 방법이 허용되는지 판정한다. Controller는 정책 원문을 새로 수집하거나 자연어 정책을 다시 해석하지 않는다.
...
- `BLOCKED`는 외부 설정·정책 준비·승인 또는 resource profile 변경을 기다릴 때만 사용한다. 정책 수집·파싱·최신성 확인이 진행 중이면 `waiting_for=[DEPENDENCY]`로 두고 Sandbox attempt와 R7 Agent를 시작하지 않는다. 준비가 완료되면 exact 정책 입력을 work에 고정한 뒤 같은 work의 새 `attempt_id`, `trigger=RESUME`를 만든다.
...
- `SandboxPolicyDecision`은 실제 검사한 exact `PolicyCollectionResult`, `ProgramPolicyRecord`와 `testing_restriction_item_ids`를 기록한다. `ALLOW`는 `FOUND`인 collection, `CURRENT`인 policy record와 실행 방법에 적용되는 금지 테스트 항목 검사를 모두 통과했을 때만 가능하다.
- 정책이 준비 중이면 아직 `SandboxPolicyDecision`이나 실행 결과를 만들지 않고 work를 `BLOCKED`로 유지한다.
- `ABSENT_CONFIRMED`, `COLLECTION_FAILED`, parser 실패, `STALE | UNVERIFIED` 정책 또는 금지 테스트 위반으로 안전한 실행을 허가할 수 없으면 Sandbox를 시작하지 않는다. 복구 가능하면 `BLOCKED`, 복구 불가능하면 `FAILED`로 끝내며 final verdict와 Technical Gate 입력을 만들지 않는다.
...
`DynamicReproductionState.dynamic_result_ref`, `WorkExecutionState.output_refs`와 `TransitionCommit.output_refs`는 같은 exact `DynamicReproductionResult.meta.record_id`를 가리키는 current 반환 동적 결과만 R6에 전달한다. current 반환 결과에는 비종료 상태인 `BLOCKED`가 포함될 수 있고 R6는 이를 대기·복구 판단에 읽을 수 있지만, final `VerificationResult`나 Technical Gate를 포함한 Gate 입력으로 사용할 수 없다. R6는 자격을 갖춘 결과를 소비해 final verdict를 만들지만 R7 산출물을 대신 생산하지 않는다. current generation의 exact request, `SUCCEEDED + SUPPORTED` 결과, validated `poc_ref`, 그리고 실제 Sandbox 실행을 허가한 current exact `SandboxPolicyDecision(decision=ALLOW)` 중 하나라도 없으면 final TRUE 저장과 Technical Gate 호출을 모두 차단한다. R6는 `DynamicReproductionResult.policy_decision_ref`를 따라 실제 실행에 사용된 정책 revision을 추적하지만 정책의 의미를 다시 판단하지 않는다.
