...
- Sandbox 실행 Agent 호출 전 외부 격리 경계 차단도 `agent_invoked=false`, 당시 exact `RunPolicyState` 감사 reference·Sandbox 정책 결정과 `POLICY_BLOCKED` event를 가진 결과로 남긴다. 여기서 `POLICY_BLOCKED`는 프로그램 정책이나 testing restriction 판정이 아니라 Sandbox profile의 외부 경계 위반이다.
