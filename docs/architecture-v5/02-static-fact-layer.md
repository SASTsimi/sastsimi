...
- 이렇게 reachability 근거가 붙은(또는 붙지 않은) source 후보는 Hypothesis Agent의 초기 가설 생성과 Verification·Pro·Con의 반증·검증 근거로 사용한다. Pro·Con과 최종 판정을 내리는 Verification Agent는 같은 exact 정적분석 결과 revision을 사용한다. Chaining Agent는 이 정적 source 후보를 직접 소비하지 않는다 — 코드 문맥을 새로 조회하지 않고, 가설별 `PrimitiveIndexState`가 가리키는 current `Primitive`와 그 `Primitive`를 만든 exact Verification(`source_verification_ref`), 있는 경우 그 Technical review(`technical_review_ref`; final HOLD는 `null`)만 사용한다는 기존 경계([03. Agent 역할과 오케스트레이션](./03-agent-roles-and-orchestration.md), [06. Chaining](./06-chaining.md))를 그대로 따른다. `Primitive`는 별도 상태 필드를 두지 않고 등록 조건이 판정 유형마다 다르다.
  ...
  - final `TRUE`: 현재 Verification generation의 성공한 동적 재현(validated PoC), Technical Gate `ACCEPT`와 `provided_primitive_candidates`만으로 등록을 확정하지 않는다. 같은 Verification의 current `PrimitiveAdmissionDecision=ALLOW`가 있어야 제공 능력마다 `result`가 있는 `Primitive`를 하나씩 등록하며, `Primitive.admission_decision_ref`는 그 current exact ALLOW decision을 가리킨다. 일반 `rule_compliance`·`scope_compliance`·`security_impact`·`report_permission`은 reportability를 판단할 뿐 result Primitive admission의 직접 authority가 아니다. 다만 R5의 `testing_restriction_compliance`와 provenance는 R4 `PRIMITIVE_ADMISSION_RUNTIME`의 입력이며, confirmed `FAIL`은 `PrimitiveAdmissionDecision=DENY`로 매핑되어 result Primitive 등록과 Chaining을 차단한다.
