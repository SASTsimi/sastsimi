...
| confirmed/held 목록 | `result=null`인 HOLD Primitive와 Technical-accepted + testing-restriction-PASS TRUE의 result Primitive, current `PrimitiveIndexState` |
...
8. Technical `ACCEPT` 뒤 official `ProgramPolicyRecord`·Rule Scope Impact Gate의 `testing_restriction=PASS` 기반 result Primitive admission
