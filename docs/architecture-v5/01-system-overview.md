...
| Sandbox Controller | 재현 계획의 image·command·file·network·resource·cleanup 정책 검사와 실행 환경 통제 | 취약점 verdict 판단 또는 정책 미검사 실행 |
| Sandbox Runner | Controller가 승인한 exact 계획을 Docker 안에서 실행하고 단계별 log·관측 수집 | 정책 변경 또는 계획 밖 명령 실행 |
