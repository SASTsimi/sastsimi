...
- collection failure 또는 parser failure를 `VerificationResult.FALSE` 또는 `VerificationResult.HOLD`로 변환하지 않는다. Policy 준비 실패는 program-policy semantic dependency가 있는 작업(Rule Scope Gate의 policy-semantic 판정, Reporter, disclosure/report downstream)만 fail-closed시키고 기술적 Verification verdict와 분리한다. Sandbox Controller의 isolation/safety enforcement 자체는 program policy freshness·availability에 종속되지 않으며, `LOCAL_ONLY` Sandbox 실행은 정책 freshness·source 변경만으로 자동 BLOCK·EXPIRED되지 않는다.
- Sandbox Controller는 bug bounty program testing restriction의 semantic compliance를 판정하지 않는다. Sandbox 안에서 Agent가 고른 command·package·PoC를 program-policy allowlist처럼 의미 해석하지 않으며, 공식 program testing restriction과 실제 수행 행위의 의미 비교는 Rule Scope Gate의 `testing_restriction_compliance`가 유일한 authoritative 판정자다. `SandboxPolicyDecision`의 `ALLOW`는 external/live target testing 허가가 아니고, `POLICY_BLOCKED`은 isolation/safety/external-boundary enforcement 실패이지 `testing_restriction_compliance=FAIL`이 아니다.
...
- 정책은 run 초기화에서 확정한 exact revision을 run 종료까지 사용한다. freshness 만료와 parser version 변경은 다음 analysis run을 시작할 때 이전 정책 자료 재사용 여부만 결정하며, run 도중에는 freshness 재계산·`POLICY_FETCH` 재실행·record 교체·`policy_generation` 증가를 하지 않는다. run 도중 공식 정책 변경이 확인되면 현재 정책 reference를 바꾸지 않고 policy-dependent downstream(새 Rule Scope 판정·Reporter·disclosure)을 fail-closed(`UNCERTAIN + DENY`)하며 갱신된 정책은 새 analysis run에서 준비한다. isolated `LOCAL_ONLY` Sandbox reproduction은 이 정책 변경 때문에 자동 차단·만료 처리하지 않는다.
...
| N45 | run 초기화 정책 준비(collection 또는 parser)가 실패했는데 그 hypothesis의 `VerificationResult`를 `FALSE | HOLD`로 저장하거나 기술적 hypothesis를 제거 | verdict 변경·hypothesis 제거 거절; `COLLECTION_FAILED`를 보존하고 program-policy semantic 작업(Rule Scope Gate policy-semantic 판정, Reporter, disclosure downstream)만 fail-closed; Sandbox isolation/safety enforcement는 미영향 |
| N46 | run 도중 정책 freshness 만료나 공식 정책 변경이 감지되었는데 현재 run의 정책 reference를 새 revision으로 교체하거나 `POLICY_FETCH`를 다시 실행하거나 `policy_generation`을 올림 | 교체·재실행·generation 증가 거절; 현재 run 정책 reference는 그대로 두고 policy-dependent downstream(새 Rule Scope 판정·Reporter·disclosure)을 fail-closed(`UNCERTAIN + DENY`); 갱신 정책은 새 analysis run에서 준비; `LOCAL_ONLY` Sandbox reproduction은 자동 차단·만료하지 않음 |
| N47 | 새 analysis run 시작 시 이전 run의 `STALE` 정책 자료나 parser version이 비호환인 자료를 새 collect + parse 없이 current 정책으로 그대로 사용 | 재사용 거절; 새 run은 새 collection + parsing으로 자기 run이 소유하는 정책 record를 만들고, 과거 `STALE` 자료를 current Gate 입력으로 쓰지 않음 |
