...
final `VerificationResult`에서 `NOT_REQUIRED`는 실행을 요청하지 않았다는 뜻이므로 `dynamic_result_ref=null`이어야 하며 final `TRUE`에는 사용할 수 없다. final TRUE는 decision과 무관하게 current Verification generation의 exact request, `SUCCEEDED + SUPPORTED` 결과와 실제 실행으로 검증된 `poc_ref`가 모두 필요하다. `LIMITED_REPRO | FULL_REPRO` 실행이 종료됐다면 `dynamic_result_ref`는 필수이고 대상 `mode`가 decision과 같아야 한다. 실행 중 상태는 final 결과가 아니라 `DynamicReproductionState`로 관리한다.

| decision | 결과 status | 구조 계약 | 의미 및 Gate 처리 |
|---|---|---|---|
| `NOT_REQUIRED` | 실행 없음 | `dynamic_result_ref=null`, validated `poc_ref=null`; final TRUE 금지 | 동적 결과 ref가 있으면 invalid output으로 Gate 전에 차단. PoC candidate 존재만으로 실행·성공을 주장할 수 없음 |
