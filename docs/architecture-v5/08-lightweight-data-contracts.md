...
`freshness_status=CURRENT`이면 `freshness_criterion_ref`, 하나 이상의 `freshness_evidence_refs`, `freshness_checked_at`과 미래의 `freshness_valid_until`이 모두 필수다. freshness 기준값과 재수집 주기는 R8이 승인한 versioned 설정만 사용한다. 기준을 넘었으면 `STALE`, 확인 자체가 실패했거나 기준을 적용할 수 없으면 `UNVERIFIED`다. 두 상태 모두 `freshness_warning` 또는 `PolicyMissingInfo(area=FRESHNESS, blocks_allow=true)`에 이유가 있어야 하며 Gate의 `PASS | ALLOW` 근거로 사용할 수 없다. `STALE | UNVERIFIED` 여부는 새 analysis run을 시작할 때 이전 정책 자료를 재사용할 수 있는지 판단하는 조건이다. run이 시작되면 그 run은 확정한 exact 정책 revision을 run 종료까지 사용하고, Rule Scope Gate work와 Reporter는 이 고정된 revision을 소비하며 run 도중 freshness를 다시 계산하거나 `freshness_valid_until`을 재확인하지 않는다. 기준 설정의 의미·임곗값·재수집 운영은 R8, 정책 해석은 R5, exact field와 run 시작 시 만료 차단은 R4 책임이다.

각 `RuleScopeImpactReview`는 사용한 exact `ProgramPolicyRecord` revision을 `policy_record_ref.record_id`로 추적한다. 이 revision은 run에 고정되며 같은 run 안에서 교체하지 않는다. run 도중 공식 정책 변경이 별도로 확인되면 현재 정책 reference를 바꾸거나 `POLICY_FETCH`를 다시 실행하지 않고, 새 Rule Scope 판정·Reporter 등 policy-dependent downstream을 fail-closed(`UNCERTAIN + DENY`)하며 갱신된 정책은 새 analysis run에서 준비한다. `STALE | UNVERIFIED` 여부와 parser version 변경은 다음 analysis run이 이전 정책 자료를 재사용할지 판단하는 조건이며, run 도중의 downstream 무효화 트리거가 아니다.
