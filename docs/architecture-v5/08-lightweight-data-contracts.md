...
| `DYNAMIC_REPRO` | Verification의 제안 뒤 Runtime Validator가 호출을 허가하고 Sandbox Controller가 정책을 승인 | Sandbox Controller·Runner runtime | `DynamicReproductionResult` |
...
| `RUN_SANDBOX` | SCHEMA, AUTHORITY, REVISION, STATE, BUDGET | 요청자·현재 work·예산과 exact `ReproductionPlan` reference를 확인해 Sandbox Controller 호출만 허가 |
...
- `result_kind=verification_result`이면 원본 `required_validation`의 모든 항목이 `required_validation_dispositions`에 정확히 한 번씩 대응하는지 검사한다. `disposition=PERFORMED | SUBSTITUTED`인데 `evidence_refs`가 비어 있거나, `record_id=null`인 reference가 있거나, 그 `record_id`가 가리키는 record의 `workspace_id`·`commit_id`·`RecordMeta.hypothesis_id`가 이 `VerificationResult`와 다르면 `SCHEMA` check를 `FAIL`로 만들어 저장을 거절한다.
...
`disposition`별 `evidence_refs` 요구는 다음과 같다. `PERFORMED`는 실제 수행 결과를 가리키는 `evidence_refs`가 하나 이상이어야 한다. `SUBSTITUTED`는 대체 판단을 뒷받침하는 정적·Pro/Con·동적 근거의 `evidence_refs`가 하나 이상이어야 한다. 두 disposition의 각 reference는 `record_id`가 `null`이 아닌 `StoredDataRef`여야 하고(독립 artifact가 아니라 실제 저장 record를 가리켜야 함), `workspace_id`·`commit_id`가 이 `VerificationResult`와 같아야 하며, 그 `record_id`가 가리키는 record의 `RecordMeta.hypothesis_id`도 이 `VerificationResult`의 `hypothesis_id`와 같아야 한다. `NOT_NEEDED`는 `rationale`에 근거 있는 설명이 필수이지만, 저장된 artifact 없이 판단할 수도 있으므로 `evidence_refs`는 비어 있을 수 있다. `UNRESOLVED`는 아직 근거가 없다는 뜻이므로 `evidence_refs` 유무를 강제하지 않는다.
