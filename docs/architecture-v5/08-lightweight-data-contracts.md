...
| `question_id` | proposal 출력 검증 runtime 또는 플레이북 질문 적용을 확정하는 trusted runtime | 전체 시스템 | `FalsificationQuestion`과 `FalsificationResult` | proposal 질문은 등록 가설까지 같은 ID를 유지한다. 플레이북의 질문 템플릿을 가설에 적용할 때는 새로운 전역 고유 ID를 발급하며, 템플릿 이름이나 다른 가설의 ID를 재사용하지 않는다. |
