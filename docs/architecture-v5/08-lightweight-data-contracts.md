...
| `restriction_id` | proposal 출력 검증 runtime 또는 Verification 출력 검증 runtime | 전체 시스템 | `Restriction`과 이를 승계한 Verification·Primitive·Chaining·ReportDraft | Agent가 낸 출력 지역 값을 그대로 채택하지 않고 검증 runtime이 부여한다. 변경 없이 승계하면 유지하고, Verification이 새로 만들거나 `statement`·`fact_refs`·`evidence_refs`가 바뀌면 Verification 출력 검증 runtime이 새 값을 발급한다 |
