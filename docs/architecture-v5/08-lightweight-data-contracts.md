...
- `result_kind=environment_requirements | reproduction_plan | poc_candidate | r7_agent_conclusion`이면 R7_AGENT만 저장할 수 있다. 한 LLM 호출은 이 중 한 result kind와 한 parsed output만 만든다. requirements는 R6 request의 모든 `environment_needs`를 빠뜨리거나 약화하지 않고, plan의 request·purpose·hypothesis·profile은 request와 exact match하며 current requirements를 가리킨다. plan에는 mode·exact command·step·payload·cleanup allowlist를 넣지 않는다. candidate의 `llm_call_id`는 current request·plan·attempt를 입력으로 사용한 성공한 `CREATE_POC_CANDIDATE` 호출과 같아야 하고, conclusion의 `llm_call_id`는 같은 attempt의 request·plan·환경·candidate·관찰을 사용한 성공한 `INTERPRET_ATTEMPT` 호출과 같아야 한다. 두 호출 log의 `parsed_output_ref`는 각 exact output revision을 단방향으로 가리킨다. candidate를 처음 저장할 때는 current request·plan·attempt, 성공 호출과 content digest를 검사하며 아직 뒤따를 AgentLog event를 요구하지 않는다. conclusion은 `TRUE | FALSE | HOLD`를 출력하지 않는다. candidate나 conclusion 존재만으로 실행이나 성공을 인정하지 않고, `DynamicReproductionResult`와 validated PoC를 저장할 때 same-attempt `AgentLog`의 작성·실행 event가 exact candidate revision·digest·관찰을 가리키는지 검사한다.
...
`DynamicReproductionResult.agent_conclusion_ref`는 값이 있으면 위 exact conclusion을 가리킨다. `agent_invoked=false`이면 이 값은 반드시 `null`이다. 동적 해석이 완료돼 `status=SUCCEEDED | PARTIAL`이거나 `hypothesis_outcome=SUPPORTED | DISPROVED`이면 같은 attempt의 conclusion이 필수다. 정책 차단, 환경 실패, 취소 또는 Agent가 결과를 반환하기 전의 실패에서는 `null`일 수 있다. 비-LLM Reproduction Session Manager는 conclusion을 실제 `AgentLog`, 환경, PoC candidate와 관찰 record에 대조한 뒤에만 `DynamicReproductionResult`를 확정하며, 제안된 outcome이 실제 사건과 다르면 결과를 성공으로 저장하지 않는다.
