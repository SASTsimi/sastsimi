...
이미 저장된 조합이라 검토를 건너뛴 경우는 여기 넣지 않는다. 조상 재사용 제외는 `excluded_lineage_refs`, 예산·오류로 확인하지 못한 후보는 work 상태와 `errors`가 담는다. Runtime Validator는 위 구조와 소속만 검사하고 `reason_code`·`detail`의 판단 내용은 다시 판정하지 않는다.
