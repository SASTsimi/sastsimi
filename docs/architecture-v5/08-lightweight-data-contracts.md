...
| `restriction_id` | proposal 출력 검증 runtime 또는 Verification 출력 검증 runtime | 전체 시스템 | `Restriction`과 이를 승계한 Verification·Primitive·Chaining·ReportDraft | Agent가 낸 출력 지역 값을 그대로 채택하지 않고 검증 runtime이 정렬한 `fact_refs`의 canonical SHA-256에서 유도한다. `fact_refs`가 비어 있을 때만 새 값을 발급한다. 승계할 때 유지하고 `fact_refs`가 바뀌면 다른 값이 된다 |
...
| `primitive_match_id` | Chaining 출력 검증 runtime | 전체 시스템 | `PrimitiveMatchCandidate`와 이를 가리키는 `source_primitive_match_id` | Agent가 낸 출력 지역 값을 그대로 채택하지 않고 `(upstream_result_ref, downstream_input_ref, matched_input_id)`의 canonical SHA-256에서 유도한다. 세 값이 같으면 같은 값이 된다 |
...
Chaining work는 새 Primitive 저장을 계기로 등록한다. `PRIMITIVE_UPDATE`가 Primitive와 `PrimitiveIndexState` 갱신을 COMMITTED한 뒤에만 그 Primitive를 `trigger_primitive_ref`로 가리키는 `CHAINING` work를 등록하며, 저장 전에 등록하지 않는다. 같은 analysis 안에서 이 등록은 다음 `PRIMITIVE_UPDATE`가 COMMITTED되기 전에 끝난다. 따라서 각 work의 후보 pool은 실제 저장 순서를 그대로 따르고, 서로 다른 두 Chaining work의 pool이 서로를 담는 상태는 생기지 않는다. `subject_type=ANALYSIS`, `subject_id`는 `analysis_id`이고 계기가 된 Primitive는 `trigger_primitive_ref`로만 식별한다.
...
한 조합의 담당은 두 Primitive 중 자기 후보 pool에 상대가 들어 있는 work다. pool은 `REGISTER_WORK`가 COMMITTED된 index에서 고정하므로 실제 저장 순서를 그대로 따른다. 등록 순서가 보장되므로 두 work가 서로를 pool에 담는 일은 없고, 한 조합의 담당은 항상 하나로 정해진다. 각 work는 담당인 조합만 검토·저장한다. 담당이 아닌 조합은 검토 대상이 아니라서 `no_match_reasons`에도 넣지 않고 별도 skip 목록도 두지 않는다.
...
ActionRequest:
  meta: RunMeta | RecordMeta
  action_id: string
  requested_by: ORCHESTRATION | HYPOTHESIS | PRO | CON | VERIFICATION | CWE_LABELING | CHAINING | TECHNICAL_GATE | RULE_SCOPE_GATE | REPORTER | REPOSITORY_LOADER | CONTEXT_RETRIEVAL_SERVICE | STATIC_ANALYSIS | POLICY_COLLECTOR | POLICY_PARSER | PRIMITIVE_ADMISSION_RUNTIME | DYNAMIC_REPRODUCTION | REPRODUCTION_SETUP_AUTOMATION | SANDBOX_CONTROLLER | REPRODUCTION_SESSION_MANAGER | BUDGET_RUNTIME | R8_EVALUATION_RUNTIME | RECOVERY
...
- `result_kind=hypothesis_proposal`은 schema-valid proposal을 전역 등록하는 비-LLM ORCHESTRATION runtime만 저장한다. LLM이 반환한 proposal candidate를 source 결과에 넣기 전에 trusted proposal 출력 검증 runtime이 `proposal_id`, 질문별 `question_id`, 검증 항목별 `validation_id`를 발급하고 구조·의미를 검사한다. `origin=INITIAL`은 성공한 `HYPOTHESIS / GENERATE_INITIAL` 호출의 검증된 배열 원소, `origin=VERIFICATION`은 current Verification output의 `material_child_proposals` 원소, `origin=CHAINING`은 `ChainingResult.chained_hypothesis_proposals` 원소여야 한다. ORCHESTRATION 등록 runtime은 source 결과가 COMMITTED된 뒤 그 안의 proposal ID·문장·목록·순서와 exact parent를 그대로 사용해 별도 immutable record로 저장하며 ID를 다시 발급하거나 Agent 문장을 수정하지 않는다. source 역할·origin이 다르거나 source parent에 존재하지 않는 proposal은 저장하지 않는다. INITIAL은 exact `StaticFactBundle`과 성공 invocation, VERIFICATION과 CHAINING은 각 COMMITTED source result와 그 transitive provenance를 `SAVE_RESULT.input_refs`에 고정한다. source work의 output과 proposal 등록은 서로 다른 atomic transition이며, source 결과가 COMMITTED되기 전에는 proposal을 등록하지 않는다. 등록 뒤 `ProposalProcessState.proposal_ref`가 exact proposal revision을 가리키며, 일부 proposal 누락·다른 attempt 혼합·nested proposal의 부모 계보 유실을 허용하지 않는다.
...
- `result_kind=chaining_result`이면 `considered_primitive_refs`는 Runtime이 `REGISTER_WORK(work_type=CHAINING)`에서 고정한 exact Primitive 입력 집합과 set-equal하다. `SAVE_RESULT.input_refs`에도 current work·candidate와 함께 그 Primitive, work가 고정한 `PrimitiveIndexState`가 모두 있어야 한다. work 시작 뒤 생긴 새 Primitive나 다른 work가 고정한 reference를 결과에 추가하면 `STALE_RESULT`로 거절한다. index revision이 올라간 것만으로는 기존 work를 거절하지 않는다. Chaining 출력 검증 runtime은 저장 전에 각 candidate의 `primitive_match_id`를 `(upstream_result_ref, downstream_input_ref, matched_input_id)`에서 유도한 값으로 바꾸고, 같은 출력 안의 `chained_hypothesis_proposals[].source_primitive_match_id`도 같은 값으로 맞춘다. Agent가 낸 지역 값을 그대로 저장하지 않는다.
...
- 각 `chained_hypothesis_proposals`는 COMMITTED match의 exact `source_primitive_match_id`와 부모 가설을 보존한다. `origin=CHAINING`이면 `observed_facts=[]`만 허용한다. `target_entities`·`target_locations`·`suspected_path`는 비어 있을 수 있지만, 값을 넣으면 부모 Primitive의 `result.entity_refs`와 `inputs[].entity_refs`에서 exact하게 얻을 수 있는 범위를 벗어날 수 없다. 부모 reference가 무효하거나 Verification이 사용할 entity·location 시작점을 하나도 복원할 수 없으면 가설 등록과 Verification 배정을 거절한다. proposal restrictions는 두 부모 Primitive의 `Restriction` 객체를 `restriction_id` 기준으로 중복 없이 합친 집합과 exact match한다. 같은 ID가 양쪽에 있으면 canonical bytes가 사전순으로 앞서는 객체 하나만 남긴다. `restriction_id`가 `fact_refs`에서 유도되므로 같은 ID인데 `fact_refs`가 다르면 상류 발급 오류이며 저장하지 않는다. 남은 `PrimitiveDraft`마다 `description`과 같은 assumption을 정확히 하나 보존하고, 결합 지점을 겨냥한 반증 질문이 하나 이상 있는지(목록이 비어 있지 않은지만) 확인한다 — 그 질문이 실제로 결합 지점을 겨냥했는지는 Technical Evidence Gate의 의미적 충분성 검토 몫이다.
...
- `result_kind=environment_requirements | reproduction_plan | poc_candidate | dynamic_reproduction_tool_request | dynamic_reproduction_conclusion`이면 DYNAMIC_REPRODUCTION만 저장할 수 있다. 한 LLM 호출은 이 중 한 result kind와 한 parsed output만 만든다. requirements는 R6 request의 모든 `environment_needs`를 빠뜨리거나 약화하지 않고, plan의 request·purpose·hypothesis·profile은 request와 exact match하며 current requirements를 가리킨다. plan에는 mode·exact command·step·payload·cleanup allowlist를 넣지 않는다. candidate의 `llm_call_id`는 current-generation exact R6 request와 현재 R7 work·attempt의 plan을 입력으로 사용한 성공한 `CREATE_POC_CANDIDATE` 호출과 같아야 한다. tool request는 current-generation exact R6 request와 같은 R7 attempt의 plan·READY 환경·현재 AgentLog를 사용한 `EXECUTE_REPRODUCTION` 호출에서만 만든다. conclusion의 `llm_call_id`는 current-generation exact R6 request와 같은 R7 attempt의 plan·환경·candidate·관찰을 사용한 성공한 `INTERPRET_ATTEMPT` 호출과 같아야 한다. 각 호출 log의 `parsed_output_ref`는 exact output revision 하나를 단방향으로 가리킨다. candidate를 처음 저장할 때는 current-generation exact R6 request, 현재 R7 work·attempt의 plan, 성공 호출과 content digest를 검사하며 아직 뒤따를 AgentLog event를 요구하지 않는다. conclusion은 `TRUE | FALSE | HOLD`를 출력하지 않는다. candidate·tool request·conclusion 존재만으로 실행이나 성공을 인정하지 않고, `DynamicReproductionResult`와 validated PoC를 저장할 때 same-attempt `AgentLog`의 작성·실행 event가 exact candidate revision·digest·tool request·관찰을 가리키는지 검사한다.
...
`Restriction`은 공격을 제한하는 검사·경계를 자연어만으로 남기지 않고 exact 근거에 연결하는 공통 객체다. `fact_refs`와 `evidence_refs` 중 하나 이상은 비어 있지 않아야 한다. `CodeFactRef.bundle_ref`는 해당 사실을 포함한 final `StaticFactBundle`의 exact revision을 가리키고 `fact_id`는 그 bundle 안에 정확히 한 번 존재해야 한다. 모든 참조는 현재 record와 같은 analysis·workspace·commit이어야 한다. `restriction_id`는 검증 runtime이 `bundle_ref`까지 채운 완전한 `fact_refs`를 canonical JSON으로 만든 SHA-256에서 유도한다. `fact_id`는 같은 `workspace_id + commit_id` 안에서만 유일하므로 `bundle_ref` 없이 유도하면 전역 ID가 충돌한다. `fact_refs`는 순서가 의미 없는 배열이므로 canonical 직렬화 전에 항목 전체 canonical bytes로 정렬한다. `statement`와 `evidence_refs`는 유도에 넣지 않는다. 표현이 다르거나 확인 경로가 달라도 같은 코드 근거를 가리키는 제한은 같은 ID를 갖게 하기 위해서다. 따라서 한 `fact_refs` 집합에는 제한을 하나만 만든다. 같은 근거로 서로 다른 조건을 말해야 하면 한 제한으로 합치거나 근거를 나눠 인용한다. `fact_refs`가 비어 있으면 유도할 재료가 없으므로 그때만 검증 runtime이 새 값을 발급한다. 같은 제한을 Verification·Primitive·Chaining·ReportDraft로 전달할 때는 ID를 그대로 유지하고, `fact_refs`를 바꾸면 기존 객체를 덮어쓰지 않고 다른 `restriction_id`가 된다.
...
downstream Primitive의 `result=null`이면 TRUE_HOLD, non-null이면 TRUE_TRUE로 유도한다. `primitive_match_id`는 분석 전체에서 유일하고 같은 `(upstream_result_ref, downstream_input_ref, matched_input_id)` 조합도 중복 저장하지 않는다. 세 값이 match 하나를 유일하게 식별하므로 중복 판정을 위한 별도 요약 값은 두지 않으며 `normalized_fingerprint`는 사용하지 않는다. Chaining 출력 검증 runtime이 그 세 값을 순서대로 담은 canonical JSON의 SHA-256에서 `primitive_match_id`를 유도한다. Agent가 낸 지역 값은 같은 출력 안에서 `chained_hypothesis_proposals[].source_primitive_match_id`가 어느 candidate를 가리키는지 잇는 데만 쓰고, runtime이 유도 값으로 바꿀 때 그 참조도 함께 바꾼다. match는 queue message, verdict, Finding 또는 impact 확정이 아니다.
...
`PoCCandidate.llm_call_id`는 같은 analysis·hypothesis·work·attempt에서 candidate를 만든 성공한 `DYNAMIC_REPRODUCTION / CREATE_POC_CANDIDATE` 호출 ID와 같아야 한다. 그 호출의 `LLMInvocationLog.parsed_output_ref`가 candidate exact revision을 단방향으로 가리키며 candidate는 log record를 역참조하지 않는다.

`DynamicReproductionConclusion`은 Dynamic Reproduction Agent의 해석 제안이지 최종 실행 사실이나 취약점 판정이 아니다. `llm_call_id`는 같은 analysis·hypothesis·work·attempt의 성공한 `DYNAMIC_REPRODUCTION / INTERPRET_ATTEMPT` 호출 ID와 같아야 하고, 그 log의 유일한 `parsed_output_ref`는 이 conclusion exact revision이어야 한다. conclusion은 log record를 역참조하지 않는다. conclusion의 request·plan·environment·candidate·observation reference는 그 호출의 허용 입력과 같아야 하며 다른 attempt의 record를 섞지 않는다.
...
`PromptPayload`는 trusted Prompt Builder가 그 entry와 허용된 exact context로 조립한 불변 호출 입력이다. 각 binding의 `source_ref`는 원본 exact record, `projected_data_ref`는 허용 `field_paths`만 뽑아 canonical serialization한 불변 artifact다. repository 코드·문서·정책 원문·도구·이전 LLM 출력은 `UNTRUSTED_DATA`로만 binding하며 `TRUSTED_INSTRUCTION`으로 승격하지 않는다. `rendered_prompt_ref`는 template과 projected data를 결합하고 비밀값과 host 절대 경로를 제거한 실제 전송 직전 artifact를 가리킨다.
