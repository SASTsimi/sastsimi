...
current Finding 정규화는 이 표에 새 LLM producer 행을 추가하지 않는다. `RULE_SCOPE_GATE`가 `SUCCEEDED`로 종료된 뒤 신뢰 runtime이 exact chain에서 Finding을 조립한다. 이 저장 단계를 전용 `work_type`으로 둘지, `RULE_SCOPE_GATE` 종료 transaction 또는 `REPORT_DRAFT` 준비 runtime의 추가 output으로 묶을지는 R4가 확정한다([구현 모듈 맵](implementation/01-module-map.md) B2). 어느 경우에도 새 autonomous Agent나 새 `action_type`을 만들지 않는다.
...
Finding의 저장 primitive(전용 `work_type` 사용 여부 또는 Rule Scope 이후 신뢰 runtime finalization의 추가 output), `Finding`/`FindingCandidate` schema 본문, result owner identity, current pointer·revision·CAS·stale enforcement와 `AnalysisRunResult.finding_refs` 연결은 R4 trusted runtime이 기존 immutable revision + atomic current pointer + `TransitionCommit` 계약을 재사용해 확정한다. 이 작업은 새 autonomous Agent role, 새 `action_type` 또는 새 producer enum을 추가하지 않는다. 구체 binding은 [구현 모듈 맵](implementation/01-module-map.md) B2에서 R4·R5가 함께 확정한다. Finding이 stale해지는 upstream 변경(Verification generation/revision, `CWELabel` revision, `TechnicalEvidenceReview`·`RuleScopeImpactReview` 재생성, `DynamicReproductionResult`·validated `poc_ref` 변경, 고정한 `ProgramPolicyRecord` revision 변경)이 생기면 기존 Finding은 감사 이력으로만 남고 새 Reporter 실행에 사용할 수 없다. 새 Verification generation은 이전 generation의 Finding을 재사용하지 않는다.
