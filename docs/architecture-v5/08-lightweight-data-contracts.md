...
`FindingCandidate` 본문과 품질 기준은 R5가 소유한다. R4-03은 활성화되어 이미 저장된 Finding revision만 `finding_refs`로 전달할 뿐 새 Finding claim을 만들거나 빠진 Finding을 추정하지 않는다. 현재 독립 Finding record가 활성화되지 않았거나 존재하지 않으면 `HumanReviewPacket`과 `AnalysisRunResult`의 `finding_refs`는 비어 있을 수 있다. 현재 Finding record가 활성화되지 않은 상태에서 빈 `finding_refs`의 처리,
`blocked_reasons`와의 관계, 두 `finding_refs` 목록의 set-equality 의미는 R4/R8 공통 계약으로 확정한다.
R5-04는 빈 `finding_refs` 자체를 독립적인 Human Review 차단 조건으로 새롭게 확정하지 않으며,
Finding schema 또는 activation rule을 추가하지 않는다. `blocked_reasons`에는 보고서 조건·provenance·restriction·unresolved condition 부족처럼 사람이 보완해야 할 report 차단 사유를 기록한다. packet 자체의 stale/mismatched revision, schema/reference set 또는 redaction 실패는 packet을 만들지 않고 `PREPARE_HUMAN_REVIEW`를 `DENY`한다.
