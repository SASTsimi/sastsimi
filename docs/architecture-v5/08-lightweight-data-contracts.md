...
같은 owner가 새 work에서 보완을 마치면 새 `VerificationResult`와 새 VERIFICATION work의 `SUCCEEDED`, `HypothesisProcessState.status=TERMINAL`, 새 `verification_result_ref`, `verification_work_ref=null`을 한 atomic commit으로 확정한다. 이전 `CWELabel`은 history/provenance로 보존하고 R5-01 `CWE_LABELING` work가 새 root cause·Evidence 정렬을 재평가해 새 `CWELabel` revision을 current provenance로 확정한 뒤에만 새 Technical Gate work를 등록한다. 새 Gate 작업은 달라진 `input_refs`, `input_hash`, `dedupe_key`, `work_id`를 사용하고 첫 `WorkAttempt`는 새 `attempt_id`, `attempt_number=1`, `trigger=INITIAL`을 사용한다. 이전 Gate review와 새 review는 `RecordMeta.previous_record_id`로 이어지는 같은 논리 record의 revision chain에 남긴다. 반대로 provider timeout이나 일시 오류처럼 domain input이 바뀌지 않은 일반 retry는 같은 `work_id`·`dedupe_key`·`input_hash`를 유지하고 새 `attempt_id`와 `trigger=RETRY`를 사용한다. 동일 입력의 새 attempt만 만들어 `REVISE`를 다시 투표하거나 과거 Gate reference를 새 Verification revision에 재사용하는 것은 금지한다.
...
- Technical Gate의 `REVISION`은 action `input_refs`와 call spec context가 final `VerificationResult(verdict=TRUE)`와 `CWELabel`의 정확한 `record_id`·`content_hash`를 direct input으로 가리키고, 두 대상에서 도달 가능한 Gate-relevant transitive closure가 work의 `input_refs`와 일치하는지 검사한다. 이 TRUE의 `dynamic_request_ref`, `dynamic_result_ref`, `poc_ref`는 current Verification generation의 exact request, `SUCCEEDED + SUPPORTED` 결과와 validated PoC를 가리켜야 한다. 각 direct/transitive reference는 대상 존재, exact revision, workspace·commit, hypothesis-scoped artifact의 `hypothesis_id`, `content_hash`를 공통 계약으로 검증한다. Gate 출력의 `TechnicalEvidenceReview.verification_result_ref`와 `cwe_label_ref`도 바로 이 두 direct record를 가리켜야 한다. `GATE_ORDER`는 모든 결과가 현재 work에서 `COMMITTED`됐고 final인지 검사한다. validated PoC가 없는 TRUE, HOLD와 FALSE는 Technical Gate action을 만들 수 없다.
...
### Dynamic request/result/PoC compatibility

final TRUE는 current Verification generation의 exact request, `SUCCEEDED + SUPPORTED` 결과와 실제 실행으로 검증된 `poc_ref`가 모두 필요하다. `LIMITED_REPRO | FULL_REPRO`는 별도 decision이 아니라 R7 `ReproductionPlan.mode`이며, 실행 결과의 `mode`는 exact plan의 mode와 같아야 한다. 실행을 요청하지 않았다면 request·plan·result·validated PoC reference가 모두 없고 final TRUE에는 사용할 수 없다. 실행 중 상태는 final 결과가 아니라 `DynamicReproductionState`로 관리한다.

| `ReproductionPlan.mode` | 결과 status | 구조 계약 | 의미 및 Gate 처리 |
|---|---|---|---|
| `LIMITED_REPRO` | `SUCCEEDED + SUPPORTED` | current generation 결과 ref와 실제 실행으로 검증된 `poc_ref` 필수 | exact log·observation·attempt가 맞을 때만 final TRUE 및 Gate 가능 |
| `LIMITED_REPRO` | `PARTIAL` | 결과 ref 필수. candidate와 validated PoC를 구분하고 같은 attempt로 연결 | `INCONCLUSIVE`와 limitations 필수. final TRUE 및 Gate 금지 |
| `LIMITED_REPRO` | `FAILED`, `BLOCKED`, `CANCELLED` | 결과 ref 필수. 생성되었지만 실행되지 않은 PoC candidate 허용, validated `poc_ref=null` | 실패는 성공·반증 또는 `FALSE | HOLD` 근거가 아니며 final verdict와 Gate를 만들지 않음 |
| `FULL_REPRO` | `SUCCEEDED + SUPPORTED` | current generation 결과 ref와 실제 실행으로 검증된 `poc_ref` 필수 | exact log·observation·attempt가 맞을 때만 final TRUE 및 Gate 가능 |
| `FULL_REPRO` | `PARTIAL` | 결과 ref 필수. candidate와 validated PoC를 구분하고 같은 attempt로 연결 | `INCONCLUSIVE`와 limitations 필수. final TRUE 및 Gate 금지 |
| `FULL_REPRO` | `FAILED`, `BLOCKED`, `CANCELLED` | 결과 ref 필수. 생성되었지만 실행되지 않은 PoC candidate 허용, validated `poc_ref=null` | 실패는 성공·반증 또는 `FALSE | HOLD` 근거가 아니며 final verdict와 Gate를 만들지 않음 |

Dynamic/PoC의 생성·실행·상태·reference·redaction·attempt 정합성은 R7-04와 위 공통 계약을 따른다. 생성되었지만 실행되지 않은 PoC candidate는 존재할 수 있고, validated `poc_ref`와 실행 주장은 Runner가 실제 사용한 command/input 및 같은 execution attempt의 step log·observation과 연결되어야 한다. redaction에 실패한 raw observation·PoC는 일반 Verification → Gate → Reporter evidence closure에 전달하지 않는다. Technical Gate는 이 producer/schema 계약을 다시 정의하지 않고 현재 Verification revision이 가리키는 exact artifact와 claim alignment만 검토한다. 구조 불변조건 위반은 Runtime Validator·schema·semantic validation에서 invalid output으로 차단하며 Gate의 `REVISE | REJECT`를 생성하지 않는다.
