...
AgentLogEvent:
  ...
  actor: DYNAMIC_REPRODUCTION | R7_SETUP_AUTOMATION | TOOL_RUNTIME | SANDBOX_CONTROLLER | REPRODUCTION_SESSION_MANAGER
...
| `ReproductionPlan.environment_requirements_ref` | `environment_requirements` | Dynamic Reproduction Agent | Runtime Validator, R7 Setup Automation | 필요한 애플리케이션 환경의 current exact revision |
...
ModelRoute:
  ...
  required_capabilities: [STRUCTURED_OUTPUT | NEW_SESSION | RESUME_SESSION | PARALLEL | CANCELLATION | USAGE]
...
`ModelProfile`은 role·task별 허용 model route와 명시적 fallback 순서를 고정한다. 각 `ModelRoute.provider_profile_ref`는 R3-04에서 model·environment 단위로 검증한 exact ProviderProfile을 가리키고 route의 `model`도 그 profile과 같아야 한다. registry의 `provider_profile_refs` 집합은 ModelProfile routes의 provider profile 집합과 같아야 한다. `ExecutionLimits`, `LLMRetryPolicy`, `LLMToolPolicy`, `PromptRedactionPolicy`, `OutputSchemaSpec`, `SemanticValidatorSpec`도 모두 exact revision으로 고정한다. `OutputSchemaSpec.result_kind`, registry의 `result_kind`와 result-owner registry 항목은 같아야 한다. 실제 한 호출에서 선택한 profile·model과 limits 값은 `LLMCallSpec`에 다시 고정한다. `allowed_tools=[]`이면 provider tool을 허용하지 않는다.
...
Finding 의미·품질 기준은 R5가 소유하고 canonical 저장 schema·authority는 아래 R4 계약을 따른다. `FindingCandidate`는 별도 domain record로 정의하지 않고, 동일한 `Finding` schema의 immutable staging candidate를 기존 `SAVE_RESULT.candidate_result_ref`로 고정한다. R4는 exact upstream을 조립할 뿐 새 Finding claim을 만들거나 빠진 Finding을 추정하지 않는다. Finding이 없으면 `CREATE_REPORT_DRAFT`를 허용하지 않고 `AnalysisRunResult.report_draft_refs=[]`를 유지한다. 원인은 `REPORT_NOT_READY`와 연결된 오류·상태 기록으로 추적한다.
...
Reporter의 action 승인·provider 호출 직전·draft 저장 시 current index와 exact upstream을 재검사하며 current non-stale Finding만 소비한다. `REPORT_READY` 6축은 별도 검사이고 Finding 저장 eligibility에 적용하지 않는다. `AnalysisRunResult.finding_refs`는 확정 시 각 `CURRENT` index가 가리키는 유효한 Finding의 중복 없는 집합과 set-equal해야 한다. run finalization은 index version과 upstream을 CAS snapshot으로 확인하고 invalidation이 진행 중이면 확정하지 않는다. 확정 후 upstream이 변경되는 경우 과거 run result는 immutable snapshot으로 보존하며 current run result pointer는 invalidation 완료 후 새 `AnalysisRunResult` revision으로 atomic 갱신한다. 과거 Finding을 current 목록에 재사용하지 않는다.
