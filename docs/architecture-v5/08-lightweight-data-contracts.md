...
AnalysisRunInput:
  ...
  production_authority_catalog_ref: RunStoredDataRef | null
```

`AnalysisRunInput`의 `workspace_id`, `commit_id`, `production_profile_ref`,
`production_onboarding_ref`, `production_authority_catalog_ref` 다섯 필드만
레거시 record 읽기 호환을 위해 생략 가능하며 기본값은 `null`이다.
새 `PRODUCTION` 실행은 다섯 필드 모두 non-null로 고정해야 한다.
...
레거시 exact bytes와 hash를 보존하기 위해 위 다섯 필드의 `null` 값만
...
ProductionAuthorityCatalog:
  ...
  role_identities: [ProductionRoleIdentity]

ProductionRoleIdentity:
  role: RequesterRole
  identity_ref: RunStoredDataRef | StoredDataRef
```

이 catalog는 `RunMeta`나 record ID가 없는 immutable configuration artifact이며
result registry에 등록하지 않는다. canonical bytes의 SHA-256이 artifact의 ID와
hash다. 현재 `RequesterRole` 전체를 enum 순서로 정확히 한 번씩 기록하며,
identity reference를 중복하지 않는다. REPOSITORY_LOADER는 execution profile을,
나머지 역할은 같은 analysis·workspace·commit의 서로 다른 ACTIVE PRODUCTION
WorkBudgetProfile을 가리킨다. 공통 work profile을 역할 identity로 재사용하지 않는다.
role mapping은 이름·profile key·한도·action 이력에서 추론하지 않는다.

artifact를 먼저 승격한 뒤, 그 ref를 입력·초기 상태·execution pin과 같은 SQLite
초기화 transaction에서 연결한다. 연결되지 않은 artifact는 권한을 부여하지 않는다.
state에는 catalog ref를 복제하지 않으며 immutable `analysis_input_ref`로만 접근한다.
기존 네 descriptor 필드 입력도 exact bytes/hash를 유지하지만 catalog 없이는 검사에
통과하지 않는다. catalog는 미래의 binding ref를 포함하지 않는다. workspace READY
뒤 state에 고정하는 기존 full binding만 binding identity의 정본이며 그 네 constituent
profile ref가 catalog와 일치해야 한다. binding 전 검사는 상태 변경 없이
`PRODUCTION_AUTHORITY_BINDING_NOT_PINNED`로 종료한다.

재시작 검사는 저장된 exact catalog·profile·승인·current pointer와 scope를 검증한다.
ID·권한 객체·승인 decision·새 binding을 만들거나 credentials를 읽지 않는다.
검사 snapshot은 실행 권한이 아니며 공개 resume은 성공한 검사 뒤에도 exit 4와
`PRODUCTION_RESUME_DISPATCH_NOT_AVAILABLE`을 유지한다.
...
`CodeWorkspace`는 `WORKSPACE_PREP` work가 생산하는 append-only run record다. 상태가 바뀔 때 같은 `logical_record_id`에 새 `record_id`와 증가한 `revision_number`를 만들고 과거 revision을 덮어쓰지 않는다. `AnalysisRunState.workspace_ref`가 현재 revision을 가리키며 `workspace_id`와 `commit_id`는 참조 대상 값과 같아야 한다.

`RepositoryProfile`은 준비가 끝난 정확한 작업공간에서 Git이 추적하는 일반 파일만 읽어 만든 저장소 탐지 결과다.
...
RepositoryProfile:
  ...
  tracked_files: [RepositoryTrackedFile]
  languages: [RepositoryLanguage]
  frameworks: [RepositoryFramework]
  config_files: [RepositoryConfigFile]
  execution_hints: [RepositoryExecutionHint]
  gaps: [RepositoryProfileGap]
  errors: [RepositoryProfileError]
  status: READY | NEEDS_CONFIRMATION
  confirmation_reasons: [string]
```

`tracked_files`는 경로 순으로 정렬하며 경로를 중복하지 않는다. `manifest_hash`는 이 목록 전체의 canonical JSON hash다. 언어·프레임워크의 근거 경로와 설정 파일·실행 힌트의 경로는 모두 이 목록 안에 있어야 한다. `READY`이면 `confirmation_reasons=[]`이고, 확실하지 않은 언어 또는 실행 방법이 있으면 `NEEDS_CONFIRMATION`과 하나 이상의 이유를 기록한다.

`RepositoryExecutionSelection`은 위 profile과 현재 host에서 `ACTIVE`인 정확한 Git·정적 분석 capability revision을 결합한 실행 선택 결과다.
...
RepositoryExecutionSelection:
  ...
  selected_tools: [RepositorySelectedTool]
  gaps: [DataGap]
...
| `collection_result_id` | 정책 수집 결과를 확정하는 runtime | 전체 시스템 | `PolicyCollectionResult` | 결과까지 도달한 `POLICY_FETCH` attempt마다 새 값. 한 attempt에서는 최대 하나 |
...
DynamicReproductionState:
  meta: RecordMeta
  verification_generation: integer
  dynamic_work_ref: StoredDataRef | null
...
`AnalysisStartRequest`가 검증되고 `analysis_id`가 발급되면 runtime은 저장소 입력, 요청 commit, 프로그램, 실행 목적을 credential-free `AnalysisRunInput`으로 한 번 저장한다. 새 `PRODUCTION` 실행은 미리 할당한 `workspace_id`, exact `commit_id`, `production_profile_ref`, `production_onboarding_ref`, `production_authority_catalog_ref`도 이때 함께 고정한다. `AnalysisRunState.analysis_input_ref`는 이 immutable run record의 exact revision을 가리키며 같은 `analysis_id`에서 바꾸지 않는다. `WORKSPACE_PREP` 입력도 이 reference를 포함해야 하므로 프로세스를 다시 시작한 뒤에도 동일 입력만 검사하고 checkout 전 실패도 원래 요청 기준으로 집계할 수 있다. descriptor 검증 성공은 재개 실행 권한이나 checkout 완료의 증거가 아니며, 실제 dispatch는 별도의 복구·현재 상태·권한·예산 검사를 통과해야 한다.
...
`AnalysisRunState.run_policy_state_ref`는 workspace 준비 전에는 `null`이다. `POLICY_FETCH` work를 만들 때 최초 `PREPARING` state의 exact reference를 같은 atomic transition으로 연결한다. 결과까지 도달한 attempt는 자기 `PolicyCollectionResult`와 같은 transition에서 새 `RunPolicyState` revision을 만들 수 있다. 재시도 대기인 `BLOCKED`에서 resume하면 같은 work의 새 attempt와 새 state revision을 사용하고, 이전 collection·state revision은 감사 이력으로 남긴다. `CURRENT | ABSENT | FAILED | UNVERIFIED`로 terminal 준비를 확정한 뒤에는 run 종료까지 정책 내용을 가리키는 reference를 바꾸지 않는다. 분석 종료 시 non-null 값은 `AnalysisRunResult.run_policy_state_ref`와 exact match해야 한다. 이름만 current인 전역 조회나 가설별 복사본으로 대체하지 않는다.
...
| `RUNNING` | `READY`, `BLOCKED`, `SUCCEEDED`, `PARTIAL`, `FAILED`, `CANCELLED` | attempt 결과와 atomic transition이 있음. `READY`는 DYNAMIC_REPRO Agent session 재시작의 `trigger=RETRY`에만 허용 |
...
일반 작업의 재시도 가능한 attempt 실패는 `WorkAttempt.status=FAILED`와 오류를 보존하고 작업을 `BLOCKED`로 전환한다. `DYNAMIC_REPRO`에서는 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정을 현재 attempt의 event로 계속 기록하며 상태 전이나 새 attempt를 만들지 않는다. session 재시작이 필요할 때만 이전 attempt의 결과·로그를 보존한 채 `RUNNING -> READY -> RUNNING`을 atomic하게 확정하고 같은 `work_id`의 새 `attempt_id`, `trigger=RETRY`를 사용한다. 재인증·승인·외부 환경 정비·resource 확보처럼 현재 work의 불변 입력을 바꾸지 않는 외부 조건을 기다릴 때는 `BLOCKED`와 `waiting_for`를 사용하고, 조건 해소 뒤 `BLOCKED -> READY -> RUNNING`으로 같은 work의 새 `attempt_id`, `trigger=RESUME`를 사용한다. 새 attempt는 R8의 호출 전 잔여 시간과 새 attempt 한도가 남아 있을 때만 허용하며 과거 attempt artifact를 current 결과에 섞지 않는다. 재시도할 수 없거나 한도를 소진한 때만 작업을 최종 `FAILED`로 끝낸다. 종료된 작업을 되돌리지 않는다. 사람이 같은 입력으로 새 논리 실행을 명시적으로 승인하면 이전 값보다 1 큰 `work_generation`과 새 `work_id`를 만들고, 두 작업을 restart 관계로 debug trace에 연결한다.
...
`REGISTER_WORK(work_type=CHAINING)` 시 trusted runtime은 같은 analysis·workspace·commit의 current `PrimitiveIndexState` revision을 읽고, 각 index의 `primitive_refs`에서 non-null `result` 또는 하나 이상의 `inputs`를 가진 current exact Primitive만 중복 없이 펼친다. admission은 Primitive 등록 시점의 1회 판정이므로 후보 선정 단계에서 decision을 다시 확인하지 않는다. runtime은 사용한 index revision과 펼친 Primitive exact reference를 함께 고정한다. 이 전체를 `WorkExecutionState.input_refs`, `input_hash`와 `dedupe_key`에 반영하며 Primitive reference 집합이 해당 work의 `considered_primitive_refs`다. 진행 중 새 Primitive가 저장돼 index revision이 올라가도 기존 work 입력에 섞지 않고 새 Chaining work에서 처리한다. index revision이 올라간 것만으로는 기존 work를 무효화하지 않는다. 결과에 해당 work가 고정하지 않은 index·Primitive reference가 섞여도 `STALE_RESULT`로 거절한다. 이는 별도 Snapshot 모듈이 아니라 실행 입력의 exact reference 고정이다.
...
`origin=CHAINING` 가설의 새 Verification·Gate·Primitive update·Reporter work를 등록하거나 그 결과를 저장할 때 trusted runtime은 부모 계보의 admission을 다시 확인하지 않는다. 부모 Primitive는 등록 시점에 자격이 확정됐고 run 안에서 뒤집히지 않기 때문이다. 자식 work는 공통 규칙대로 자기 고정 입력의 exact revision만 검사한다.
...
이 DB transaction에는 `ActionDecision`의 `UNUSED -> USED`, old/new work state, process state, application·질문·child work, 새 dynamic state와 `outcome_refs`를 함께 넣는다. `(analysis_id, hypothesis_id, expected_generation)`에는 successor generation 하나만 허용하고 target generation은 정확히 `expected_generation+1`이다. transaction commit 전 crash는 전부 rollback되어 old generation만 current이고, commit 뒤 crash는 새 generation 전체가 current다. 따라서 중간 상태나 두 current Verification work는 허용하지 않는다. Recovery Runtime은 이미 저장된 exact action과 COMMITTED transaction을 재투영하거나 rollback된 action을 같은 CAS로 다시 실행할 수 있을 뿐, 입력 변경 필요·reason·새 request/profile 의미를 만들지 않는다. 이미 successor가 있으면 기존 결과를 반환하고 또 다른 generation을 만들지 않는다.

old generation의 action·attempt·environment·AgentLog·DynamicReproductionResult·PoC·CWELabel·Gate review는 append-only history에 남지만 새 generation의 input, current pointer, final TRUE, CWE·Gate·Primitive·Reporter에 연결할 수 없다. 늦은 old attempt event/result는 `ATTEMPT_NOT_ACTIVE | STALE_RESULT`, old action 재사용은 `ACTION_NOT_ALLOWED`로 격리한다. 이 전이 전·transaction 중·commit 직후 crash와 동시 중복 요청은 `R3-REC-DYN-010` 및 계약 부정 시험으로 검증한다.
...
| `DYNAMIC_REPRO` | Verification의 `REQUEST_DYNAMIC_REPRO`를 Runtime Validator가 허가 | Dynamic Reproduction Agent·Setup Automation·Sandbox Controller, 결과는 Reproduction Session Manager | current generation의 exact R6 request와 같은 R7 work·attempt의 plan·recipe·환경·AgentLog·PoC가 연결된 `DynamicReproductionResult` |
...
| `POLICY_FETCH` | `CodeWorkspace.status=READY` 뒤 실행 시작 runtime | 비-LLM Policy Collector와 LLM Policy Parser | `RunPolicyState`, `PolicyParserResult`, `PolicyCollectionResult`, `FOUND`이면 `ProgramPolicyRecord` |
| `TECHNICAL_GATE` | Verification runtime | Technical Evidence Gate Agent | `TechnicalEvidenceReview` |
| `RULE_SCOPE_GATE` | Verification runtime | Rule Scope Impact Gate Agent | `RuleScopeImpactReview` |
...
current Finding 저장은 `RULE_SCOPE_GATE`의 `SUCCEEDED`와 review의 `COMMITTED` 이후 실행하는 전용 비-LLM `FINDING_NORMALIZE` work로 확정한다. Step 19에서 Reporter readiness 검사보다 먼저 실행하며 Reporter 차단 여부와 독립적이다. 기존 `REGISTER_WORK`, `START_ATTEMPT`, `SAVE_RESULT`, `CHANGE_WORK_STATE`를 사용하고 §11의 Finding pointer와 함께 commit한다. Rule Scope와 Reporter work의 추가 output으로 묶지 않는다.
...
- `WORKSPACE_PREP`의 각 상태 변경과 `AnalysisRunState.workspace_ref`는 같은 `CodeWorkspace` revision을 가리킨다. `READY` 또는 `FAILED`로 끝나는 attempt의 `WorkAttempt.output_refs`, `WorkExecutionState.output_refs`와 `TransitionCommit.output_refs`도 그 exact revision 하나를 가리킨다. cleanup으로 `REMOVED`를 만들 때는 별도 transition으로 workspace pointer만 새 revision에 옮기고 과거 준비 결과를 덮어쓰지 않는다.
...
- 모든 `STATIC_TOOL` 종료 transition은 정확히 하나의 `ToolRunResult`를 output으로 확정한다. `FAILED`와 `SKIPPED`도 빈 성공으로 숨기지 않고 실제 status·gap·error를 가진 result를 남긴다. 다음 정규화 단계는 COMMITTED work output이 가리키는 exact result만 읽는다.
...
- `HYPOTHESIS_PROPOSAL` work는 source output에서 검증한 proposal 각각의 COMMITTED `HypothesisProposal` reference를 output에 중복 없이 모두 포함한다. proposal 등록과 `ProposalProcessState.proposal_ref`·존재하는 새 `HypothesisProcessState`의 생성은 해당 proposal별 atomic transition으로 확정하며 source Agent가 전역 등록을 직접 확정하지 않는다.
- `VERIFICATION`의 `SUCCEEDED`와 `HypothesisProcessState.status=TERMINAL`은 같은 final `VerificationResult.record_id`를 가리킨다.
- `PRIMITIVE_UPDATE`가 final TRUE를 처리하면 `PrimitiveAdmissionDecision`, 허용된 경우의 모든 `Primitive`, 새 `PrimitiveIndexState`를 같은 `COMMITTED` transition으로 확정한다. `decision=DENY`이면 result Primitive를 만들지 않는다. final HOLD는 `required_primitive_candidates`가 하나 이상일 때만 admission decision 없이 HOLD Primitive와 index revision을 함께 확정한다. 후보가 비어 있으면 `PRIMITIVE_UPDATE`나 Chaining work를 만들지 않는다.
...
- `POLICY_FETCH`의 종료 transition은 parser를 실행했다면 모든 `PolicyParserResult`, 정확히 하나의 `PolicyCollectionResult`, 같은 work를 가리키는 `RunPolicyState`, 그리고 collection이 `FOUND`일 때만 exact `ProgramPolicyRecord`를 함께 가리킨다. `ABSENT_CONFIRMED | COLLECTION_FAILED`에서는 정책 record를 임의 생성하지 않는다.
...
- `FINDING_NORMALIZE`의 `SUCCEEDED`는 정확히 하나의 `Finding.record_id`를 가리킨다. `WorkAttempt`, `WorkExecutionState`, `StateTransition`, `TransitionCommit`의 `output_refs`는 모두 그 하나와 같으며 `PARTIAL` Finding output은 허용하지 않는다.
- `REPORT_DRAFT`의 `SUCCEEDED`와 `ReportProcessState.status=DRAFTED`는 같은 `ReportDraft.record_id`를 가리킨다.
- 분석 종료 transition과 `AnalysisRunState.analysis_result_ref`는 같은 `AnalysisRunResult`를 가리킨다. `analysis_run_result`는 ORCHESTRATION 신뢰 runtime만 `SAVE_RESULT`로 확정하며, 종료 시점의 exact `workspace_ref`, current 결과 집합, 완료한 work·attempt·transition·오류·gap·자원 reference를 입력으로 고정한다. `AnalysisRunResult`, 종료 `AnalysisRunState`와 `analysis_result_ref`는 같은 `TransitionCommit`에서 함께 COMMITTED되고 별도 current pointer는 만들지 않는다.
...
ActionRequest:
  meta: RunMeta | RecordMeta
  action_id: string
  requested_by: ORCHESTRATION | HYPOTHESIS | PRO | CON | VERIFICATION | CWE_LABELING | CHAINING | TECHNICAL_GATE | RULE_SCOPE_GATE | REPORTER | REPOSITORY_LOADER | STATIC_ANALYSIS | POLICY_COLLECTOR | POLICY_PARSER | PRIMITIVE_ADMISSION_RUNTIME | DYNAMIC_REPRODUCTION | REPRODUCTION_SETUP_AUTOMATION | SANDBOX_CONTROLLER | REPRODUCTION_SESSION_MANAGER | BUDGET_RUNTIME | R8_EVALUATION_RUNTIME | RECOVERY
  requester_identity_ref: RunStoredDataRef | StoredDataRef
  action_type: REGISTER_WORK | CHANGE_WORK_STATE | START_ATTEMPT | CANCEL_WORK | RESTART_VERIFICATION_GENERATION | READ_CODE | RUN_TOOL | CALL_LLM | FETCH_POLICY | REQUEST_DYNAMIC_REPRO | RUN_SANDBOX | SAVE_RESULT | CALL_TECHNICAL_GATE | CALL_RULE_SCOPE_GATE | CREATE_REPORT_DRAFT
  ...
  resource_profile_ref: StoredDataRef | null
  run_policy_state_ref: StoredDataRef | null
...
| `RESTART_VERIFICATION_GENERATION` | SCHEMA, AUTHORITY, IDENTITY, REVISION, STATE, BUDGET | `VERIFYING` 중 exact dynamic request/profile 변경만 허용하고 old Verification·dynamic 종료와 새 generation·application·질문·Pro/Con·current pointer를 한 transaction으로 확정 |
...
| `FETCH_POLICY` | SCHEMA, AUTHORITY, REVISION, STATE, BUDGET, TOOL, REDACTION | 분석 단위 policy work에서 승인된 공식 source만 원문 후보로 저장 |
...
| `RUN_SANDBOX` | SCHEMA, AUTHORITY, IDENTITY, REVISION, STATE, BUDGET | 같은 R7 attempt에서 image build와 container 실행을 각각 별도 action으로 허가한다. 두 action 모두 current work, exact request·requirements·plan·profile을 고정하며 실행 당시 `RunPolicyState`는 감사 reference로 별도 기록한다. |
...
| `CALL_RULE_SCOPE_GATE` | SCHEMA, AUTHORITY, IDENTITY, REVISION, STATE, BUDGET, PROVIDER, SESSION, GATE_ORDER, REDACTION | `TRUE`+Technical `ACCEPT`, current `RunPolicyState`, exact 정책 수집 결과와 `FOUND`일 때 state가 가리키는 정책 record, exact LLM call spec 필요 |
| `CREATE_REPORT_DRAFT` | SCHEMA, AUTHORITY, IDENTITY, REVISION, STATE, BUDGET, PROVIDER, SESSION, REPORT_READY, REDACTION | current non-stale Finding, Reporter 6축 readiness 전부 충족, exact LLM call spec 필요 |

Gate와 Reporter action의 기존 check는 다음 exact revision을 검사한다. 검사는 `ActionDecision`을 만들 때와 실제 provider 호출 직전에 같은 기준으로 다시 수행한다.

`REQUEST_DYNAMIC_REPRO`의 ALLOW는 R6 요청을 R7의 한 `DYNAMIC_REPRO` work로 등록할 수 있다는 뜻이다. Runtime Validator는 `(analysis_id, hypothesis_id, verification_generation, work_type=DYNAMIC_REPRO)` unique key를 강제하고 같은 generation에서 purpose가 다른 두 번째 request나 work를 거절한다. `RUN_SANDBOX`의 `ActionDecision=ALLOW`는 Docker 실행 성공이나 Sandbox profile 외부 경계 통과를 뜻하지 않는다. Runtime Validator는 Reproduction Setup Automation 호출자의 권한, 현재 work·attempt와 예산, 변경되지 않은 exact `DynamicReproductionRequest`·current `EnvironmentRequirements`·current exact `ReproductionPlan`·`sandbox_profile_ref`·exact `DynamicReproductionLifecycleProfile`을 검사하고 그 exact revision을 action `input_refs`와 `resource_profile_ref`에 고정한다. `ActionDecision.checked_config_refs`에는 두 profile exact revision이 모두 있어야 한다. `ActionRequest.run_policy_state_ref`에는 요청 당시 조회한 exact state를 기록하지만, 이 값은 프로그램 정책의 최신성을 이용해 Sandbox 권한을 넓히는 설정이 아니라 감사용 provenance다. 따라서 policy state는 `input_refs`나 `checked_config_refs`에 중복해 넣지 않으며, current policy pointer 또는 freshness만 바뀌었다는 이유로 로컬 전용 `RUN_SANDBOX` decision을 만료시키지 않는다. 반면 current `ReproductionPlan`이나 두 profile revision, 실행 대상·network·mount·secret 같은 격리 경계 입력이 바뀌면 실행 전 기존 `UNUSED` decision을 `EXPIRED`로 확정하고 새 `action_id`·action·decision을 만들어야 한다. 이후 Sandbox Controller는 host·Docker daemon/socket·mount/namespace·secret·egress·다른 workspace 같은 외부 격리 경계만 검사해 exact Sandbox 경계 판정을 저장한다. Controller는 Agent가 Sandbox 내부에서 고른 command·package·PoC를 allowlist로 검사하지 않는다. Sandbox 경계 거절은 `policy_decision_ref`와 append-only `agent_log_ref`가 필수인 `DynamicReproductionResult(status=BLOCKED | FAILED, failure_category=POLICY_BLOCKED, agent_invoked=false, poc_ref=null)`로 기록한다. 여기서 `POLICY_BLOCKED`는 프로그램 정책 freshness가 아니라 Sandbox profile의 외부 경계 위반을 뜻한다. R7이 retry에서 새 requirements·plan·recipe·candidate revision을 만들더라도 R6 request의 목적·가설·profile을 바꾸거나 외부 경계 검사를 생략할 수 없다.

최초 Sandbox 준비에는 같은 `attempt_id` 안에서 서로 다른 `action_id`를 가진 `RUN_SANDBOX` 두 개가 필요하다. build action은 파일만 읽는 preflight가 만든 exact `recipe_source_ref`를 고정하고 `image_digest=null`이어야 한다. 이 decision을 부작용 직전에 `USED`로 claim한 뒤에만 Docker image inspect/build를 호출한다. build 결과로 actual `sha256` digest와 immutable `EnvironmentRecipe`가 저장되면, run action이 그 exact recipe·digest와 build action/decision provenance를 입력으로 고정한다. 별도의 run decision을 `USED`로 claim하고 Sandbox Controller가 다시 허용한 뒤에만 container create/start를 호출한다. 아직 만들지 않은 image digest를 예상하거나 tag를 최종 승인값으로 사용하지 않는다. 재생성도 current recipe·실제 digest·이전 환경·재생성 사유를 고정한 새 run action과 decision을 요구한다. 두 단계는 권한 의미가 다르므로 하나의 decision을 재사용하지 않으며 새 `ActionType`을 만들지는 않는다.
...
- Rule Scope Gate의 `REVISION`은 같은 Verification·current CWELabel revision, `RuleScopeImpactReview.technical_review_ref`가 가리킬 exact `TechnicalEvidenceReview`, 준비 완료 때 고정한 `RunPolicyState`, 그 state가 가리키는 exact `PolicyCollectionResult`와 `FOUND`일 때 exact `ProgramPolicyRecord`를 검사한다. Gate 호출은 새 collection/parsing을 실행하지 않는다. `GATE_ORDER`는 Technical review가 그 두 revision을 검토한 `ACCEPT`이고 Verification verdict가 `TRUE`이며 정책 수집 결과가 `COLLECTION_FAILED`가 아닌지 검사한다. Gate 실행 순서(Verification TRUE → Technical `ACCEPT` → Rule Scope Gate → Finding/Reporter)는 정책 준비 시점 변경과 무관하게 유지한다. state가 `UNVERIFIED`이면 호출 결과는 구조적으로 `UNCERTAIN + DENY`만 허용한다. 준비 때 재사용이 거절된 `STALE` 정책 record는 current state에 연결하거나 Gate 입력으로 사용할 수 없다.
...
- LLM 출력에서 만드는 candidate는 성공한 같은 work·attempt의 `LLMInvocationResult`, `LLMInvocationLog`와 두 record가 공통으로 가리키는 schema·semantic 검증 완료 canonical JSON artifact를 `SAVE_RESULT.input_refs`에 포함해야 한다. 이 artifact는 `data_kind=artifact`, `record_id=null`이며 Agent가 반환한 내용만 보존하는 비신뢰 원본이다. T10부터 사용하는 역할별 trusted domain finalizer가 exact artifact만 읽어 `RecordMeta`, `logical_record_id`, `record_id`와 계약상 runtime 소유 ID를 새로 발급하고 domain candidate를 만든다. finalizer는 기존 역할 runtime 안의 비-LLM 권한 경계이지 새 Agent나 새 pipeline 단계가 아니다. Runtime Validator는 artifact의 내용과 candidate의 Agent 소유 필드가 일치하는지, runtime 소유 필드가 신뢰 경로에서 발급됐는지, 호출·work·attempt·role·result kind가 같은지 확인한 뒤에만 `SAVE_RESULT`를 허용한다. `parsed_output_ref`를 domain record reference로 바꾸거나 Agent 출력의 ID·`meta`를 복사하지 않는다.
- 핵심 registry 항목은 `analysis_run_state -> AnalysisRunState -> ORCHESTRATION`, `hypothesis_process_state -> HypothesisProcessState -> ORCHESTRATION`, `verification_assignment -> VerificationAssignment -> ORCHESTRATION`, `code_workspace -> CodeWorkspace -> REPOSITORY_LOADER`, `repository_profile -> RepositoryProfile -> STATIC_ANALYSIS`, `repository_execution_selection -> RepositoryExecutionSelection -> STATIC_ANALYSIS`, `tool_run_result -> ToolRunResult -> STATIC_ANALYSIS`, `static_fact_bundle -> StaticFactBundle -> STATIC_ANALYSIS`, `rule_execution_record -> RuleExecutionRecord -> STATIC_ANALYSIS`, `hypothesis_proposal -> HypothesisProposal -> ORCHESTRATION`, `hypothesis_duplicate_review -> HypothesisDuplicateReview -> HYPOTHESIS`, `pro_evidence_result -> EvidenceAgentResult(role=PRO) -> PRO`, `con_evidence_result -> EvidenceAgentResult(role=CON) -> CON`, `verification_result -> VerificationResult -> VERIFICATION`, `primitive_admission_decision -> PrimitiveAdmissionDecision -> PRIMITIVE_ADMISSION_RUNTIME`, `primitive -> Primitive -> PRIMITIVE_ADMISSION_RUNTIME`, `chaining_result -> ChainingResult -> CHAINING`, `dynamic_reproduction_request -> DynamicReproductionRequest -> VERIFICATION`, `environment_requirements -> EnvironmentRequirements -> DYNAMIC_REPRODUCTION`, `reproduction_plan -> ReproductionPlan -> DYNAMIC_REPRODUCTION`, `environment_recipe -> EnvironmentRecipe -> REPRODUCTION_SETUP_AUTOMATION`, `sandbox_environment -> SandboxEnvironment -> REPRODUCTION_SETUP_AUTOMATION`, `cleanup_result -> CleanupResult -> REPRODUCTION_SETUP_AUTOMATION`, `sandbox_policy_decision -> SandboxPolicyDecision -> SANDBOX_CONTROLLER`, `sandbox_command_record -> SandboxCommandRecord -> REPRODUCTION_SESSION_MANAGER`, `poc_candidate -> PoCCandidate -> DYNAMIC_REPRODUCTION`, `dynamic_reproduction_conclusion -> DynamicReproductionConclusion -> DYNAMIC_REPRODUCTION`, `agent_log -> AgentLog -> REPRODUCTION_SESSION_MANAGER`, `poc_bundle -> PoCBundle -> REPRODUCTION_SESSION_MANAGER`, `dynamic_reproduction_result -> DynamicReproductionResult -> REPRODUCTION_SESSION_MANAGER`, `cwe_label -> CWELabel -> CWE_LABELING`, `run_policy_state -> RunPolicyState -> POLICY_COLLECTOR`, `policy_cache_record -> PolicyCacheRecord -> POLICY_COLLECTOR`, `policy_parser_result -> PolicyParserResult -> POLICY_PARSER`, `policy_collection_result -> PolicyCollectionResult -> POLICY_COLLECTOR`, `program_policy_record -> ProgramPolicyRecord -> POLICY_COLLECTOR`, `technical_evidence_review -> TechnicalEvidenceReview -> TECHNICAL_GATE`, `rule_scope_impact_review -> RuleScopeImpactReview -> RULE_SCOPE_GATE`, `finding -> Finding -> VERIFICATION`, `report_draft -> ReportDraft -> REPORTER`, `budget_reservation -> BudgetReservation -> BUDGET_RUNTIME`, `budget_ledger_entry -> BudgetLedgerEntry -> BUDGET_RUNTIME`, `evaluation_run_result -> EvaluationRunResult -> R8_EVALUATION_RUNTIME`, `evaluation_recommendation -> EvaluationRecommendation -> R8_EVALUATION_RUNTIME`, `analysis_run_result -> AnalysisRunResult -> ORCHESTRATION`이다. 앞 값은 `result_kind`·`data_kind`, 가운데 값은 검사할 schema, 뒤 값은 유일한 생산 역할이다. `ORCHESTRATION`, `BUDGET_RUNTIME`, `R8_EVALUATION_RUNTIME`은 LLM Agent가 아니라 각각 등록·집계, 예산 예약·장부, 평가 집계·제안을 담당하는 신뢰 runtime identity다. 다른 result kind도 versioned result-owner registry에 정확히 한 schema와 생산 역할을 등록해야 하며, broad requester 표만으로 저장 권한을 얻지 않는다.
- `result_kind=code_workspace`이면 REPOSITORY_LOADER만 저장할 수 있다. candidate의 `meta.analysis_id`, `workspace_id`, repository URL과 current `WORKSPACE_PREP` work·attempt가 일치해야 한다. `PREPARING`에서 `READY | FAILED`로 바꿀 때 같은 logical record의 새 revision을 만들고, `READY`에는 checkout으로 검증한 non-null `commit_id`가 필수다. 저장과 `AnalysisRunState.workspace_ref`, `workspace_id`, 존재하는 `commit_id`, `WORKSPACE_PREP` output·상태는 같은 `TransitionCommit`으로 확정한다. cleanup의 `REMOVED` revision도 같은 ID 연결을 보존하며 과거 `READY` revision을 수정하지 않는다.
- 핵심 registry에 `proposal_process_state -> ProposalProcessState -> ORCHESTRATION`을 추가한다. 이 상태는 Agent 출력이 아니라 trusted proposal 검증·등록 runtime이 exact proposal과 같은 atomic projection으로 저장하는 비-LLM 제어 결과다.
- `result_kind=tool_run_result`이면 STATIC_ANALYSIS만 저장할 수 있다. candidate의 `meta.analysis_id`, `meta.workspace_id`, `meta.commit_id`, `meta.attempt_id`, 도구 이름·버전이 current `STATIC_TOOL` work·attempt·tool profile과 일치해야 한다. 규칙 기반 결과의 `rule_execution_ref`와 raw artifact reference, 상태·gap·error 조합을 함께 검사하며 같은 `STATIC_TOOL` 종료 transition에서 output으로 확정한다. retry는 새 attempt와 새 record를 만들며 도구별 current 결과는 work의 COMMITTED output이 정한다.
- R3-05의 중간 제어 출력은 `verification_initial_assessment -> VerificationInitialAssessment -> VERIFICATION`, `dynamic_reproduction_tool_request -> DynamicReproductionToolRequest -> DYNAMIC_REPRODUCTION`으로 등록한다. 전자는 Pro·Con 뒤의 다음 경로만 고르고, 후자는 승인된 Sandbox 안에서 수행할 다음 한 작업만 제안한다. 둘 다 final 취약점 판정이나 실행 사실이 아니다.
...
- `result_kind=hypothesis_proposal`은 schema-valid proposal을 전역 등록하는 비-LLM ORCHESTRATION runtime만 저장한다. `origin=INITIAL`에서는 성공한 `HYPOTHESIS / GENERATE_INITIAL` 호출의 schema·semantic 검증 완료 canonical JSON artifact 배열을 T10 trusted proposal finalizer가 순서대로 읽고 `proposal_id`, 질문별 `question_id`, 검증 항목별 `validation_id`와 `RecordMeta`를 발급해 staging `HypothesisProposal`을 만든다. Agent가 ID나 `meta`를 반환하면 호출은 `INVALID_OUTPUT`이며 proposal을 만들지 않는다. `origin=VERIFICATION`은 current Verification output의 `material_child_proposals` 원소, `origin=CHAINING`은 `ChainingResult.chained_hypothesis_proposals` 원소여야 한다. ORCHESTRATION 등록 runtime은 INITIAL이면 exact source artifact·성공 invocation·`StaticFactBundle`, 나머지 origin이면 COMMITTED source result와 transitive provenance를 `SAVE_RESULT.input_refs`에 고정한다. source 역할·origin이 다르거나 배열 원소의 누락·추가·순서 변경, source parent에 없는 proposal이면 저장하지 않는다. VERIFICATION·CHAINING source에 이미 runtime이 발급한 proposal ID가 있으면 이를 그대로 사용하고 다시 발급하거나 Agent 문장을 수정하지 않는다. source work의 output과 proposal 등록은 서로 다른 atomic transition이며, source 결과가 필요한 origin에서는 그 결과가 COMMITTED되기 전 proposal을 등록하지 않는다. 등록 뒤 `ProposalProcessState.proposal_ref`가 exact proposal revision을 가리키며 다른 attempt 혼합·nested proposal의 부모 계보 유실을 허용하지 않는다.
...
- `result_kind=verification_initial_assessment`이면 VERIFICATION만 저장할 수 있다. candidate의 work·generation·hypothesis·policy·playbook·application·Pro·Con reference와 `llm_call_id`는 current Verification work에 고정된 exact 입력과 성공한 `ASSESS_INITIAL` 호출에 모두 일치해야 한다. `next_step=POC_CONFIRMATION`은 `proposed_verdict=TRUE`, `VERDICT_EVIDENCE`는 `HOLD`, `FINALIZE_WITHOUT_DYNAMIC`은 `FALSE | HOLD`만 허용한다. assessment는 다음 task 선택을 위한 중간 record이며 runtime은 자연어 근거를 다시 판정하지 않는다. initial assessment를 final `VerificationResult`나 Gate 입력으로 사용하지 않는다.
- `result_kind=dynamic_reproduction_tool_request`이면 DYNAMIC_REPRODUCTION만 저장할 수 있다. current `DYNAMIC_REPRO` work·attempt의 exact request·plan·READY environment와 Sandbox 경계를 허용한 USED decision이 필수이며, 성공한 `EXECUTE_REPRODUCTION` 호출의 input과 `llm_call_id`가 같아야 한다. `RUN_COMMAND`는 non-null `command`만, `USE_POC_CANDIDATE`는 같은 attempt의 non-null `poc_candidate_ref`만, `REQUEST_SANDBOX_RECREATE`는 non-null `recreate_reason`만 요구하고 `FINISH`에는 세 payload가 모두 `null`이어야 한다. 다른 가설·attempt·environment, host·Docker daemon/socket·다른 workspace 경로를 대상으로 하는 요청은 저장·실행하지 않는다.
- `result_kind=verification_result`이면 `SCHEMA | REVISION | STATE`는 가설의 모든 `ValidationCheck.validation_id`와 candidate의 `ValidationCheckResult.validation_id`가 중복 없이 set-equal인지, 모든 `ValidationCheckResult.completion=COMPLETE`인지, 각 결과의 `evidence_refs`가 하나 이상인지 확인한다. `INCOMPLETE` 항목이 하나라도 있으면 final candidate를 `COMMITTED`하지 않는다. `falsification_results[].question_id`는 exact 가설 질문과 current `PlaybookApplication` 질문 ID의 합집합과 중복 없이 set-equal하고 각각 정확히 한 번 처리되어야 하며 사용한 근거는 현재 `workspace_id + commit_id`의 저장 record여야 한다. 운영 분석은 독립 Pro/Con work가 모두 정상 종료되어 exact output이 action `input_refs`에 있어야 한다. final 합성 호출은 같은 work·generation의 exact `VerificationInitialAssessment`도 입력으로 사용해야 한다. assessment가 `FINALIZE_WITHOUT_DYNAMIC`이면 candidate verdict는 `FALSE | HOLD`이고 dynamic reference는 모두 `null`이어야 하며, 다른 두 경로이면 assessment에 맞는 exact dynamic request·result를 사용해야 한다. assessment 자체를 candidate evidence나 Gate input에 넣지 않는다. 일부 Context 조회 오류가 있어도 제한 retry·대체 조회·다른 정상 근거로 이 조건을 완료했다면 final candidate를 검사할 수 있지만, 필수 Context 또는 운영 Pro/Con을 확보하지 못했다면 final candidate를 `COMMITTED`하지 않는다. 이 경우 retry 가능 work는 `BLOCKED`로 두고 가설은 `VERIFYING`을 유지한다. 더 시도할 수 없으면 같은 atomic transition에서 work와 `HypothesisProcessState`를 `FAILED`로 끝내며 runtime이 `HOLD`를 대신 만들지 않는다. Runtime Validator는 구조·reference·완료 상태만 검사한다. final `TRUE` 근거의 의미적 충분성과 코드·실행 근거 연결은 Technical Evidence Gate가 exact final TRUE revision을 대상으로 별도로 검토한다. `FALSE | HOLD`는 Technical Gate 입력이 아니며, 구조 검사를 통과했다는 사실이 Gate 승인을 의미하지 않는다.
...
- `result_kind=report_draft`이면 REPORTER만 저장할 수 있고 candidate의 `action_decision_ref`는 같은 초안을 허용한 exact `CREATE_REPORT_DRAFT` decision을 가리켜야 한다. `finding_ref`, Verification·CWELabel·두 Gate·정책, 존재하는 동적 결과·PoC reference는 action `input_refs`와 current upstream record에 정확히 일치해야 한다. `content_ref` 본문의 모든 코드 `path:line`은 exact Verification의 `EvidenceClaim.code_locations` 중 같은 workspace·commit·file·line이어야 한다. `restrictions`와 `unresolved_conditions`는 Verification의 값을 빠짐없이 보존하고 `limitations`는 연결된 동적 결과와 Gate가 남긴 제한을 빠뜨리지 않는다. 위치 불일치나 `redaction_status=PASSED`·action `REDACTION=PASS` 중 하나라도 확인되지 않으면 저장을 거절한다.
...
- `considered_primitive_refs`, `input_primitive_refs`, `source_result_refs`와 candidate별 `parent_hypothesis_ids`, `parent_verification_refs`는 각각 중복이 없어야 한다. `source_result_refs`는 `input_primitive_refs`가 가리키는 Primitive들의 `source_verification_ref`와 non-null `technical_review_ref` 합집합과 set-equal하고 모두 같은 `SAVE_RESULT.input_refs`에 포함되어야 한다. 각 candidate의 `parent_hypothesis_ids`는 그 upstream/downstream Primitive의 `source_hypothesis_id` 합집합, `parent_verification_refs`는 두 Primitive의 `source_verification_ref` 합집합과 각각 set-equal해야 한다. 누락·추가·다른 work의 부모 reference는 저장하지 않는다.
...
- `result_kind=environment_requirements | reproduction_plan | poc_candidate | dynamic_reproduction_tool_request | dynamic_reproduction_conclusion`이면 DYNAMIC_REPRODUCTION만 저장할 수 있다. 한 LLM 호출은 이 중 한 result kind의 schema·semantic 검증 완료 canonical JSON artifact 하나만 만든다. 역할별 trusted domain finalizer가 같은 호출·work·attempt의 artifact에서 해당 domain candidate를 만들고, artifact·invocation result·log를 `SAVE_RESULT.input_refs`에 함께 고정한다. requirements는 R6 request의 모든 `environment_needs`를 빠뜨리거나 약화하지 않고, plan의 request·purpose·hypothesis·profile은 request와 exact match하며 current requirements를 가리킨다. plan에는 mode·exact command·step·payload·cleanup allowlist를 넣지 않는다. candidate의 `llm_call_id`는 current-generation exact R6 request, 현재 R7 work·attempt의 plan·READY environment, request의 `code_refs`가 가리키는 exact `CodeContextResponse`, 각 response의 `code_fragment_refs`가 가리키는 검증·redaction 완료 artifact를 입력으로 사용한 성공한 `CREATE_POC_CANDIDATE` 호출과 같아야 한다. tool request는 current-generation exact R6 request와 같은 R7 attempt의 plan·READY 환경·현재 AgentLog를 사용한 `EXECUTE_REPRODUCTION` 호출에서만 만든다. conclusion의 `llm_call_id`는 current-generation exact R6 request와 같은 R7 attempt의 plan·환경·candidate·관찰을 사용한 성공한 `INTERPRET_ATTEMPT` 호출과 같아야 한다. candidate를 처음 저장할 때는 current-generation exact R6 request, 현재 R7 work·attempt의 plan·environment, 성공 호출, exact code response·fragment reference와 source artifact content digest를 검사하며 아직 뒤따를 AgentLog event를 요구하지 않는다. code response는 같은 analysis·workspace·commit·hypothesis여야 하지만 앞 단계의 context 수집 attempt일 수 있고, plan·environment만 current R7 attempt와 같아야 한다. reference 불일치, attempt 혼합 또는 artifact 저장소 읽기·digest 검증 실패는 provider 호출 전에 거절하며 PoC candidate나 동적 결과를 저장하지 않는다. conclusion은 `TRUE | FALSE | HOLD`를 출력하지 않는다. candidate·tool request·conclusion 존재만으로 실행이나 성공을 인정하지 않고, `DynamicReproductionResult`와 validated PoC를 저장할 때 same-attempt `AgentLog`의 작성·실행 event가 exact candidate revision·digest·tool request·관찰을 가리키는지 검사한다.
- `result_kind=environment_recipe | sandbox_environment | cleanup_result`이면 REPRODUCTION_SETUP_AUTOMATION만 저장할 수 있다. recipe는 저장소 선언 의존성 source, 서로 구분된 base/built digest, build/reuse 결정을 기록한다. environment는 current generation의 exact R6 request와 같은 R7 work·attempt의 plan·recipe·requirements, container instance·생성/재사용 사유를 가리킨다. cleanup은 실제 생성 자원과 환경을 빠짐없이 가리킨다. 다른 가설의 writable container 공유와 근거 없는 reuse는 거절한다.
...
구현의 저장 port에서는 `RecordRef = RunStoredDataRef | StoredDataRef | PolicyCacheRef` type alias를 사용한다. 이 공통 type은 함수 signature를 단순화할 뿐 domain별 허용 범위를 넓히지 않는다. `CodeWorkspace`·`AnalysisRunResult` 같은 run-level record는 `RunStoredDataRef`, 코드·가설·검증·동적·Gate·보고 record는 `StoredDataRef`, 실행 간 공식 정책 cache는 `PolicyCacheRef`만 허용한다. reference 종류와 target `data_kind`가 맞지 않으면 저장소 I/O 전에 `RECORD_REVISION_MISMATCH`로 거절하며 latest lookup이나 다른 reference 형식으로 보정하지 않는다.
...
`hit_count`는 정규화 전 raw 도구 결과에서 해당 규칙이 만든 결과 수다. `CodeFact` 수와 같다고 추정하거나 normalizer의 중복 제거 때문에 값을 바꾸지 않는다. `CodeFact.producer.attempt_id`는 이 사실을 만든 exact `ToolRunResult.meta.attempt_id`와 같아야 한다. `CodeFact.producer.rule_id`가 존재하면 같은 attempt의 `RuleExecutionItem`은 반드시 `SELECTED + EXECUTED`, `hit_count>0`이어야 한다. 반대로 `CodeFact`가 없다는 사실만으로 0건을 추정하지 않고 exact `RuleExecutionRecord`를 확인한다.
...
| `POLICY` | 정책 준비 계층 | `POLICY_SOURCE_MISSING`, `POLICY_SOURCE_STALE` | Policy preparation runtime, 결과 집계, R8 평가 |

`code`는 정확한 누락 종류, `reason`은 공통 원인 범주다. 빈 결과를 숨기지 않으며 소비자는 gap을 안전함이나 반증으로 해석하지 않는다. `POLICY_SOURCE_STALE`은 새 run의 cache 거절·재수집 사유로만 사용하고 Rule Scope Gate나 Reporter에 직접 전달하지 않는다. Gate와 Reporter는 새 run에서 준비해 고정한 `RunPolicyState`와 그 exact collection·policy record만 읽는다.
...
`AnalysisRunState.execution_budget_profile_ref`·`budget_binding_ref`, `ActionRequest.generation_restart_reason`·`generation_restart_basis_refs`, `WorkBudgetProfile`, `BudgetProfileBinding.work_budget_profile_ref`·`approval_ref`는 실행 허용 조건과 저장 의미를 바꾸는 필수 계약이므로 새 MAJOR schema와 migration으로 적용한다. 과거 record에 당시 사용하지 않은 예산 profile·binding 승인 근거나 동적 입력 변경 근거를 current 값으로 추정해 채우지 않는다. migration 뒤에도 exact provenance를 입증하지 못하는 과거 실행은 새 work의 current budget·generation restart 입력으로 사용할 수 없다.
...
`origin=CHAINING` proposal은 Chaining 입력 경계에 `CodeFact`가 없으므로 `observed_facts=[]`만 허용한다. 부모에서 물려받은 restriction과 assumption은 위 필드에 보존하고, 코드 사실은 Context Retrieval Service가 `CONTEXT_RETRIEVAL` work에 고정된 exact proposal의 `source_primitive_match_id`를 따라 부모 Primitive의 `result.entity_refs`와 `inputs[].entity_refs`에서 검증 시작점을 복구한 뒤 현재 workspace·commit에서 조회한다. 자식 Verification은 반환된 `CodeContextResponse`를 사용한다. `target_entities`·`target_locations`·`suspected_path`는 비어 있을 수 있으며 채운 값은 이 계보에서 exact하게 복원되는 범위를 벗어나면 안 된다. 계보가 끊겼거나 Verification 시작점을 하나도 얻지 못하면 proposal을 등록하지 않는다.
...
VerificationInitialAssessment:
  ...
  next_step: POC_CONFIRMATION | VERDICT_EVIDENCE | FINALIZE_WITHOUT_DYNAMIC
  proposed_verdict: TRUE | FALSE | HOLD
  rationale: string
  evidence_refs: [StoredDataRef]
...
Pro·Con이 모두 끝나면 Verification Agent는 먼저 `ASSESS_INITIAL`을 실행해 `VerificationInitialAssessment` 하나를 만든다. 이 호출은 current Verification work·generation에 고정된 exact `PlaybookPolicy`, `VerificationPlaybook`, `PlaybookApplication`, Pro·Con과 정적·Context 근거를 모두 입력으로 사용한다. `POC_CONFIRMATION`은 initial TRUE의 PoC 확인, `VERDICT_EVIDENCE`는 동적 근거가 더 필요한 initial HOLD, `FINALIZE_WITHOUT_DYNAMIC`은 정적·Pro·Con만으로 final `FALSE | HOLD` 합성을 진행하는 경로다. final TRUE는 이 단계에서 만들 수 없다.

Runtime은 assessment의 구조와 exact reference만 검사하고 근거 의미를 대신 판단하지 않는다. `POC_CONFIRMATION | VERDICT_EVIDENCE`이면 같은 assessment를 exact 입력으로 사용해 `CREATE_DYNAMIC_REQUEST`를 호출하고, `FINALIZE_WITHOUT_DYNAMIC`이면 동적 work를 만들지 않고 같은 assessment를 exact 입력으로 `FINAL_VERDICT`를 호출한다. assessment record는 호출 분기와 감사 이력에만 사용하며 `VerificationResult`의 evidence나 Technical Gate·Rule Scope Gate의 직접 입력에 넣지 않는다. 다른 work·generation·application의 assessment는 `STALE_RESULT`로 거절한다.
...
`EvidenceAgentResult`는 수정할 수 없는 역할별 결과 record다. `role=PRO`이면 `evidence`의 모든 `source_role`은 `PRO`, `role=CON`이면 모두 `CON`이어야 한다. 한 결과 안의 `claim_id`는 중복될 수 없고 `summary`는 비어 있을 수 없다. 찾은 근거가 없다면 빈 `evidence`와 그 사실·확인 범위를 설명하는 `summary`·`limitations`를 저장하며 근거를 만들어 내지 않는다. `parent_work_id`는 현재 generation의 부모 `VERIFICATION.work_id`, `evidence_work_id`는 결과를 확정한 역할별 자식 work ID, `verification_generation`은 부모가 속한 현재 가설 generation, `meta.attempt_id`는 그 자식의 성공 attempt ID와 같아야 한다. `llm_call_id`는 결과를 만든 성공 호출 하나와 같아야 한다. 자식 work의 일반 `input_hash`는 역할별 template까지 포함하므로 Pro와 Con이 다를 수 있지만, 두 결과의 `debate_input_hash`는 반드시 같다. 해당 `LLMInvocationResult.parsed_output_ref`와 `LLMInvocationLog.parsed_output_ref`는 같은 schema·semantic 검증 완료 JSON artifact를 가리킨다. T10 trusted domain finalizer는 그 exact artifact에 runtime 소유 ID와 `RecordMeta`를 붙여 `EvidenceAgentResult` candidate를 만들고, source artifact·invocation result·log를 포함한 `SAVE_RESULT` 뒤 자식 work의 `output_refs`가 COMMITTED result revision을 가리키게 한다. domain result가 invocation record나 종료 work revision을 역참조하지 않으므로 content hash 순환을 만들지 않는다.
...
체이닝 재료 자격의 최종 authority는 R4 `PRIMITIVE_ADMISSION_RUNTIME`이 만든 current `PrimitiveAdmissionDecision`이다. Rule Scope의 `review_status`와 `report_permission`의 `FAIL | UNCERTAIN | DENY`는 Reporter를 막지만, COMMITTED `RuleScopeImpactReview`가 존재하면 current Finding은 보존한다. 또한 current admission이 `ALLOW`인 실제 코드 능력의 Primitive와 Chaining도 막지 않는다. `testing_restriction_compliance=FAIL`만 이 정책 축에서 `DENY`가 되며, `UNCERTAIN`은 `ALLOW`, `COLLECTION_FAILED`는 review 없는 `NOT_EVALUATED + ALLOW`로 구분한다. Technical Gate는 정책 record를 입력으로 갖지 않으므로 금지 재현을 판정하지 않고, validated PoC 연결(`dynamic_linkage`), 실제 코드 경로 연결(`code_flow_linkage`), restrictions 표현의 정확성(`restriction_assessment`)만 검토한다.
...
decision의 `meta`는 `PRIMITIVE_UPDATE` current attempt와 같은 `attempt_id`를 사용하고, Verification·Technical review·Rule Scope review가 모두 같은 analysis·workspace·commit·hypothesis와 exact Verification revision을 가리켜야 한다. `policy_collection_result_ref`도 Rule Scope review가 사용한 reference와 같아야 한다. `COLLECTION_FAILED`에서는 Rule Scope review가 없으므로 decision이 가리키는 collection result와 그 오류 reference를 직접 검사한다. 이 `NOT_EVALUATED + ALLOW` 특별 경로는 `RunPolicyState.collection_result_ref != null`이고 그 exact `PolicyCollectionResult.status=COLLECTION_FAILED`일 때만 허용하며, `RunPolicyState.status`(`BLOCKED | FAILED` 포함)만 보고 진행하지 않는다. `collection_result_ref=null`(collection 결과 확정 전 `PREPARING | BLOCKED | FAILED`)이면 exact `PolicyCollectionResult`가 없으므로 `RuleScopeImpactReview`도 `PrimitiveAdmissionDecision`도 만들지 않고 Reporter도 진행하지 않으며, 재개 가능한 상태면 정책 준비 완료를 기다린다.
...
`ChainingResult.considered_primitive_refs`와 `excluded_lineage_refs` 추가, `source_admission_refs` 제거, `PrimitiveMatchCandidate`의 필드 제거, `no_match_reasons`의 `NoMatchReason` 전환은 기존 결과의 필수 필드를 바꾸므로 새 MAJOR schema로 배포한다. 이전 MAJOR 결과에 세 목록을 빈 배열로 추정하지 않는다. 기존 결과는 감사 이력으로만 보존하고 새 Chaining·가설 등록 입력으로 자동 승격하지 않는다.
...
SandboxProfile:
  ...
  allowed_egress_refs: [StoredDataRef]
  isolation_policy_refs: [StoredDataRef]
  cpu_limit_millicores: integer
  memory_limit_bytes: integer
  disk_limit_bytes: integer
  pid_limit: integer
  max_requested_execution_ms: integer
  created_at: timestamp
...
ExecutionBudgetProfile:
  ...
  currency: string
  pricing_revision_ref: RunStoredDataRef | StoredDataRef
  max_total_work: integer
...
VerificationBudgetProfile:
  meta: RecordMeta with hypothesis_id null and attempt_id null
  profile_key: string
  max_verification_elapsed_ms: integer
  max_work_per_verification: integer
  max_llm_calls_per_verification: integer
  max_retries_per_work: integer
  max_parallel_evidence_calls: integer
  status: DRAFT | ACTIVE | RETIRED
...
BudgetReservation:
  ...
  ledger_entry_ref: RunStoredDataRef | StoredDataRef | null
...
BudgetLedgerEntry:
  ...
  usage_refs: [RunStoredDataRef | StoredDataRef]
  sequence: integer
...
AgentLogEvent:
  ...
  actor: DYNAMIC_REPRODUCTION | REPRODUCTION_SETUP_AUTOMATION | TOOL_RUNTIME | SANDBOX_CONTROLLER | REPRODUCTION_SESSION_MANAGER
  environment_ref: StoredDataRef | null
  environment_recipe_ref: StoredDataRef | null
  poc_candidate_ref: StoredDataRef | null
  tool_request_ref: StoredDataRef | null
  command_ref: StoredDataRef | null
  command_digest: string | null
  redaction_status: REDACTED | NOT_REQUIRED | null
...
DynamicReproductionToolRequest:
  ...
  command: SandboxCommandInput | null
  poc_candidate_ref: StoredDataRef | null
...
DynamicReproductionResult:
  ...
  agent_log_ref: StoredDataRef
  agent_conclusion_ref: StoredDataRef | null
  environment_recipe_ref: StoredDataRef | null
  environment_ref: StoredDataRef | null
  poc_candidate_ref: StoredDataRef | null
  poc_ref: StoredDataRef | null
...
R8 trusted Budget Profile Registry만 `ExecutionBudgetProfile`, `WorkBudgetProfile`, `VerificationBudgetProfile`, `DynamicReproductionLifecycleProfile`과 이를 묶는 `BudgetProfileBinding`을 게시한다. 이 record들은 승인된 설정값을 각 analysis에 고정한 실행별 revision이며, 서로 다른 analysis가 같은 승인값을 사용하더라도 exact record reference를 공유하지 않는다. `ExecutionBudgetProfile.status=ACTIVE`이면 R8·사람 승인을 가리키는 non-null `approval_ref`, `approved_by`, `approved_at`이 필수이고 trusted registry가 이 근거와 profile 내용을 대조한다. `WorkBudgetProfile`은 07번의 역할·작업 종류별 시간·attempt·호출·조회 한도를 담는 versioned 설정이다. `work_type`, `operation_kind`와 `agent_role`은 trusted runtime이 실제 work/action에서 결정하며 Agent가 임의 문자열로 고르지 않는다. 같은 profile 안의 `limit_key`와 `(work_type, operation_kind, agent_role)`은 중복될 수 없고 적용 대상은 정확히 하나로 해석되어야 한다. 해당 한도와 분석 전체·Verification·동적 lifecycle 한도는 모두 적용하며 서로 다른 한도가 같은 자원을 제한하면 더 먼저 소진되는 한도를 따른다. 07번 표의 숫자는 이 profile의 DRAFT 후보이지 코드 상수나 별도 숨은 설정이 아니다. R8 승인과 사람 승인을 받은 exact revision만 `ACTIVE`가 될 수 있고 `DRAFT` 숫자로 실행하지 않는다.
...
`ACTIVE` binding은 같은 `analysis_id`의 run-local `ACTIVE` execution profile과, 같은 analysis·workspace·commit에 묶인 `ACTIVE` work-kind profile·Verification profile·dynamic lifecycle profile exact revision을 하나씩 가리켜야 한다. `purpose`도 `AnalysisRunState`와 모든 profile에서 같아야 한다. 같은 `analysis_id`에서 full `ACTIVE` binding은 최대 하나다. 서로 다른 analysis의 profile·binding·reservation·ledger를 섞지 않는다. registry의 current 조회 키는 `analysis_id`이며 purpose는 일치 검사용 값이지 단독 current 조회 키가 아니다. `status=ACTIVE`이면 R8·사람 승인을 가리키는 non-null exact `approval_ref`, `approved_by`, `approved_at`이 필수이고 `activate_for_run`에 전달된 승인 reference와 같아야 한다. DRAFT에서는 `approval_ref=null`, `approved_at=null`이다. 운영 숫자는 R8 승인 전 임의로 채우지 않는다. `DRAFT | RETIRED` profile 또는 이름만 같은 다른 revision은 사용할 수 없다. 승인된 한도를 바꾸려면 해당 profile의 새 revision과 R8 승인이 필요하다. full binding 또는 work-kind limit을 확인할 수 없으면 후속 실행은 `BLOCKED + waiting_for=BUDGET`이다.
...
정확한 실행 비용 또는 잔여 예산을 입증할 수 없으면 새 실행은 `BLOCKED + waiting_for=BUDGET`이다. 실제 승인 한도를 소진한 경우에만 `BUDGET_EXCEEDED`를 기록한다. Provider가 token usage를 제공하지 않은 사실만으로는 비용 불명을 단정하거나 작업을 차단하지 않는다. 고정 요금·구독 경로도 R8이 승인한 pricing revision에서 해당 실행의 계산 규칙과 비용을 명시해야 한다. `BudgetRemaining`은 ledger와 reservation에서 계산한 read model이며 별도 판단 권한이나 mutable 잔액의 정본이 아니다.
...
`PoCCandidate.llm_call_id`는 같은 analysis·hypothesis·work·attempt에서 candidate를 만든 성공한 `DYNAMIC_REPRODUCTION / CREATE_POC_CANDIDATE` 호출 ID와 같아야 한다. 그 호출은 current request·plan·READY environment뿐 아니라 request의 exact `CodeContextResponse`와 검증된 모든 `code_fragment_refs`를 `LLMCallSpec.context_refs`에 고정한다. 실제 code artifact는 원래 exact reference와 digest를 유지하되 prompt에는 민감정보가 제거된 `redacted_body`만 투영한다. 그 호출의 `LLMInvocationLog.parsed_output_ref`는 candidate 내용을 담은 schema·semantic 검증 완료 canonical JSON artifact를 가리키며 `PoCCandidate` domain revision을 직접 가리키지 않는다. trusted domain finalizer가 그 exact artifact에 runtime 소유 ID와 `RecordMeta`를 붙여 candidate를 만들고 source artifact·호출 provenance와 함께 `SAVE_RESULT`한다. candidate는 log record를 역참조하지 않는다.

`DynamicReproductionConclusion`은 Dynamic Reproduction Agent의 해석 제안이지 최종 실행 사실이나 취약점 판정이 아니다. `llm_call_id`는 같은 analysis·hypothesis·work·attempt의 성공한 `DYNAMIC_REPRODUCTION / INTERPRET_ATTEMPT` 호출 ID와 같아야 한다. 그 log의 유일한 `parsed_output_ref`는 conclusion 내용을 담은 schema·semantic 검증 완료 canonical JSON artifact이고 domain conclusion revision이 아니다. trusted domain finalizer가 exact artifact에 runtime 소유 ID와 `RecordMeta`를 붙여 conclusion candidate를 만들고 source artifact·호출 provenance와 함께 `SAVE_RESULT`한다. conclusion은 log record를 역참조하지 않는다. conclusion의 request·plan·environment·candidate·observation reference는 그 호출의 허용 입력과 같아야 하며 다른 attempt의 record를 섞지 않는다.

`DynamicReproductionToolRequest`는 Sandbox 경계가 허용되고 `SandboxEnvironment.status=READY`가 된 뒤 Dynamic Reproduction Agent가 다음 한 작업만 제안하는 구조화 출력이다. provider 내장 file·command·web tool은 사용하지 않는다. SASTSIMI Runtime은 exact request·plan·environment·work·attempt와 `tools.dynamic-reproduction-inner.v1`을 검사하고, 허용된 요청만 Setup Automation이 제공한 in-container 통로로 보낸다. `RUN_COMMAND`이면 Session Manager가 redaction·digest 계산 뒤 exact `SandboxCommandRecord.tool_request_ref`를 붙여 `COMMAND_STARTED | COMMAND_FINISHED`를 기록한다. 실행 결과와 새 관찰은 다음 `EXECUTE_REPRODUCTION` 호출의 비신뢰 입력이 된다. `USE_POC_CANDIDATE`는 committed candidate artifact의 digest를 재검증하고 그 bytes를 host 파일로 만들지 않은 채 current container의 runtime-owned `/tmp/sastsimi-poc-candidate`에만 materialize한다. 바로 다음 `RUN_COMMAND`가 `/bin/sh`를 executable로, 이 경로를 첫 argv argument로 사용하고 같은 candidate content ref·command·environment·recipe가 `POC_EXECUTION_STARTED | POC_EXECUTION_FINISHED`에 결속된 경우에만 candidate 실행으로 인정한다. `REQUEST_SANDBOX_RECREATE`는 Setup Automation 경로로 보내며, `FINISH` 뒤에는 새 명령 없이 `INTERPRET_ATTEMPT`로 이동한다.

같은 Dynamic Reproduction Agent 논리 session에서 `EXECUTE_REPRODUCTION`을 반복하더라도 매 turn은 새 `llm_call_id`, immutable PromptPayload와 `DynamicReproductionToolRequest`를 가진다. `turn_number`는 attempt 안에서 1부터 증가하고 같은 번호를 재사용하지 않는다. Runtime은 Agent의 요청을 임의 command로 바꾸거나 Sandbox 밖에서 실행하지 않는다. budget·취소·Sandbox profile 경계·session 재시작 조건은 기존 work/attempt lifecycle을 따르며, 이전 attempt의 turn·command·관찰을 current 결과에 섞지 않는다.

`DynamicReproductionResult.agent_conclusion_ref`는 값이 있으면 위 exact conclusion을 가리킨다. `agent_invoked=false`이면 이 값은 반드시 `null`이다. 동적 해석이 완료돼 `status=SUCCEEDED | PARTIAL`이거나 `hypothesis_outcome=SUPPORTED | DISPROVED`이면 같은 attempt의 conclusion이 필수다. Sandbox profile 경계 차단, 환경 실패, 취소 또는 Agent가 결과를 반환하기 전의 실패에서는 `null`일 수 있다. conclusion이 필요한 결과에서는 DynamicReproductionResult의 `hypothesis_outcome`, `hypothesis_evidence_refs`, `hypothesis_linkage`, `limitations`가 각각 exact `DynamicReproductionConclusion.proposed_outcome`, `hypothesis_evidence_refs`, `hypothesis_linkage`, `limitations`와 값·순서 또는 집합 계약에 맞게 동일해야 한다. 비-LLM Reproduction Session Manager는 conclusion을 실제 `AgentLog`, 환경, PoC candidate와 관찰 record에 대조할 뿐이며 **Session Manager는 이 네 값을 새로 판단하거나 바꾸지 않는다**. 불일치하거나 log로 확인할 수 없으면 결과 저장을 거절하고 오류를 남긴다.

`DynamicReproductionRequest`는 R6 Verification이 R7에 무엇을 왜 재현할지 전달하는 불변 record다. `verification_assignment_ref`, `verification_generation`과 `hypothesis_ref`는 current Verification work와 exact match한다. `POC_CONFIRMATION`은 `initial_verdict=TRUE`, `VERDICT_EVIDENCE`는 동적 근거가 더 필요한 `initial_verdict=HOLD`에 사용한다. 이 request를 만든 `CREATE_DYNAMIC_REQUEST` 호출은 같은 work·generation의 exact `VerificationInitialAssessment`, `PlaybookPolicy`, `VerificationPlaybook`, `PlaybookApplication`, Pro·Con을 입력으로 가져야 하고 purpose와 initial verdict는 assessment의 `next_step`·`proposed_verdict`와 일치해야 한다. R7은 request의 purpose·goal·가설·필수 환경 조건과 `sandbox_profile_ref`를 변경하지 않는다.
...
같은 `DYNAMIC_REPRO` attempt 안에서 Sandbox 경계 판정 provenance는 exact reference로 이어진다. build와 run의 `RUN_SANDBOX` action·decision·`SandboxPolicyDecision`은 모두 append-only로 보존한다. `AgentLog`의 `SESSION_STARTED` event `input_refs`에는 Agent가 실제로 들어간 container를 허용한 current run 단계의 exact `SandboxPolicyDecision`을 포함하며, run action 입력은 build 단계 policy와 action decision도 가리킨다. build 단계에서 차단되면 build policy를, run 또는 재생성 단계에서 차단되면 해당 단계 policy를 `POLICY_BLOCKED` event에 남긴다. `DynamicReproductionResult.policy_decision_ref`는 `AgentLog`가 그 attempt에 고정한 current run 단계의 exact `SandboxPolicyDecision`과 동일해야 하고, `action_decision_ref`도 그 단계의 exact `USED` decision이어야 한다. 재생성에 성공하면 둘 다 새 재생성 run 단계 reference로 갱신한다. `agent_log_ref`는 같은 attempt의 `AgentLog` exact revision을 가리킨다. `request_ref`·`meta.attempt_id`·`meta.hypothesis_id`·`verification_generation`·record revision closure가 서로 다르면 stale·invalid provenance로 저장을 거절하며, latest lookup으로 다른 attempt의 policy decision·log·result를 보정하지 않는다. 정책이 없거나 `RunPolicyState.status=BLOCKED | FAILED | UNVERIFIED`여도 이 chain으로 "왜 `LOCAL_ONLY`였는지"를 추적할 수 있어야 하며, 특정 상태에서 `policy_collection_result_ref`·`policy_record_ref`가 null이어야 하는 위 canonical 규칙이 우선한다.

어느 policy 상태에서도 `execution_scope`는 `LOCAL_ONLY`다. Runtime은 다음 관측 가능한 기준으로 로컬 대상과 live 대상을 구분한다.

- clone 사용은 current `CodeWorkspace`의 exact `workspace_id + commit_id`에서 만든 read-only build context와 그 Sandbox 내부 복사본만 인정한다. resolve된 경로가 다른 workspace나 host 경로를 가리키면 거절한다.
- fixture와 mock은 같은 `DYNAMIC_REPRO` work·attempt에서 Setup Automation 또는 Dynamic Reproduction Agent가 생성하고 exact environment·AgentLog·artifact reference로 추적되는 것만 인정한다. 출처를 증명하지 못한 endpoint·계정·데이터는 live로 간주해 거절한다.
- 공격 대상 endpoint는 loopback 또는 현재 `SandboxEnvironment`에 속한 격리 network의 container address만 허용한다. 외부 DNS·공인/사설 원격 IP·실제 서비스 URL과 재사용 가능한 외부 계정은 거절한다.
...
`action_decision_ref`는 첫 build `RUN_SANDBOX` 요청 자체를 만들기 전에 종료된 pre-boundary 결과에서만 `null`일 수 있다. plan 입력 부족·모순은 `failure_category=PLAN`으로 기록하고, 경계 전 Provider/Agent 호출 실패·timeout·resource/retry 한도·내부 실패는 실제 원인에 맞는 `AGENT | TIMEOUT | RESOURCE_LIMIT | RETRY_LIMIT | INTERNAL`을 보존한다. 이때 `agent_invoked=false`다. 이 값은 run 경계 승인 뒤 Sandbox 안에서 Dynamic Reproduction Agent를 시작했는지만 뜻하며, 경계 전 requirements·plan LLM 호출 여부를 뜻하지 않는다. `policy_decision_ref=null`이고 recipe·환경·candidate·validated PoC reference도 모두 `null`이어야 하며 `AgentLog`가 종료 event를 보존한다. 반대로 `EXTERNAL_CONFIGURATION | POLICY_BLOCKED | ENVIRONMENT_SETUP | DEPENDENCY | EXECUTION | OBSERVATION`은 Sandbox 경계 또는 실행 단계의 원인이므로 `action_decision_ref=null`인 pre-boundary 결과에 사용할 수 없다. 그 밖의 결과에서 `action_decision_ref.record_id`는 최종 사용 환경을 허용했거나 마지막으로 차단한 build/run 단계의 exact `RUN_SANDBOX` decision을 가리킨다. ALLOW 결과라면 그 decision은 `USED`여야 하며, build decision과 run decision은 서로 다른 action에 속한다. 이 decision은 plan·recipe·PoC·command 성공을 뜻하지 않으며 다른 가설·attempt에 재사용하지 않는다. Sandbox Controller는 R7 소유 `sandbox_profile_ref`의 host, Docker daemon/socket, host mount·namespace, secret, 허용되지 않은 egress, 다른 workspace 격리와 CPU·RAM·disk·PID·요청 가능 최대 시간 수치를 강제한다. 컨테이너 내부 command를 allowlist로 재판단하지 않는다. `failure_category=POLICY_BLOCKED`이면 `action_decision_ref`와 `policy_decision_ref`가 반드시 존재하고, Sandbox 경계 판정은 `decision=DENY`이며 적용한 `SandboxProfile`의 exact revision과 사유 코드를 확인할 수 있어야 한다. 이 Sandbox 경계 판정은 Technical Gate와 다른 결과이며 서로 변환하지 않는다.

실행 순서는 `EnvironmentRequirements·ReproductionPlan 확정 → 파일 전용 recipe preflight → build RUN_SANDBOX 승인·claim·Controller 검사 → image inspect/build와 EnvironmentRecipe 저장 → run RUN_SANDBOX 승인·claim·Controller 검사 → container·READY environment 준비 → Dynamic Reproduction Agent의 candidate·tool request·command·관찰 → DynamicReproductionConclusion → Reproduction Session Manager 최종 확정`으로 고정한다. build 경계를 통과하기 전에 Docker daemon에 접근하지 않고, run 경계를 통과하기 전에 container 생성·시작·command 실행을 하지 않는다. Dynamic Reproduction Agent는 격리된 container 안에서 환경 설정, 저장소가 필요로 하는 package, 계정, fixture/mock, PoC, command, 관찰과 재시도를 자율적으로 선택할 수 있다. Dynamic Reproduction Agent는 Docker daemon이나 host를 직접 제어하지 않고 Reproduction Setup Automation이 제공한 in-container 실행 통로만 사용한다. 새로운 package registry나 외부 target 접근은 여전히 versioned profile의 egress 경계를 통과해야 한다. Dynamic Reproduction Agent가 선택한 command의 내용은 plan allowlist와 비교하지 않지만 모든 실제 event는 `AgentLog`에 기록한다.

비-LLM `Reproduction Session Manager`는 runtime/tool/lifecycle 계층에서 발생한 event를 durable append-only `AgentLog`로 저장하고, 같은 attempt의 exact reference만 사용해 `DynamicReproductionResult`와 validated PoC를 확정하는 result owner다. Dynamic Reproduction Agent의 실행 전략이나 해석을 대신 쓰지 않으며 Dynamic Reproduction Agent 호출·중단, command 허용, 재시도 또는 cleanup 전략을 결정하지 않는다. `agent_invoked`는 외부 경계 승인 뒤 Sandbox 안에서 실행하는 Dynamic Reproduction Agent 단계가 시작됐는지를 뜻하며, 경계 승인 전에 requirements·plan을 작성한 LLM invocation과 구분한다. Sandbox 안의 Dynamic Reproduction Agent가 호출되기 전 Sandbox profile 외부 격리 경계 차단도 `agent_invoked=false`와 `POLICY_BLOCKED` event를 가진 로그·결과로 확정할 수 있다. `event_id`는 시스템 전체에서 고유하고 `sequence`는 attempt별 1부터 엄격히 증가한다. 시작과 종료 event는 동일한 `action_id`로 연결한다. `COMMAND_STARTED`와 대응하는 `COMMAND_FINISHED`는 같은 attempt의 동일한 exact `DynamicReproductionToolRequest`, `SandboxCommandRecord.command_ref`·`command_digest`, `action_id`, `environment_ref`, `environment_recipe_ref`를 사용한다. command event에는 `tool_request_ref`, `command_ref`, `command_digest`, `redaction_status`가 필수다. `POC_EXECUTION_STARTED | POC_EXECUTION_FINISHED`도 candidate 실행을 주장할 때 동일한 command provenance를 필수로 가지며, `input_refs`의 유일한 값은 digest가 재검증되어 container에 materialize된 exact candidate `content_ref`다. 그 밖의 event에서는 command provenance 필드가 모두 `null`이다. `SandboxCommandRecord`는 실제 실행한 executable·argv·working directory·환경 binding·stdin을 정본으로 보존하되 secret 원문은 저장하지 않고 opaque `secret_refs`로 대체한다. 그 내용은 `tool_request_ref`의 `command`과 redaction 후 의미가 같아야 한다. `command_digest`는 이 redacted canonical command에 대해 계산하고 두 event와 record에서 같아야 한다. redaction이 필요하면 `REDACTED`, 민감값이 없으면 `NOT_REQUIRED`이며 검사 실패 command나 log는 저장하지 않는다. 각 append를 바로 durable revision으로 확정하므로 crash 뒤에도 이전 event가 남고, 종료된 이전 attempt의 늦은 event는 current log나 결과에 붙이지 않는다.

`poc_candidate_ref`는 Dynamic Reproduction Agent가 작성했거나 실행을 시도한 exact `PoCCandidate`를 가리킨다. candidate가 있으면 같은 attempt의 `AgentLog`에 `POC_CANDIDATE_CREATED` 또는 `POC_EXECUTION_STARTED` event와 exact candidate revision·digest가 있어야 한다. 생성하지 못했다면 `poc_candidate_ref=null`이다. candidate 생성·실행 실패에서도 candidate와 관련 log는 감사 이력으로 남길 수 있지만 성공을 뜻하지 않는다.

`poc_ref`는 실제 취약점 재현에 성공한 `PoCBundle`만 가리킨다. `status=SUCCEEDED`, `hypothesis_outcome=SUPPORTED`, `agent_invoked=true`이고 같은 attempt의 `AgentLog`가 exact candidate revision과 `content_digest`를 실제 실행해 지지 관측을 만들었음을 보여야 한다. `PoCBundle`의 request·plan·recipe·environment·log·candidate·execution action과 digest는 결과가 가리키는 값과 exact match한다. 환경 실패, Sandbox profile 외부 격리 경계 차단, candidate 생성·실행 실패, timeout, `DISPROVED | INCONCLUSIVE`, `PARTIAL | FAILED | BLOCKED | CANCELLED`에서는 `poc_ref=null`이다. “가장 최신 PoC”를 다시 조회하거나 candidate와 validated PoC를 한 reference로 덮어쓰지 않는다.

candidate 생성·환경 구성·실행 실패는 동적 재현 실패 사실일 뿐 `FALSE | HOLD`나 technical impact 사실로 변환하지 않는다.

`cleanup_required`는 container·network·volume·image/build 임시 자원·임시 파일 등 정리 대상이 하나라도 생성됐는지를 나타낸다. `false`이면 `cleanup_status=NOT_REQUIRED`, `true`이면 `cleanup_status=SUCCEEDED | FAILED`만 허용한다. Sandbox profile 외부 격리 경계 차단이라는 이유만으로 `NOT_REQUIRED`를 선택하지 않는다. 실제 자원이 있는데 `cleanup_required=false` 또는 `cleanup_status=NOT_REQUIRED`인 결과는 계약 위반이다.

plan 자체가 모순되거나 필수 입력이 없으면 별도 `PlanIssue` 대신 결과의 `plan_execution_status=NEEDS_REVISION`, `plan_issues`와 근거로 반환한다. 해결 가능한 package·setup·PoC·실행 문제는 같은 Dynamic Reproduction Agent session이면 현재 attempt에서 조정하고, session 재시작이 필요할 때만 R8 한도 안에서 같은 work의 새 attempt로 재시도한다. 한도를 소진하면 `DynamicReproductionResult(status=FAILED, hypothesis_outcome=INCONCLUSIVE, poc_ref=null)`로 반환하며 R6가 후속 흐름을 결정한다.

다음 reference는 모두 `record_id`가 있는 exact `StoredDataRef`를 사용한다. target record의 analysis·workspace·commit·hypothesis는 동적 결과와 같아야 한다. request의 `verification_generation`, plan·recipe·candidate·result의 `request_ref`, plan/result의 `purpose`가 exact match해야 한다. current `EnvironmentRecipe`, terminal `SandboxEnvironment`, `AgentLog`, `PoCCandidate`, `PoCBundle`, cleanup과 `DynamicReproductionResult`는 모두 결과와 같은 `DYNAMIC_REPRO.work_id + attempt_id`에 속한다. baseline 재사용을 나타내는 `EnvironmentRecipe.baseline_recipe_ref`만 과거 attempt를 가리킬 수 있으며 현재 binding recipe와 `built_image_digest`가 같아야 한다. log event가 가리키는 environment·recipe·candidate도 event가 속한 attempt의 exact record다. 같은 Dynamic Reproduction Agent session의 command·PoC·환경 조정은 같은 attempt의 event로 기록한다. session 재시작이 필요하면 같은 work의 새 `attempt_id`와 `trigger=RETRY`를 사용하고, 외부 조건 해소 뒤 재개하면 같은 work의 새 `attempt_id`와 `trigger=RESUME`를 사용한다. 과거 attempt artifact와 늦은 event는 current 결과에 섞지 않는다. 별도 `reproduction_id`를 발급하지 않고 exact `DynamicReproductionRequest.record_id + work_id + attempt_id`로 시도를 식별한다.
...
| `ReproductionPlan.environment_requirements_ref` | `environment_requirements` | Dynamic Reproduction Agent | Runtime Validator, Reproduction Setup Automation | 필요한 애플리케이션 환경의 current exact revision |
| `sandbox_profile_ref` | `sandbox_profile` | R7 sandbox policy owner가 승인하고 trusted sandbox policy registry runtime이 게시 | Runtime Validator, Sandbox Controller, Session Manager | network·격리와 CPU·RAM·disk·PID·요청 가능 최대 시간의 immutable exact revision |
| `resource_profile_ref` | `dynamic_reproduction_lifecycle_profile` | R8 evaluation/budget owner가 승인하고 trusted resource policy registry runtime이 게시 | Runtime Validator, Sandbox Controller, Session Manager | 호출 전 work 잔여 시간 검사 정책과 새 attempt 한도의 immutable exact revision |
| `environment_recipe_ref` | `environment_recipe` | Reproduction Setup Automation | Session Manager, Verification, Gate | base·built image digest와 저장소 선언 의존성을 고정한 current attempt recipe |
| `poc_candidate_ref` | `poc_candidate` | Dynamic Reproduction Agent | Session Manager, Verification | 작성 또는 실행을 시도한 PoC script·요청·입력 |
...
| `environment_ref` | `sandbox_environment` | Reproduction Setup Automation | Session Manager, R6 Verification, Gate | 실제 container instance, 생성·재사용 사유와 요구사항 비교 |
| `tool_request_ref` | `dynamic_reproduction_tool_request` | Dynamic Reproduction Agent | Runtime Validator, Session Manager | 같은 attempt에서 Agent가 제안한 다음 한 작업과 실행 전 입력 |
| `command_ref` | `sandbox_command_record` | Reproduction Session Manager | Runtime Validator, Verification, Gate, 운영 디버깅 | 실제 실행한 redacted canonical command와 digest의 same-attempt 정본 |
...
`SandboxProfile`은 `data_kind=sandbox_profile`인 immutable versioned config다. R7 sandbox policy owner가 network·격리와 상자 입장용 CPU·RAM·disk·PID·요청 가능 최대 시간을 소유·확정하고 trusted sandbox policy registry runtime만 새 revision을 current로 게시한다. Sandbox Controller는 요청이 이 상한을 넘으면 `SANDBOX_POLICY_DENIED`로 Agent 시작을 거절한다.

`DynamicReproductionLifecycleProfile`은 `data_kind=dynamic_reproduction_lifecycle_profile`인 immutable versioned config다. R8 evaluation/budget owner는 호출 전 `WORK_REMAINING_TIME`을 검사하는 budget policy와 새 attempt 한도만 소유한다. trusted resource policy registry runtime만 승인된 새 revision을 current로 게시한다. Runtime Validator는 호출 전 잔여 시간이 없으면 `BUDGET_EXCEEDED`로 거절하고, 같은 session 조정은 attempt 수에 포함하지 않으며 session 재시작 `RETRY`와 외부 조건 해소 뒤 `RESUME`로 생기는 새 attempt만 `max_new_attempts`에 센다. CPU·RAM·disk·PID·요청 가능 최대 시간은 이 R8 profile에 넣지 않는다.

build와 run의 각 `RUN_SANDBOX`에서 `ActionRequest.input_refs`와 `ActionDecision.checked_config_refs`에는 exact `SandboxProfile`과 exact `DynamicReproductionLifecycleProfile` revision이 모두 있어야 한다. build action은 exact `recipe_source_ref`와 `image_digest=null`, run action은 exact `EnvironmentRecipe`와 actual built image digest를 추가로 고정한다. `ActionRequest.sandbox_profile_ref`와 `SandboxPolicyDecision.sandbox_profile_ref`는 같은 R7 profile exact `record_id + content_hash`, `ActionRequest.resource_profile_ref`와 `SandboxPolicyDecision.resource_profile_ref`는 같은 R8 lifecycle profile exact `record_id + content_hash`를 가리켜야 한다. 어느 하나가 없거나 서로 다르거나 current revision이 바뀌면 실행 전 decision을 `EXPIRED`로 만들고 해당 단계의 새 action을 요구한다.

R4는 공통 record·필드명·자료형·null·exact reference·상태·생산자와 소비자·오류 규칙을 정한다. R6은 `VerificationInitialAssessment`·`DynamicReproductionRequest`, 반환 결과 소비와 최종 가설 판정을 맡는다. Dynamic Reproduction Agent는 requirements·plan·candidate·Sandbox tool request와 동적 근거 해석을 만들고, Reproduction Setup Automation은 recipe·image·container·cleanup을 실제로 수행한다. Sandbox Controller는 외부 격리 경계만 판정한다. 비-LLM Reproduction Session Manager는 append-only log, validated PoC와 `DynamicReproductionResult`의 유일한 result owner다. Verification은 COMMITTED 결과를 읽어 판정하고, Gate는 final TRUE에 연결된 validated PoC와 동적 근거를 검토하며, Reporter는 두 Gate를 통과한 결과만 사용한다.

Runtime Validator는 build와 run 부작용 직전에 각각의 `RUN_SANDBOX` decision에 고정된 exact request·current requirements·current exact plan·R7 `sandbox_profile_ref`·exact R8 `DynamicReproductionLifecycleProfile`·해당 단계 source/recipe가 current인지 다시 검사하고 호출 전 잔여 시간·새 attempt 한도를 강제한다. Sandbox Controller는 각 단계에서 R7 sandbox profile의 외부 격리·입장 수치 경계를 강제한다. 결과를 저장할 때 `SAVE_RESULT(requested_by=REPRODUCTION_SESSION_MANAGER, result_kind=dynamic_reproduction_result)`는 current Verification generation의 exact R6 request reference와 purpose가 결과에 그대로 연결됐는지 확인한다. 별도로 plan·recipe·최종 run 정책 결정·환경·AgentLog·candidate·validated PoC·cleanup과 동적 결과가 같은 R7 `DYNAMIC_REPRO` work·attempt에 속하고, run action이 build policy·decision provenance를 가리키는지 다시 확인한다. Verification은 `DynamicReproductionState.dynamic_result_ref`, work output과 commit이 같은 final 결과만 읽으며 `DynamicReproductionResult`를 직접 만들거나 수정하지 않는다.
...
| `BLOCKED` | `NONE` 이외, 비어 있지 않은 자유형 reason | current work 입력을 바꾸지 않는 재인증·승인·외부 환경 정비·resource 확보를 기다림. 반드시 `INCONCLUSIVE`, final verdict 없음 |
...
| Dynamic Reproduction Agent와 어떤 자원도 만들기 전 Sandbox profile 외부 격리 경계 차단 | `action_decision_ref`와 `agent_log_ref`, `policy_decision_ref(decision=DENY)` 필수. `agent_invoked=false`. 사전에 만든 same-attempt plan은 보존할 수 있지만 recipe·environment·candidate·validated PoC는 `null`. `BLOCKED | FAILED + POLICY_BLOCKED + INCONCLUSIVE` |
| Sandbox 안의 Dynamic Reproduction Agent 호출 뒤 생성·실행 실패 | `agent_invoked=true`, `agent_log_ref` 필수. 생성된 plan·recipe·environment·candidate는 same-attempt exact ref로 보존하고 `poc_ref=null` |
| 동적 재현 성공과 가설 지지 | `SUCCEEDED + SUPPORTED`, `action_decision_ref`와 `policy_decision_ref(decision=ALLOW)`, `agent_invoked=true`, plan·recipe·environment·AgentLog·candidate와 validated `poc_ref` 필수. 결과와 각 산출물은 current Verification generation의 같은 exact R6 request reference를 사용하고, R7 산출물은 모두 같은 `DYNAMIC_REPRO` work·attempt에 속하며 연결된 digest가 exact match함 |
| 환경 또는 임시 자원을 만든 뒤 오류·Sandbox profile 외부 격리 경계 차단 | `environment_ref`가 있으면 `cleanup_required=true`, `cleanup_status=SUCCEEDED | FAILED`, `cleanup_ref` 필수. 실제 자원이 없을 때만 `NOT_REQUIRED` |
| 필수 환경 요구사항 불일치 | 같은 Dynamic Reproduction Agent session이면 현재 attempt에서 recipe를 고친다. session 재시작은 새 `attempt_id`·`trigger=RETRY`, 외부 수정 대기 해소 뒤 재개는 새 `attempt_id`·`trigger=RESUME`를 사용한다. 복구 불가능하거나 한도 소진이면 `FAILED`; 모두 `ENVIRONMENT_SETUP + INCONCLUSIVE`, `poc_ref=null` |

Dynamic Reproduction Agent가 command·PoC·환경을 같은 session에서 다시 시도하는 것은 한 attempt의 event로 기록한다. session crash처럼 새 attempt가 필요한 일시 오류라도 외부 입력을 기다리지 않고 R8 한도가 남아 있으면 실패 attempt를 보존하고 같은 work를 `RUNNING -> READY -> RUNNING`으로 넘겨 자동 재시도한다. 현재 work 입력을 바꾸지 않는 재인증·승인·외부 환경 정비·resource 확보를 기다릴 때만 `BLOCKED`이며, 조건이 해결되면 `trigger=RESUME`인 새 attempt를 시작한다. 프로그램 정책의 `RunPolicyState`는 실행 당시 감사 provenance이며 이 상태만으로 `LOCAL_ONLY` work를 차단하지 않는다. request나 profile exact revision이 바뀌면 기존 work를 재개하지 않는다. 복구 불가능하거나 한도를 소진하면 Session Manager가 최종 `FAILED + INCONCLUSIVE`를 확정하고 work와 현재 Verification을 verdict 없이 `FAILED`로 끝낸다. PoC 생성·실행 실패를 `FALSE | HOLD`로 변환하지 않는다. `SUPPORTED | DISPROVED`는 실제 관측 reference가 필수다. `DISPROVED`이면 `hypothesis_disproved=true`와 disproof refs가 필요하고, 정상 종료 또는 신뢰 가능한 부분 완료의 `INCONCLUSIVE`일 때만 R6가 final HOLD를 만들 수 있다. `POC_CONFIRMATION | VERDICT_EVIDENCE` 모두 실제 반증은 FALSE, 정상 관측의 불충분은 HOLD, `SUCCEEDED + SUPPORTED`와 validated PoC만 TRUE로 이어진다.
...
- pre-boundary가 아닌 실패 범주에 `action_decision_ref=null`을 사용함, 또는 pre-boundary 실패에 정책·recipe·환경·candidate·validated PoC reference가 붙음
- `failure_category=POLICY_BLOCKED`인데 `action_decision_ref=null`, `policy_decision_ref=null` 또는 `SandboxPolicyDecision.decision`이 `DENY`가 아님
...
- `AgentLog`의 attempt 시작 event `input_refs`에 그 attempt의 exact `SandboxPolicyDecision`이 없거나, `DynamicReproductionResult.policy_decision_ref`가 그 decision과 다르거나, `agent_log_ref`가 다른 attempt의 `AgentLog`를 가리킴
...
- `poc_ref`가 가리키는 `PoCBundle.request_ref`가 결과의 exact request reference와 다르거나, `PoCBundle`의 plan·recipe·environment·log·candidate·execution action·digest·R7 work·attempt 중 하나라도 결과와 다름
...
`COLLECTED` 경로의 `PolicyParserResult`, `ProgramPolicyRecord`, `PolicyCollectionResult`는 같은 non-null `meta.attempt_id`를 사용한다. Parser가 여러 공식 원문을 따로 처리하면 같은 attempt 안에서 원문별 `PolicyParserResult`를 만들 수 있지만, 그 결과를 모은 collection은 최대 하나다. `REUSED_CACHE` 경로는 현재 attempt의 collection·policy record만 새로 만들고 cache closure의 과거 Parser attempt reference를 그대로 보존한다. cache 안의 parser 결과를 현재 attempt가 새로 생산한 것처럼 복사하거나 `attempt_id`를 바꾸지 않는다.
...
새 수집의 `PolicyCollectionResult.status=FOUND | ABSENT_CONFIRMED`이면서 freshness 확인까지 끝나 `RunPolicyState.status=CURRENT | ABSENT`로 확정할 수 있을 때만 새 `PolicyCacheRecord`를 게시한다. Policy Collector는 collection·parser·policy·freshness reference를 exact하게 묶어 결과 확정과 함께 게시하고 `RunPolicyState.policy_cache_ref`에 고정한다. cache 재사용이면 이미 선택한 cache ref를 고정한다. `COLLECTION_FAILED`, `BLOCKED`, `FAILED`, `UNVERIFIED` 준비 결과는 `PolicyCacheRecord`로 게시하지 않는다. `RunPolicyState.status=PREPARING`에서는 `preparation_source=null`, `policy_cache_ref=null`일 수 있고, `CURRENT | ABSENT`에서는 둘 다 필수다.

`CURRENT | ABSENT` 새 수집 경로는 current collection·존재하는 policy record·새 cache·final `RunPolicyState`를 하나의 `TransitionCommit` output으로 확정한다. `UNVERIFIED` 새 수집 경로는 exact collection·존재하는 policy record·final state만 같은 commit으로 확정하고 cache output을 포함하지 않는다. 재사용 경로는 선택한 cache를 work input으로 고정하고 current-run collection·존재하는 policy record·final state를 한 commit으로 확정한다. 어느 경로든 일부만 COMMITTED되거나 state가 가리키는 cache·collection·policy가 work input/output과 다르면 recovery 전 소비하지 않는다.
...
`action_decision_ref.record_id`는 `CALL_RULE_SCOPE_GATE`를 허가하고 `USED`로 claim한 decision revision을 가리킨다. 성공 호출의 `LLMInvocationResult.parsed_output_ref`와 `LLMInvocationLog.parsed_output_ref`는 같은 schema·semantic 검증 완료 canonical JSON artifact를 가리킨다. RULE_SCOPE_GATE trusted domain finalizer는 그 exact artifact에서 runtime 소유 ID와 `RecordMeta`를 가진 `RuleScopeImpactReview` candidate를 만들고 source artifact·invocation result·log를 포함한 `SAVE_RESULT`로 확정한다. 이후 decision revision의 `outcome_refs`에는 같은 call spec을 실행한 log와 COMMITTED current review가 각각 한 번 포함된다. `parsed_output_ref`는 review가 아니라 `record_id=null`인 source artifact를 계속 가리키며 review는 log를 역참조하지 않는다. `verification_result_ref.record_id`, `technical_review_ref.record_id`, `cwe_label_ref.record_id`, `run_policy_state_ref.record_id`, `policy_collection_result_ref.record_id`는 필수다. `run_policy_state_ref`는 action과 call spec에 고정한 같은 분석·프로그램의 current state exact revision이고 collection·policy reference는 그 state와 같아야 한다. collection status는 `FOUND | ABSENT_CONFIRMED`만 허용한다. `FOUND`이면 `policy_record_ref`가 collection result와 같은 exact `ProgramPolicyRecord`를 가리키고, `ABSENT_CONFIRMED`이면 null이다. state가 `UNVERIFIED`여도 이 exact collection 연결은 생략할 수 없으며, collection 전 중단으로 reference가 없는 `BLOCKED | FAILED` state에는 Rule Scope work를 만들지 않는다. `COLLECTION_FAILED`인 collection result를 가리키는 review는 존재할 수 없다. `technical_review_ref` 대상은 `status=ACCEPT`이고, 그 대상의 Verification과 CWELabel reference `record_id`는 Rule Scope review가 직접 가리키는 두 `record_id`와 각각 같아야 한다. runtime은 각 reference의 `workspace_id`, `commit_id`, `content_hash`가 실제 대상 record와 일치하는지 확인한다. 어느 입력 revision이든 바뀌면 이전 review를 재사용하지 않는다.
...
Provider·모델·session·prompt 조합의 품질과 비용을 비교하고, 기술적으로 호출 가능한 profile과 운영 품질을 통과한 조합을 분리하는 기록입니다.

```yaml
UsageMeasurement:
  token_source: PROVIDER_REPORTED | ADAPTER_REPORTED | UNAVAILABLE
  input_tokens: integer | null
  output_tokens: integer | null
  total_tokens: integer | null
  token_unavailable_reason: string | null
  provider_units: map
  cost_source: PROVIDER_REPORTED | PRICING_REVISION_CALCULATED | RESERVED_MAXIMUM | UNAVAILABLE
  cost_minor_units: integer | null
  currency: string | null
  pricing_revision_ref: RunStoredDataRef | StoredDataRef | null
...
ResourceUsageSummary:
  ...
  retry_count: integer
  llm_call_count: integer
  dynamic_attempt_count: integer
  cost_minor_units: integer | null
  currency: string | null
  pricing_revision_refs: [RunStoredDataRef | StoredDataRef]
  usage_measurement_refs: [RunStoredDataRef | StoredDataRef]
...
EvaluationRunConfig:
  ...
  corpus_refs: [StoredDataRef]
  ground_truth_refs: [StoredDataRef]
  grader_refs: [StoredDataRef]
...
EvaluationRunResult:
  ...
  analysis_result_refs: [RunStoredDataRef]
  grader_result_refs: [StoredDataRef]
...
EvaluationRecommendation:
  ...
  target_model: string
  target_session_policy: NEW | RESUME | AUTO
  target_prompt_registry_entry_ref: StoredDataRef
  decision: ACCEPT_FOR_PRODUCTION | REJECT | NEEDS_MORE_EVIDENCE
  rationale: string
  decided_by: R8_EVALUATION_RUNTIME
  decided_at: timestamp
```

같은 `comparison_group_id`의 결과는 `corpus_refs`, `ground_truth_refs`, `grader_refs`, output schema와 budget profile이 exact reference 기준으로 set-equal할 때만 직접 비교한다. provider·model·session·prompt 중 비교하려는 축 외의 입력은 같아야 하며, 다른 revision을 이름만 같다는 이유로 합치지 않는다. grader는 사람 정답과 실행 결과를 비교해 `grader_result_refs`를 만들고, R8 Evaluation Runtime이 품질·시간·사용량·비용을 `EvaluationRunResult`로 집계한 뒤 `EvaluationRecommendation`을 만든다. 추천은 설정 채택 제안일 뿐 Prompt Registry를 직접 활성화하는 권한이 아니다.

`purpose=EVALUATION` 분석과 그 결과는 별도 analysis identity와 exact `eval_config_refs`를 유지한다. Evaluation Runtime은 운영 `HypothesisProcessState`, Finding, Primitive index, Gate, ReportDraft와 production Prompt Registry current pointer를 갱신할 수 없다. 평가 결과를 운영 artifact로 복사하거나 purpose만 바꾸는 것도 금지한다.

`UsageMeasurement`에서 token 값이 있으면 source가 `UNAVAILABLE`일 수 없고 세 token 값은 음수가 아니며 `total_tokens=input_tokens+output_tokens`여야 한다. token이 없으면 세 값은 모두 `null`이고 `token_unavailable_reason`이 필수다. 비용은 금액·통화·가격 기준 revision이 함께 있을 때만 알려진 값이다. `cost_source=PRICING_REVISION_CALCULATED | RESERVED_MAXIMUM`이면 exact `pricing_revision_ref`가 필수이고, `UNAVAILABLE`이면 금액·통화·revision은 `null`, 사유는 필수다. Provider raw 필드는 `provider_units`에 보존하되 공통 token·비용 값으로 지어내어 변환하지 않는다.
...
ExecutionLimits:
  ...
  max_calls_per_work: integer

LLMRetryPolicy:
  meta: RecordMeta
  policy_key: string
  max_schema_repairs: integer
  max_semantic_repairs: integer
  max_retries: integer
  max_failovers: integer
  retryable_statuses: [FAILED | INVALID_OUTPUT | TIMED_OUT | RATE_LIMITED | AUTH_REQUIRED]
  backoff_policy_ref: StoredDataRef | null
...
PromptRedactionPolicy:
  meta: RecordMeta
  policy_key: string
  remove_categories: [CREDENTIAL | COOKIE | TOKEN | BROWSER_PROFILE | HOST_ABSOLUTE_PATH | HIDDEN_REASONING]
  fail_closed: true
...
SemanticValidatorSpec:
  ...
  test_refs: [StoredDataRef]

PromptInputSlot:
  slot: string
  data_kind: string
  field_paths: [string]
  cardinality: REQUIRED_ONE | OPTIONAL_ONE | REQUIRED_MANY | OPTIONAL_MANY
...
`PromptRegistryEntry`는 trusted Prompt Registry Runtime이 등록한 역할·작업별 설정이다. `prompt_key`는 사람이 읽는 등록 이름일 뿐 exact identity가 아니며, 실행할 항목은 `StoredDataRef(record_id + content_hash)`로 고정한다. `ACTIVE` entry는 `agent_role + task_kind + purpose` 조합마다 정확히 하나여야 한다. template, input slot, output schema 또는 semantic validator를 바꾸면 기존 record를 덮어쓰지 않고 새 revision을 만든다.

`input_slots`는 builder가 읽을 수 있는 입력 allowlist다. `field_paths`는 source record에서 노출할 JSON Pointer 목록이며 전체 record를 허용할 때만 `"$"`를 단독으로 쓴다. 같은 slot은 registry 안에서 한 번만 정의하고 `cardinality`로 필수 여부와 개수를 정한다. `REQUIRED_ONE`은 정확히 1개, `OPTIONAL_ONE`은 0개 또는 1개, `REQUIRED_MANY`는 1개 이상, `OPTIONAL_MANY`는 0개 이상이다. 정상적인 빈 집합이 가능한 slot에는 `OPTIONAL_MANY`를 사용하며 필수 개수를 맞추기 위해 관계없는 record를 넣지 않는다. `forbidden_context_kinds`는 어떤 slot에도 들어갈 수 없는 방어 목록이며 `input_slots.data_kind`와 겹치면 registry 자체를 거절한다. `TRUSTED_INSTRUCTION` slot은 사람이 승인한 versioned 정책·template 자료만 허용하고 repository·정책 원문·도구·LLM 출력에는 사용할 수 없다.

`PromptRegistryEntry.provider_profile_refs`는 role·task별로 허용한 exact `ProviderProfile` revision을 우선순위 순서로 고정한다. 첫 항목은 primary이고 이후 항목은 사전에 허용한 fallback이며, 각 profile은 R3-04에서 자기 model·environment 조합의 capability를 검증받아야 한다. `ExecutionLimits`, `LLMRetryPolicy`, `LLMToolPolicy`, `PromptRedactionPolicy`, `OutputSchemaSpec`, `SemanticValidatorSpec`도 모두 exact revision으로 고정한다. `OutputSchemaSpec.result_kind`, registry의 `result_kind`와 result-owner registry 항목은 같아야 한다. 실제 한 호출에서 선택한 `provider_profile_ref`와 `model`은 `LLMCallSpec`에 다시 고정하고 model은 해당 `ProviderProfile.model`과 같아야 한다. `allowed_tools=[]`이면 provider tool을 허용하지 않는다.

Provider Profile Validation(`PVD-01`~`PVD-16`)은 profile이 기술적으로 호출 가능한지만 증명하며 취약점 분석 품질 승인이 아니다. `purpose=EVALUATION`인 ACTIVE entry는 PVD를 통과한 exact profile을 평가 격리 경로에서 사용할 수 있고 `quality_evaluation_ref=null`을 허용한다. `purpose=PRODUCTION`인 ACTIVE entry는 R8 `EvaluationRecommendation(decision=ACCEPT_FOR_PRODUCTION)` exact revision이 `quality_evaluation_ref`에 필수다. recommendation의 target은 평가에 사용한 EVALUATION entry exact revision이고, 새 PRODUCTION entry는 그 target과 provider profile·model·session·template·input slots·schema·validator·limits 등 실행 의미가 field-by-field 같아야 한다. 새 entry의 `previous_record_id`는 사람이 승인한 DRAFT PRODUCTION 후보를 가리키며 quality reference 추가와 `ACTIVE` 전환 외의 실행 의미를 동시에 바꾸지 않는다. `REJECT | NEEDS_MORE_EVIDENCE`, 다른 corpus/config의 결과 또는 이름만 같은 과거 revision으로 운영 ACTIVE를 만들 수 없다. R8은 recommendation을 생산하고 trusted Prompt Registry Runtime만 사람 승인 뒤 새 PRODUCTION entry revision을 활성화한다.

`agent_role=DYNAMIC_REPRODUCTION`, `task_kind=EXECUTE_REPRODUCTION`, `session_policy=AUTO`는 반복형 Sandbox turn에만 쓰는 제한된 규칙이다. 같은 work·attempt의 첫 turn은 `NEW`이고 `parent_session_ref=null`이어야 한다. 후속 turn은 바로 앞 성공 turn의 exact `session_ref`를 `parent_session_ref`로 둔 `RESUME`이어야 한다. 새 attempt·다른 work·다른 가설이거나 선행 session을 신뢰할 수 없어 재시작하면 다시 `NEW`이며, Runtime Validator는 AUTO를 다른 관계의 임의 session 재사용으로 해석하지 않는다.

`PromptPayload`는 trusted Prompt Builder가 그 entry와 허용된 exact context로 조립한 불변 호출 입력이다. 각 binding의 `source_ref`는 원본 exact record다. `projected_data_ref`는 허용 `field_paths`만 뽑은 뒤 비밀값과 host 절대 경로를 먼저 제거하고 canonical serialization한 불변 artifact다. redaction 전 projection은 저장하지 않는다. repository 코드·문서·정책 원문·도구·이전 LLM 출력은 `UNTRUSTED_DATA`로만 binding하며 `TRUSTED_INSTRUCTION`으로 승격하지 않는다. `rendered_prompt_ref`는 template과 이 redacted projected data를 결합한 실제 전송 직전 artifact를 가리킨다.

`PromptPayload.context_bindings[*].source_ref`의 중복 없는 집합은 `LLMCallSpec.context_refs`와 `LLMInvocationRequest.context_refs`의 집합과 정확히 같아야 한다. 각 binding의 slot·data kind·field paths·trust class와 개수는 registry의 exact `PromptInputSlot`과 같고 금지 목록에는 없어야 한다. action `input_refs`에는 source와 projected data reference를 모두 넣는다. 필수 slot 누락, 허용 field 밖의 projection, 다른 workspace·commit·hypothesis의 reference, registry에 없는 추가 context 또는 untrusted 자료의 `TRUSTED_INSTRUCTION` 승격은 provider 호출 전에 거절한다.

registry entry, PromptPayload, `LLMCallSpec`, request와 current `AnalysisRunState.purpose`는 모두 같은 `purpose`여야 한다. EVALUATION entry로 PRODUCTION 호출을 하거나 그 반대면 Provider 호출 전에 `AUTHORITY_DENIED`로 차단한다.

`PromptRegistryEntry.purpose`·`quality_evaluation_ref`, `PromptPayload.purpose`, `LLMCallSpec.purpose`, `LLMInvocationRequest.purpose`는 운영과 평가 권한을 가르는 필수 필드이므로 각 계약의 새 MAJOR schema에서 사용한다. 이전 MAJOR entry의 용도를 이름이나 사용 이력으로 추정해 PRODUCTION으로 올리지 않는다. 명시적 migration에서 exact 평가 근거와 사람 승인을 입증하지 못하면 과거 entry는 감사 이력으로만 보존한다.

`call_spec_ref.record_id`는 수정할 수 없는 exact `LLMCallSpec` revision을 가리킨다. `action_decision_ref.record_id`는 일반 Agent이면 `CALL_LLM`, Gate이면 해당 `CALL_TECHNICAL_GATE | CALL_RULE_SCOPE_GATE`, Reporter이면 `CREATE_REPORT_DRAFT` action을 `ALLOW`하고 `USED`로 claim한 exact decision revision을 가리킨다. 그 action의 `llm_call_spec_ref.record_id`는 `call_spec_ref.record_id`와 같아야 한다. request의 `llm_call_id`, role, task, provider profile, model, session, parent session, context, prompt registry/template/payload, limits·retry·tool·redaction·schema·validator refs, output schema 이름, token budget 계획값과 timeout은 spec과 field-by-field exact equality를 만족해야 하며 runtime은 이 equality를 provider 호출 직전에 다시 확인한다. spec·request·payload의 role·task·prompt key·template revision·version·output schema ref도 registry entry와 같아야 한다. 다르면 decision을 `EXPIRED`로 바꾸고 호출하지 않는다. 이 equality는 승인된 요청의 변조를 막는 `REVISION` 검사이며 token 사용량 상한 검사가 아니다. `timeout_ms`는 exact `ExecutionLimits`의 값이고 monotonic clock으로 계산하는 0보다 큰 밀리초 실행 예산이다.

`LLMCallSpec.token_budget`과 `LLMInvocationRequest.token_budget`은 provider 호출에 예상되는 사용량을 기록하는 0 이상의 선택 계획값이다. 값을 정하지 않았으면 `null`이며, 실제 사용량이 계획값을 넘거나 provider가 usage를 제공하지 않아도 token만을 이유로 `ActionCheck.BUDGET=FAIL`, `DENY` 또는 `BUDGET_EXCEEDED`를 만들지 않는다. 실제 usage는 `LLMInvocationLog`와 `AnalysisRunResult.resources`에 출처와 함께 기록하고 제공되지 않으면 `null`로 둔다. 이 nullable 의미로 바뀐 두 계약은 새 MAJOR schema로 배포하고 이전 값을 강제 상한으로 해석하지 않는다. `LLMInvocationResult.usage`·`LLMInvocationLog.usage`의 loose map을 `UsageMeasurement`로, `AnalysisRunResult.resources`의 loose map을 `ResourceUsageSummary`로 바꾸는 것도 새 MAJOR schema와 명시적 migration이 필요하다. 출처·미제공 사유·통화·가격 revision을 입증할 수 없는 과거 map은 알려진 값으로 변환하지 않고 감사 이력으로만 보존한다.

`LLMCallSpec`은 이를 입력으로 가진 첫 `ActionDecision`이 저장된 뒤 수정하지 않는다. action `input_refs`에는 spec 자체와 spec의 registry·template·payload·model·limits·retry·tool·redaction·schema·validator reference, 모든 source/projected context reference를 포함하고 `REVISION`·`REDACTION` check를 적용한다. `CALL_LLM`에서는 spec role이 `requested_by`와 같아야 한다. `CALL_TECHNICAL_GATE | CALL_RULE_SCOPE_GATE | CREATE_REPORT_DRAFT`에서는 각각 `TECHNICAL_GATE | RULE_SCOPE_GATE | REPORTER`여야 한다. retry와 failover는 새 `llm_call_id`, spec, action과 decision을 만든다. 다른 provider로 전환해도 같은 논리 `PromptPayload`와 output schema를 사용하며, adapter가 역할 지시·context·판정 기준을 추가·삭제하면 호출 결과를 사용하지 않는다.
...
LLMInvocationResult:
  ...
  parsed_output_ref: StoredDataRef | null # SUCCEEDED이면 canonical JSON artifact, record_id=null
...
LLMInvocationLog:
  ...
  parsed_output_ref: StoredDataRef | null # SUCCEEDED이면 result와 같은 canonical JSON artifact, record_id=null
...
`LLMInvocationLog.action_decision_ref`와 `call_spec_ref`는 request와 같아야 한다. result와 log의 purpose, role·task·provider profile·model·session·prompt registry/template/payload·limits·retry·tool·redaction·schema·validator·context는 request와 spec에서 바뀌지 않으며 실제 adapter가 선택한 값과 차이가 있으면 호출을 실패 처리한다. `SUCCEEDED`이면 result와 log의 `parsed_output_ref`는 서로 같고 `data_kind=artifact`, `record_id=null`, 같은 workspace·commit의 exact `stored_data_id + content_hash`를 가진 불변 canonical JSON artifact를 가리킨다. 이 artifact는 output schema와 semantic validator를 통과한 Agent 내용일 뿐 `DomainRecord`, COMMITTED 역할 결과 또는 runtime 권한 기록이 아니다. Agent 출력에 `RecordMeta`, `logical_record_id`, domain `record_id`나 `proposal_id | question_id | validation_id` 같은 runtime 소유 ID가 있으면 `INVALID_OUTPUT`으로 거절한다.

T10에서 도입하는 역할별 trusted domain finalizer는 성공한 같은 호출·work·attempt에 고정된 exact artifact만 읽고 `RecordMeta`, `logical_record_id`, `record_id`와 해당 계약의 runtime 소유 ID를 발급해 domain candidate를 만든다. finalizer는 기존 역할 runtime 안의 비-LLM 권한 경계이며 새 Agent나 새 pipeline 단계가 아니다. `SAVE_RESULT.input_refs`에는 source artifact, `LLMInvocationResult`, `LLMInvocationLog`, 기존 exact domain 입력과 candidate를 모두 고정하고, Runtime Validator가 role·task·result kind·work·attempt·content 대응과 생산 권한을 검사한다. 성공 commit 뒤 work `output_refs`와 action decision의 후속 `outcome_refs`가 COMMITTED domain record를 가리키며 `parsed_output_ref`는 source artifact를 계속 가리킨다. 기존 역할 계약이 `HypothesisProposal[]`처럼 배열이면 artifact 하나에서 순서를 보존해 여러 domain candidate를 만들 수 있지만 서로 다른 result kind를 섞지 않는다. 서로 다른 결과 종류는 task별 독립 호출과 artifact를 사용한다. `CWELabel.llm_call_id` 같은 역할별 호출 ID는 source invocation의 `llm_call_id`와 같아야 하며 domain output은 log를 역참조하지 않는다. 실패·취소·형식 오류 호출은 `parsed_output_ref=null`이고 domain candidate나 결과를 만들지 않는다.

Prompt·model·limits·retry·tool·redaction·schema·validator 설정 record, `PromptRegistryEntry`, `PromptPayload` 추가와 `DYNAMIC_REPRODUCTION`·prompt exact reference 필드 추가는 LLM invocation 계약의 새 필수 구조이므로 새 MAJOR schema에서만 사용한다. 이전 MAJOR의 role 문자열, `prompt_template_version` 또는 output schema 이름만 보고 새 exact reference를 추정해 채우지 않는다.
...
`action_decision_ref.record_id`는 `CREATE_REPORT_DRAFT` action의 report 조건, exact LLM call spec과 redaction을 모두 통과한 `USED` decision revision을 가리킨다. 성공 호출의 `LLMInvocationResult.parsed_output_ref`와 `LLMInvocationLog.parsed_output_ref`는 같은 schema·semantic 검증 완료 canonical JSON artifact를 가리킨다. REPORTER trusted domain finalizer는 그 exact artifact에서 runtime 소유 ID와 `RecordMeta`를 가진 `ReportDraft` candidate를 만들고 source artifact·invocation result·log를 포함한 `SAVE_RESULT`로 확정한다. 이후 decision revision의 `outcome_refs`에는 같은 call spec을 실행한 log와 COMMITTED current draft가 각각 한 번 포함된다. `parsed_output_ref`는 draft가 아니라 `record_id=null`인 source artifact를 계속 가리키며 draft는 log를 역참조하지 않는다. `finding_ref`, `verification_result_ref`, `technical_review_ref`, `rule_scope_impact_review_ref`, `cwe_label_ref`, `run_policy_state_ref`와 `policy_record_ref`는 저장된 record를 가리므로 각 `StoredDataRef.record_id`가 필수다. Reporter runtime은 action 승인·실제 provider 호출·초안 저장 직전에 다음 연결을 모두 확인하고 하나라도 다르면 초안을 만들지 않는다.
...
- `content_ref` 본문에 코드 위치로 적은 모든 `path:line`은 exact `verification_result_ref`의 `EvidenceClaim.code_locations` 중 같은 `workspace_id`, `commit_id`, `file_path`와 일치하고 line이 `start_line..end_line` 안에 있어야 한다. upstream에 없는 위치나 다른 commit의 위치가 하나라도 있으면 `INVALID_OUTPUT`·`REPORT_ERROR`로 처리하고 draft 저장과 current pointer 갱신을 거절한다.
...
Finding 의미·품질 기준은 R5가 소유하고 canonical 저장 schema·authority는 아래 R4 계약을 따른다. 저장 전 후보는 별도 이름·schema·domain record를 만들지 않고, 동일한 `Finding` schema의 immutable staging data를 기존 `SAVE_RESULT.candidate_result_ref`로 고정한다. R4는 exact upstream을 조립할 뿐 새 Finding claim을 만들거나 빠진 Finding을 추정하지 않는다. Finding이 없으면 `CREATE_REPORT_DRAFT`를 허용하지 않고 `AnalysisRunResult.report_draft_refs=[]`를 유지한다. 원인은 `REPORT_NOT_READY`와 연결된 오류·상태 기록으로 추적한다.

current Finding은 새 vulnerability verdict나 impact를 만드는 Gate가 아니라, 신뢰 runtime이 이미 `COMMITTED`된 exact upstream reference를 조립한 정규화 record다. 신뢰 runtime은 한 가설의 `RuleScopeImpactReview`가 `COMMITTED`된 뒤(`review_status`·`report_permission` 값과 무관), `CREATE_REPORT_DRAFT`의 `REPORT_READY` 검사 전에, 다음 exact chain에서 current Finding을 만든다: `HypothesisProcessState.status=TERMINAL`의 final `VerificationResult(verdict=TRUE)`, current generation의 `SUCCEEDED + SUPPORTED` `DynamicReproductionResult`와 validated `poc_ref`, 그 Verification을 직접 가리키는 current `CWELabel`, 그 pair를 검토한 `TechnicalEvidenceReview.status=ACCEPT`, 같은 `verification_result_ref`·`technical_review_ref`·`cwe_label_ref`·`policy_collection_result_ref`(`FOUND`이면 `policy_record_ref`)를 가리키는 current `RuleScopeImpactReview`. upstream record의 `meta.workspace_id`·`meta.commit_id`가 일치해야 하며, hypothesis-local artifact에 대해서만 동일 `meta.hypothesis_id`를 요구한다. `ProgramPolicyRecord`, `PolicyCollectionResult` 등 hypothesis 비종속 정책 record에는 `hypothesis_id` 일치를 요구하지 않으며, 기존 exact `StoredDataRef`, `meta.workspace_id`·`meta.commit_id` 및 policy revision/provenance 계약으로 검증한다. 또한 `TechnicalEvidenceReview`·`RuleScopeImpactReview`·`CWELabel`이 같은 `verification_result_ref`를 가리키며 그 `VerificationResult`가 current `HypothesisProcessState.verification_result_ref`와 같아야 한다. current Verification generation은 새 `verification_generation` field를 `RecordMeta`나 Gate schema에 추가하지 않고 기존 `HypothesisProcessState`·`VERIFICATION` work generation·`CWELabel.verification_generation` 계약으로 확인한다. `restrictions`·`unresolved_conditions`와 evidence transitive closure를 그대로 보존한다. 서로 다른 Verification generation의 artifact·evidence를 섞으면 기존 공통 `STALE_RESULT`·revision mismatch 계약으로 차단되고, 정책 수집이 `COLLECTION_FAILED`여서 `RuleScopeImpactReview`가 없으면 Finding을 만들지 않는다. Finding claim 강도는 verified upstream evidence 이하다. 의미·생성 closure·claim 제한·stale 조건은 R5가 소유하며 [05. 이중 LLM Gate와 보고](05-llm-gate-and-reporting.md)를 따른다.

### Finding canonical schema와 current pointer
...
Finding:
  ...
  policy_record_ref: StoredDataRef | null
  evidence_refs: [StoredDataRef]
  condition_sources: [FindingConditionSource]

FindingConditionSource:
  kind: RESTRICTION | LIMITATION | UNRESOLVED_CONDITION
  source_ref: StoredDataRef
  source_path: string
...
FindingIndexState:
  ...
  finding_ref: StoredDataRef | null
  stale_finding_ref: StoredDataRef | null
  normalization_work_ref: StoredDataRef | null
  last_transition_commit_ref: StoredDataRef | null
  invalidated_by_refs: [StoredDataRef]
```

모든 record reference의 `record_id`는 필수이며 `StoredDataRef`의 identity·hash·workspace·commit exact equality를 적용한다. `poc_ref`는 dynamic result가 연결한 validated `PoCBundle`이며 candidate PoC가 아니다. `policy_record_ref`는 `FOUND`이면 review·collection result의 exact 정책 record와 같고 `ABSENT_CONFIRMED`이면 null이다. `evidence_refs`는 final Verification의 `pro_evidence_ref`·`con_evidence_ref`, `supporting_evidence`·`counter_evidence`에서 도달하는 exact evidence transitive closure와 중복 없이 set-equal하다. 독립 raw artifact에는 기존 규칙대로 `record_id=null`을 허용하되 저장 ID·내용 hash를 고정한다. 서로 다른 generation의 evidence를 합치지 않는다.
...
`FindingIndexState`의 unique key는 `(analysis_id, workspace_id, commit_id, hypothesis_id)`이고 가설별 current Finding은 최대 하나다. 최초 `EMPTY`는 version 1, 모든 nullable ref는 null이다. `CURRENT`는 `finding_ref`, 성공한 `normalization_work_ref`, 그 결과를 확정한 `last_transition_commit_ref`가 필수이고 stale ref는 null이다. `STALE`은 `finding_ref=null`, 이전 exact Finding을 `stale_finding_ref`에 보존하고 `invalidated_by_refs`에 변경을 일으킨 exact 상태·결과 revision을 기록한다. STALE 상태의 `normalization_work_ref`는 이전 Finding 생산 work를 유지하며 `last_transition_commit_ref`는 invalidation을 확정한 upstream transition commit을 가리킨다. 두 비-stale 상태의 invalidation 목록은 비어 있다. index의 `meta.attempt_id=null`이며 Finding은 실제 normalization attempt를 기록한다.

저장 시 service는 `Finding`의 새 immutable revision, work의 `SUCCEEDED`, index의 `CURRENT` 새 revision을 하나의 논리적 atomic `TransitionCommit`으로 확정한다. 기존 `StateTransition`·`TransitionCommit` schema와 unique `(work_id, target_state_version)` marker를 재사용한다. index는 추가 work output이 아닌 전문 상태 projection이다. 같은 가설의 Finding과 index는 각각 같은 `logical_record_id`를 유지하며 새 `record_id`, 증가한 `revision_number`, exact `previous_record_id`로 연결한다. CAS는 work의 expected state version·active attempt·input hash뿐 아니라 index의 expected `record_id`·`state_version`과 모든 upstream current pointer/generation을 함께 비교한다. 성공 시 index version은 정확히 1 증가한다. 충돌은 `STATE_VERSION_CONFLICT` 또는 `STALE_RESULT`로 `ABORTED` 처리하고 candidate를 current에 연결하지 않는다. `PREPARED` Finding은 소비할 수 없고, COMMITTED marker·work output·index pointer가 일치할 때만 current로 읽는다. marker 이후 projection 중단은 recovery가 같은 journal을 재적용한다.

R5의 stale 조건인 Verification generation/revision, CWELabel revision, 두 Gate 재생성, dynamic result·validated PoC, 고정 정책 등 claim-relevant upstream 변경은 해당 current 갱신과 같은 atomic 경계에서 의존 index를 `STALE`로 바꾼다. 물리적 다중 저장소에서는 변경 transition journal이 exact dependent index의 expected revision과 invalidation projection을 함께 고정하고, projection 완료 전 downstream current 조회·Reporter·run finalization을 차단한다. Finding history는 수정하지 않는다. normalization commit과 invalidation은 같은 index CAS로 직렬화하여 옛 chain이 다시 current가 되는 것을 막는다. 새로운 chain이 eligibility를 만족하면 새 Finding revision을 commit하고, `COLLECTION_FAILED` 등 review 부재에서는 stale index를 current로 복구하지 않는다.

Reporter의 action 승인·provider 호출 직전·draft 저장 시 current index와 exact upstream을 재검사하며 current non-stale Finding만 소비한다. `REPORT_READY` 6축은 별도 검사이고 Finding 저장 eligibility에 적용하지 않는다. `AnalysisRunResult.finding_refs`는 확정 시 각 `CURRENT` index가 가리키는 유효한 Finding의 중복 없는 집합과 set-equal해야 한다. run finalization은 index version과 upstream을 CAS 비교 기준 묶음으로 확인하고 invalidation이 진행 중이면 확정하지 않는다. 이는 별도 Snapshot 모듈이 아니라 atomic 갱신 시 비교하는 exact reference와 version 집합이다. 확정 후 upstream이 변경되는 경우 과거 run result는 불변 실행 기록으로 보존하며 current run result pointer는 invalidation 완료 후 새 `AnalysisRunResult` revision으로 atomic 갱신한다. 과거 Finding을 current 목록에 재사용하지 않는다.
...
AnalysisRunResult:
  ...
  dynamic_reproduction_conclusion_refs: [StoredDataRef]
...
Reporter가 허용된 경로에서는 current `ReportDraft` 저장과 해당 `REPORT_DRAFT` work 종료를 기다린다. Reporter가 차단된 경로에서는 draft 저장을 종료 전제조건으로 요구하지 않는다. 신뢰 runtime은 필요한 work와 invalidation 정리를 마친 뒤 모든 current 결과와 로그를 `AnalysisRunResult`에 묶어 `AnalysisRunState`와 atomic하게 확정한다. `ReportDraft`는 마지막 Agent 산출물이고 `AnalysisRunResult` 확정은 새 판단을 생성하지 않는 저장 작업이다. 그 다음 Agent 자동화는 종료된다. ReportDraft 이후의 검토·수정·제출·공개는 Agent 자동화 밖에서 사람이 수행한다. 이 외부 과정에는 공통 schema, action, 상태 또는 자동 공개 권한을 정의하지 않는다.
...
`AnalysisRunResult.dynamic_reproduction_conclusion_refs`에는 `dynamic_result_refs[*].agent_conclusion_ref`의 non-null exact reference를 중복 없이 넣는다. conclusion이 없는 Sandbox profile 경계 차단·사전 실패 결과에는 가짜 conclusion을 만들지 않는다. 목록의 각 conclusion은 같은 attempt의 Dynamic Reproduction Agent 호출 log와 dynamic result에서 함께 복원할 수 있어야 한다.

`AnalysisRunResult.policy_collection_result_refs`에는 한 프로그램의 단일 `POLICY_FETCH` work에서 각 attempt가 COMMITTED한 모든 `PolicyCollectionResult`를 중복 없이 넣는다. `policy_collection_result_refs`가 복수형인 이유는 한 프로그램의 같은 `POLICY_FETCH` work가 재시도될 수 있기 때문이며 여러 프로그램을 뜻하지 않는다. current `RunPolicyState.collection_result_ref`는 이 목록의 정확히 한 항목이거나, 어떤 attempt도 collection 결과까지 도달하지 못한 `PREPARING | BLOCKED | FAILED` state라면 null이다. `UNVERIFIED` state의 collection reference는 null일 수 없고 이 목록의 exact `FOUND | ABSENT_CONFIRMED` 결과를 가리킨다. 이전 실패 attempt의 결과는 목록과 append-only 이력에 남지만 Rule Scope·Reporter 입력이나 성공 cache로 승격하지 않는다. `policy_parser_result_refs`에는 그 attempts에서 실제 만든 parser 결과를 중복 없이 넣는다. cache 재사용 경로에서는 새 parser 실행 결과를 만들지 않고 cache closure의 exact parser reference를 기록한다. `policy_record_refs`는 `FOUND` collection result가 exact하게 가리킨 현재 run의 정책 record만 포함한다. `policy_cache_refs`는 `RunPolicyState.policy_cache_ref`와 set-equal하며 `CURRENT | ABSENT`이면 정확히 하나, 준비 전 종료 또는 `BLOCKED | FAILED | UNVERIFIED`이면 비어 있다. `ABSENT_CONFIRMED | COLLECTION_FAILED`에 대응하는 가짜 정책 record를 만들거나 누락된 수집 결과를 정책 record 유무만으로 추정하지 않는다. 목록의 reference는 해당 `POLICY_FETCH` work input·output과 COMMITTED transition에서 복원할 수 있어야 한다. 이 목록 추가는 `AnalysisRunResult`의 새 필수 필드이므로 새 MAJOR schema에서만 사용한다. 이전 결과에 current cache·collection·parser reference를 추정해 넣지 않는다.

`AnalysisRunResult.program_id`는 시작 상태의 값과 같아야 한다. `workspace_ref`는 finalization 당시 `AnalysisRunState.workspace_ref`와 exact match하며, non-null이면 참조 대상의 `workspace_id`·`commit_id`가 결과의 같은 필드와 일치해야 한다. `CodeWorkspace.status=READY` 전에 종료된 `FAILED | CANCELLED` 결과는 `run_policy_state_ref=null`을 허용한다. 이 경우 정책·cache 결과 목록도 비어 있어야 한다. `CodeWorkspace.status=READY` 뒤 정책 준비를 시작한 실행은 종료 상태와 관계없이 `run_policy_state_ref`가 필수다. 이 값은 해당 run에서 준비를 끝낸 `RunPolicyState` exact revision을 가리키고, 그 state의 cache·collection·policy reference는 위 목록에 포함된 같은 exact reference와 일치해야 한다. 과거 state·collection·parser·policy record는 append-only history에 남기되 다른 run의 current pointer로 가장하지 않는다. 실행 간 재사용은 exact `PolicyCacheRef`를 통해서만 표현한다. `CodeWorkspace.meta`, `AnalysisRunState.workspace_ref`, `ToolRunResult.meta`, `AnalysisRunResult.workspace_ref`와 result-owner 연결은 저장 의미를 바꾸므로 새 MAJOR schema에서만 사용하며 이전 record에 current reference를 추정해 채우지 않는다.

`AnalysisRunResult.primitive_and_chaining_refs`에는 current `PrimitiveAdmissionDecision`, current `PrimitiveIndexState`, 그 index가 가리키는 Primitive와 current ChainingResult만 넣는다. 과거 record와 verdict는 append-only 감사 이력에 보존하며 `FALSE | HOLD`로 바꾸지 않는다.
