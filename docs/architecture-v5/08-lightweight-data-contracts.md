...
Gate가 검토하는 impact·unresolved condition·실행 사실은 `RuleScopeImpactReview`에 중복 필드로 복사하지 않는다. exact `verification_result_ref`에서 `supporting_evidence`, `counter_evidence`, `verdict_rationale`, `unresolved_conditions`, primitive candidate와 `dynamic_request_ref`·`dynamic_result_ref`를 따라간다. 동적 결과에서는 exact `request_ref`·`reproduction_plan_ref`와 같은 attempt의 `agent_log_ref`, 실제 환경·recipe·입력·관측·PoC·cleanup reference를 따라간다. 이 transitive closure의 revision, current Verification generation과 R7 attempt 일치 여부를 공통 reference validator가 확인하며 Rule Scope Gate는 그 실제 수행 사실을 공식 testing restriction과 비교한다.
