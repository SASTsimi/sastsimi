...
| `RUN_SANDBOX` | SCHEMA, AUTHORITY, IDENTITY, REVISION, STATE, BUDGET | R7의 current work, exact `DynamicReproductionRequest`·current `EnvironmentRequirements`·current exact `ReproductionPlan`·`sandbox_profile_ref`·exact `DynamicReproductionLifecycleProfile`을 고정해 로컬 전용 외부 격리 경계 생성만 허가. 실행 당시 `RunPolicyState`는 감사 reference로 별도 기록 |
...
같은 `DYNAMIC_REPRO` attempt 안에서 Sandbox 경계 판정 provenance는 exact reference로 이어진다. `RUN_SANDBOX` attempt가 시작될 때 기록하는 `AgentLog`의 `SESSION_STARTED` event(자원 생성 전 Sandbox profile 외부 격리 경계 차단이면 `POLICY_BLOCKED` event)의 `input_refs`에는 그 attempt에 고정한 exact `SandboxPolicyDecision`(`data_kind=sandbox_policy_decision`)을 포함한다. `DynamicReproductionResult.policy_decision_ref`는 `AgentLog`가 그 attempt에 고정한 exact `SandboxPolicyDecision`과 동일해야 하고, `agent_log_ref`는 같은 attempt의 `AgentLog` exact revision을 가리킨다. `request_ref`·`meta.attempt_id`·`meta.hypothesis_id`·`verification_generation`·record revision closure가 서로 다르면 stale·invalid provenance로 저장을 거절하며, latest lookup으로 다른 attempt의 policy decision·log·result를 보정하지 않는다. 정책이 없거나 `RunPolicyState.status=BLOCKED | FAILED | UNVERIFIED`여도 이 chain(그 attempt에 고정한 `SandboxPolicyDecision`의 `run_policy_state_ref`·`observed_policy_status`·`reason_codes`)으로 "왜 `LOCAL_ONLY`였는지"를 추적할 수 있어야 하며, 특정 상태에서 `policy_collection_result_ref`·`policy_record_ref`가 null이어야 하는 위 canonical 규칙이 우선한다.
...
`action_decision_ref`는 `RUN_SANDBOX` 요청 자체를 만들기 전에 종료된 pre-boundary 결과에서만 `null`일 수 있다. plan 입력 부족·모순은 `failure_category=PLAN`으로 기록하고, 경계 전 Provider/Agent 호출 실패·timeout·resource/retry 한도·내부 실패는 실제 원인에 맞는 `AGENT | TIMEOUT | RESOURCE_LIMIT | RETRY_LIMIT | INTERNAL`을 보존한다. 이때 `agent_invoked=false`다. 이 값은 경계 승인 뒤 Sandbox 안에서 Dynamic Reproduction Agent를 시작했는지만 뜻하며, 경계 전 requirements·plan LLM 호출 여부를 뜻하지 않는다. `policy_decision_ref=null`이고 recipe·환경·candidate·validated PoC reference도 모두 `null`이어야 하며 `AgentLog`가 종료 event를 보존한다. 반대로 `EXTERNAL_CONFIGURATION | POLICY_BLOCKED | ENVIRONMENT_SETUP | DEPENDENCY | EXECUTION | OBSERVATION`은 Sandbox 경계 또는 실행 단계의 원인이므로 `action_decision_ref=null`인 pre-boundary 결과에 사용할 수 없다. 그 밖에 Sandbox 외부 경계 검사를 시도한 모든 결과의 `action_decision_ref.record_id`는 R7 호출자의 권한·상태·예산, exact `DynamicReproductionRequest`, current `EnvironmentRequirements`, current exact `ReproductionPlan`, `sandbox_profile_ref`와 exact `DynamicReproductionLifecycleProfile`을 확인해 외부 격리 경계 생성을 허가한 `RUN_SANDBOX` ALLOW decision의 `USED` revision을 가리킨다. 이 decision은 plan·recipe·PoC·command 성공이나 Sandbox profile 외부 경계 통과를 뜻하지 않으며 다른 가설·attempt에 재사용하지 않는다. Sandbox Controller는 R7 소유 `sandbox_profile_ref`의 host, Docker daemon/socket, host mount·namespace, secret, 허용되지 않은 egress, 다른 workspace 격리와 CPU·RAM·disk·PID·요청 가능 최대 시간 수치를 강제한다. 컨테이너 내부 command를 allowlist로 재판단하지 않는다. `failure_category=POLICY_BLOCKED`이면 `action_decision_ref`와 `policy_decision_ref`가 반드시 존재하고, Sandbox 경계 판정은 `decision=DENY`이며 적용한 `SandboxProfile`의 exact revision과 사유 코드를 확인할 수 있어야 한다. 이 Sandbox 경계 판정은 Technical Gate와 다른 결과이며 서로 변환하지 않는다.

실행 순서는 `EnvironmentRequirements·ReproductionPlan 확정 → Runtime Validator ALLOW → Sandbox Controller 외부 경계 ALLOW → Reproduction Setup Automation의 recipe·image·container·READY environment 준비 → Dynamic Reproduction Agent의 candidate·tool request·command·관찰 → DynamicReproductionConclusion → Reproduction Session Manager 최종 확정`으로 고정한다. 경계를 통과하기 전에 image build·container 생성·command 실행을 시작하지 않는다. Dynamic Reproduction Agent는 격리된 container 안에서 환경 설정, 저장소가 필요로 하는 package, 계정, fixture/mock, PoC, command, 관찰과 재시도를 자율적으로 선택할 수 있다. Dynamic Reproduction Agent는 Docker daemon이나 host를 직접 제어하지 않고 Reproduction Setup Automation이 제공한 in-container 실행 통로만 사용한다. 새로운 package registry나 외부 target 접근은 여전히 versioned profile의 egress 경계를 통과해야 한다. Dynamic Reproduction Agent가 선택한 command의 내용은 plan allowlist와 비교하지 않지만 모든 실제 event는 `AgentLog`에 기록한다.
...
같은 `RUN_SANDBOX`의 `ActionRequest.input_refs`와 `ActionDecision.checked_config_refs`에는 exact `SandboxProfile`과 exact `DynamicReproductionLifecycleProfile` revision이 모두 있어야 한다. `ActionRequest.sandbox_profile_ref`와 `SandboxPolicyDecision.sandbox_profile_ref`는 같은 R7 profile exact `record_id + content_hash`, `ActionRequest.resource_profile_ref`와 `SandboxPolicyDecision.resource_profile_ref`는 같은 R8 lifecycle profile exact `record_id + content_hash`를 가리켜야 한다. 어느 하나가 없거나 서로 다르거나 current revision이 바뀌면 실행 전 decision을 `EXPIRED`로 만들고 새 action을 요구한다.
...
Runtime Validator는 실행 직전 `RUN_SANDBOX` decision의 exact request·current requirements·current exact plan·R7 `sandbox_profile_ref`·exact R8 `DynamicReproductionLifecycleProfile`이 current인지 다시 검사하고 호출 전 잔여 시간·새 attempt 한도를 강제한다. Sandbox Controller는 R7 sandbox profile의 외부 격리·입장 수치 경계를 강제한다. 결과를 저장할 때 `SAVE_RESULT(requested_by=REPRODUCTION_SESSION_MANAGER, result_kind=dynamic_reproduction_result)`는 current Verification generation의 exact R6 request reference와 purpose가 결과에 그대로 연결됐는지 확인한다. 별도로 plan·recipe·정책 결정·환경·AgentLog·candidate·validated PoC·cleanup과 동적 결과가 같은 R7 `DYNAMIC_REPRO` work·attempt에 속하는지 다시 확인한다. Verification은 `DynamicReproductionState.dynamic_result_ref`, work output과 commit이 같은 final 결과만 읽으며 `DynamicReproductionResult`를 직접 만들거나 수정하지 않는다.
