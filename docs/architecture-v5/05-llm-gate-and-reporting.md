...
- Dynamic Reproduction Agent 호출 여부와 append-only AgentLog, recipe·실제 환경·container lifecycle, 정책 차단과 Controller 판정, 정리 필요 여부와 상태가 공통 계약에 맞는지
...
- **`SandboxPolicyDecision`의 `ALLOW`**는 `LOCAL_ONLY` 격리·실행 경계를 통과했다는 의미일 뿐 external/live testing authorization이 아니다. `report_permission`이나 Rule Scope Gate `PASS`도 external execution 허가가 아니다. `POLICY_BLOCKED`은 `SandboxProfile` 격리·외부 경계 enforcement 실패이지 `testing_restriction_compliance=FAIL`이 아니다.
- **Sandbox Controller**는 bug bounty program testing restriction의 semantic compliance를 판정하지 않는다. 전달받은 `sandbox_profile_ref` 경계와 `execution_scope=LOCAL_ONLY`만 강제한다: host·Docker daemon/socket·mount/namespace·secret·egress·다른 workspace 격리와 CPU·RAM·disk·PID·timeout. Sandbox 안에서 Dynamic Reproduction Agent가 고른 command·package·PoC를 program-policy allowlist처럼 의미 해석하지 않고 program policy allowlist를 자체 생성하지 않는다.
