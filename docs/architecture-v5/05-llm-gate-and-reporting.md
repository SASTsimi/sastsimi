...
`CWELabel`의 logical producer는 `CWE_LABELING`이고 R1~R8 실제 ownership은 R5-01이다. `CWE_LABELING`은 R5-01이 소유하는 logical producer/work role이며, final TRUE 뒤에 별도 work로 primary·alternative CWE, taxonomy version, 선택 이유, root cause와 evidence reference를 담은 current `CWELabel` revision을 확정한다. `HOLD`나 `FALSE`에 분석용 분류 메모를 남길 수는 있지만 Gate 입력 `CWELabel`이나 보고 가능한 취약점 라벨로 승격하지 않는다. 구분 근거가 부족하면 억지로 단일 CWE를 확정하지 않는다.

새 `VerificationResult` revision/generation에서는 이전 `CWELabel`을 current Gate input으로 그대로 재사용하지 않는다. 과거 label은 history/provenance로 보존하고 R5-01 `CWE_LABELING` work가 새 root cause·Evidence와 taxonomy 정렬을 재평가한다. 동일 CWE가 계속 적절할 수 있지만 그 경우에도 새 generation에 맞는 current label revision/provenance를 확정해야 한다. Technical Evidence Gate는 이 label을 읽고 정렬만 검토하며 생성·수정·덮어쓰지 않는다.
...
등록 가설, Pro/Con Evidence, 실제 code/entity/location/path, 현재 generation의 `DynamicReproductionRequest`와 `DynamicReproductionResult`, validated PoC, restriction·unresolved condition과 `CWELabel.evidence_refs`는 두 direct input에서 도달하는 transitive dependency다. 이를 `CALL_TECHNICAL_GATE`의 별도 top-level domain input으로 다시 전달하지 않는다.
...
`DynamicReproductionRequest`, `DynamicReproductionResult`, validated PoC는 별도의 최상위 Gate reference를 새로 만들지 않고,
...
- shared static evidence인 `CodeLocation`, `CodeSymbol`, `CodeFact`, `CodeRelation`, `StaticFactBundle`, `ToolRunResult`와 `RuleExecutionRecord`는 현재 분석의 `workspace_id + commit_id`, Verification/Evidence가 사용한 exact reference·revision과 hash를 확인한다. 직접적인 `hypothesis_id`는 요구하지 않으며 같은 commit의 여러 가설이 재사용할 수 있다. 규칙 기반 fact/result는 `CodeFact.producer.attempt_id`에서 current `ToolRunResult.attempt_id`와 그 `rule_execution_ref`가 가리키는 exact `RuleExecutionRecord` revision까지 closure를 따라간다. 다른 attempt·설정·catalog의 stale execution record, `NOT_EXECUTED | UNKNOWN` 규칙 또는 단순한 fact 부재를 `EXECUTED + hit_count=0` 근거로 사용할 수 없다.
...
특히 current Verification generation의 `dynamic_request_ref`와 `DynamicReproductionResult(status=SUCCEEDED, hypothesis_outcome=SUPPORTED)`에서는 exact validated `poc_ref`·`policy_decision_ref`·`environment_ref`·
`agent_log_ref`, `agent_invoked`, `environment_created`, `cleanup_required`·`cleanup_status`를 위
transitive closure로 읽는다. restriction, bypass candidate, unresolved condition과 같은 Verification에서
분리한 material child proposal의 재검증 여부도 해당 exact revision chain에서만 확인한다. `poc_candidate_ref`는 R7의 미검증 후보일 뿐이므로 final TRUE 또는 Gate 호출 근거로 승격하지 않는다.
...
- `agent_invoked`로 실제 Reproduction Agent 호출을 확인할 수 있고 같은 attempt의 `agent_log_ref`로 실행 provenance를 추적할 수 있는지, 실제 환경 생성 여부와 환경 reference, 정책 차단과 Controller 판정 reference, 정리 필요 여부와 상태가 공통 계약에 맞는지
...
- 실제 코드 경로, restriction·unresolved condition·반박·HOLD 조건과 limitation이 빠짐없이 정확하게 표현되었는지
...
- current generation의 exact `dynamic_request_ref`, 그 request에서 생성된 `dynamic_result_ref`와 `reproduction_plan_ref`가 exact chain을 이루고, Dynamic 결과의 `meta.hypothesis_id`, workspace·commit, verification generation, request·work·attempt linkage와 `hypothesis_linkage`가 현재 가설과 일치하는지 확인한다. 이전 generation 또는 서로 다른 generation·attempt의 request/result/PoC를 섞지 않는다.
- `policy_decision_ref`, `environment_ref`, `agent_log_ref`, observation과 실행 전제(권한·설정·입력·버전)가 기술 설명의
  공격 조건과 같은지 확인한다. 다른 환경에서의 결과를 현재 대상으로 일반화하지 않는다.
- `status`, `failure_category`, `hypothesis_outcome`, `agent_invoked`, `environment_created`,
...
- `poc_ref`의 존재만으로 PoC 실행, 재현 성공 또는 impact 입증을 주장하지 않는다. 생성되었지만
  실행되지 않은 PoC가 있을 수 있으며, 실행 PoC 주장은 Reproduction Agent가 실제 실행한 exact candidate와
  관련 AgentLog의 `POC_EXECUTE`·observation에 같은 revision 또는 digest로 연결되어야 한다.
- Dynamic 결과와 정책 판정·환경·AgentLog·실행 PoC·cleanup 기록은 같은 hypothesis·workspace·commit 및
...
- 필수 `EnvironmentRequirement`가 일치하지 않으면 R7이 조건을 완화하거나 공격 단계를 실행할 수 없다.
  ...
  새 `DynamicReproductionRequest`를 만들고 R7이 새 requirements·plan·PoC와 동적 결과를 생산하는 재실행·Verification revision 흐름을 밟는다. R6나 Gate가 R7 artifact를 직접 만들거나 수정하지 않는다.
...
request/result/status/PoC의 정본 compatibility 규칙은 [경량 데이터 계약](08-lightweight-data-contracts.md#dynamic-requestresultpoc-compatibility)을 따른다. final `TRUE`와 Technical Gate 입력에는 current generation의 exact request, 그 request에 exact linkage된 `SUCCEEDED + SUPPORTED` 결과, validated `poc_ref`가 모두 필요하다. 정책 차단, environment/precheck·실행 실패, timeout, `DISPROVED | INCONCLUSIVE` 또는 validated PoC 부재에서는 `poc_ref=null`이며 final TRUE나 Gate 입력으로 소비하지 않는다. 구조 불변조건 위반은 Runtime Validator·schema·semantic validation에서 Gate 전에 fail-closed로 차단한다. 의미 검토 중 linkage·completeness·provenance 부족이 드러나면 `REVISE | REJECT + NOT_READY`로 처리하며 임의로 `ACCEPT`하지 않는다. Technical Evidence Gate는 producer/schema 계약이나 공격 사실을 새로 만들지 않고, 현재 Verification revision이 참조하는 exact artifact와 실제 관측 범위만 검토한다.
...
`ACCEPT`는 final TRUE의 모든 핵심 claim과 반대 근거, 코드·동적·PoC·CWE·제한 reference가 현재 revision chain에 정렬되고 다음 소비자가 누락된 기술 근거를 새로 구성할 필요가 없을 때만 사용한다. 이는 정책상 보고 가능성, Reporter 실행 권한 또는 result가 있는 Primitive 자격을 단독으로 부여하지 않는다.
...
`REVISE`는 동일 입력 재투표나 provider retry가 아니다. 현재 Gate work는 `REVISE` review를 exact output으로 확정하고 종료한다. 그 review는 Orchestration을 경유해 목적지를 다시 선택하지 않고 같은 hypothesis의 ACTIVE `VerificationAssignment` owner에게 전달한다. runtime은 이전 종료 work를 되돌리지 않고 새 generation의 VERIFICATION work를 등록하며 hypothesis 상태를 `TERMINAL -> VERIFYING`으로 CAS 전환한다. Verification은 필요한 Context·Pro/Con·정적 근거·restriction을 보완하고, final TRUE 후보라면 새 generation의 동적 재현 요청과 validated PoC도 다시 확보한다. 새 result·work 종료·hypothesis current pointer를 atomic commit한다. 이전 CWELabel은 history/provenance로만 보존하고, R5-01 `CWE_LABELING` work가 새 root cause·Evidence 정렬을 재평가해 동일 CWE 유지 또는 변경이 반영된 current label revision/provenance를 확정한다. 그 뒤 새 `VerificationResult`와 current `CWELabel` revision을 가리키는 새 Gate work를 요청한다. 새 Gate work는 새 `input_hash`·`dedupe_key`·`work_id`와 `attempt_number=1`, `trigger=INITIAL`을 사용한다. 입력이 바뀌지 않은 호출 실패만 같은 work 안에서 `trigger=RETRY`인 새 attempt를 사용할 수 있다. 공통 token·시간·비용·work 예산을 소진하면 보고와 result Primitive admission을 차단하고 미해결 사유를 저장한다.
